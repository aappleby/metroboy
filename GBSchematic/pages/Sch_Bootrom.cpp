...
void Bootrom_tick(const Bus& bus,
  ...
  /*p07.TEXE*/ wire BOOT_BIT_RD = and(ctl.CPU_RD, dec.ADDR_FFXX, ADDR_x1x1xxxx, ADDR_0x0x0000);
  /*p07.SYPU*/ if (BOOT_BIT_RD) bus_out.D0 = prev.BOOT_BIT;

  /*p07.TULO*/ wire ADDR_00XX  = nor(bus.A15, bus.A14, bus.A13, bus.A12, bus.A11, bus.A10, bus.A09, bus.A08);
  /*p07.TERA*/ wire BOOT_BITn  = not(prev.BOOT_BIT);
  /*p07.TUTU*/ next.ADDR_BOOT  = and(BOOT_BITn, ADDR_00XX);

  /*p07.YAZA*/ wire MODE_DBG1n = not(dbg.MODE_DBG1);
  /*p07.YULA*/ wire BOOT_RD    = and(ctl.CPU_RD, MODE_DBG1n, prev.ADDR_BOOT);
...
void Bootrom_tock(const Bus& bus,
  ...
  /*p07.TUGE*/ wire BOOT_BIT_CLK = nand(ctl.CPU_WR, dec.ADDR_FFXX, ADDR_0x0x0000, ADDR_x1x1xxxx);
  /*p07.SATO*/ wire BOOT_BIT_IN  = or(bus.D0, prev.BOOT_BIT);
  /*p07.TEPU*/ next.BOOT_BIT.tock(BOOT_BIT_CLK, rst.SYS_RESETn, BOOT_BIT_IN);
}
