...
void DMA_tick(const Bus& bus,
  ...
  {
    /*p04.LAVY*/ wire FF46_WRn = nand(dec.FF46, ctl.CPU_WR2);
    /*p04.LOKO*/ wire DMA_RST  = nand(rst.sig.CUNU_RESETn, !dma.DMA_ENb);
    ...
    /*p04.LUVY*/ next.DMA_ENa.tock(clk.UVYT_xBCDExxx, rst.sig.CUNU_RESETn, DMA_EN);
    /*p04.LENE*/ next.DMA_ENb.tock(clk.MOPA_AxxxxFGH, rst.sig.CUNU_RESETn, dma.DMA_ENa);
  }
...

  {
    ...
    if (dma.DMA_DONE_SYNC || !rst.sig.CUNU_RESETn) {
  ...
  /*p04.MATU*/ next.DMA_CLKEN.tock(clk.UVYT_xBCDExxx, rst.sig.CUNU_RESETn, dma.DMA_CLKEN_LATCH);
  ...
  {
    /*p04.MAKA*/ next.FROM_CPU5_SYNC.tock(clk.ZEME_AxCxExGx, rst.sig.CUNU_RESETn, cpu.FROM_CPU5);
