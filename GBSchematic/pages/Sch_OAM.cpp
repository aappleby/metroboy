...
void OAM_tick(const Bus& bus,
...

  {
    ...
    /*p29.WOJO*/ wire WOJO = nor(!clk.sig.WUVU_xxCDxxGH, !clk.sig.WOSU_xBCxxFGx);
    /*p29.XYSO*/ wire XYSO = not(WOJO);
    /*p25.AVER*/ wire AVER = and(spr.OAM_ADDR_PARSE, XYSO);
    /*p28.AJEP*/ wire AJEP = and(spr.OAM_ADDR_PARSE, clk.sig.XOCE_AxxDExxH);
    ...
    /*p25.CUFE*/ wire OAM_WR      = and(or(dec.ADDR_OAM, dma.DMA_CLKEN), clk.sig.MOPA_AxxxxFGH); // Possible schematic error - CUFE doesn't make sense as or(and()), only as and(or())
...

  {
    ...
    /*p28.XYNY*/ wire CPU_OAM_WR_CLK1 = not(clk.sig.MOPA_AxxxxFGH);
