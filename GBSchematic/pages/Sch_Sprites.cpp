...
void Sprites_tick(const Pins& pins,
...

  {
    ...
    {
      /*p29.TOMA*/ wire SEQ_CLK = nand(clocks.LAPE_AxCxExGx, SEQ_5n);
...

    {
      ...
      /*p29.TYFO*/ next.SEQ_B0d.tock    (clocks.LAPE_AxCxExGx, VYPO, next.SPR_SEQ0);
      /*p29.TOBU*/ next.SEQ_xx23xx .tock(clocks.TAVA_xBxDxFxH, vid.RENDERING_LATCH, spr.SPR_SEQ1);    // note input is seq 1 not 2
      /*p29.VONU*/ next.SEQ_xxx34xn.tock(clocks.TAVA_xBxDxFxH, vid.RENDERING_LATCH, spr.SEQ_xx23xx);
      /*p29.SEBA*/ next.SEQ_xxxx45n.tock(clocks.LAPE_AxCxExGx, vid.RENDERING_LATCH, spr.SEQ_xxx34xn); // is this clock wrong?
    }
...

  {
    ...
    /*p29.CARE*/ wire STORE_ENn = or(clocks.XOCE_AxxDExxH, CEHA, SPR_MATCH_Y);
    /*p29.DYTY*/ next.STORE_EN = not(STORE_ENn);


    /*p28.WEFE*/ wire WEFE = not(pins.P10_B);
    /*p30.CYKE*/ wire CYKE = not(clocks.XUPY_ABxxEFxx);
