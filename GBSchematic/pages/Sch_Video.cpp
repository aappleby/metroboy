...
void P21_VideoControl_tick(const Bus& bus,
...

  {
    ...
    // But we don't enter sprite mode until background sequencer hits BG_SEQ5
    {
      ...
      /*p27.SECA*/ wire SPRITE_FETCH_BEGINn = nor(vid.SPRITE_FETCH_TRIG, rst.sig.VID_RESET5, lcd.VID_LINE_TRIG_d4n);
  ...
  {
    /*p21.WEGO*/ wire WEGO = or(rst.sig.VID_RESET3, vid.RENDER_DONE_SYNC);
...

  {
    ...
    /*p27.PYCO*/ next.WIN_MATCH_SYNC1.tock(vid.ROCO_4M,          rst.sig.VID_RESETn, vid.WIN_MATCH);
    /*p27.NUNU*/ next.WIN_MATCH_SYNC2.tock(clk.MEHE_AxCxExGx, rst.sig.VID_RESETn, vid.WIN_MATCH_SYNC1);

    /*p27.XOFO*/ wire LINE_RST = nand(regs.LCDC_WINEN, lcd.VID_LINE_TRIG_d4o, rst.sig.VID_RESETn);
    ...
    /*p27.NOPA*/ next.WIN_MODE_SYNC.tock(clk.ALET_xBxDxFxH, rst.sig.VID_RESETn, vid.WIN_MODE_LATCH);
    ...
    ///*p27.RYDY*/ next.RYDY = nor(vid.PUKU, rst.VID_RESET4, vid.BG_SEQ_7);

    if (vid.WIN_MODE_TRIG) {
      next.PUKU = 0;
      next.WIN_MODE_LATCH2 = 1;
    }
    if (rst.sig.VID_RESET4 || vid.BG_SEQ_7) {
      next.PUKU = 1;
    ...
    /*p27.SOVY*/ next.WIN_MODE_SYNC2.tock(clk.ALET_xBxDxFxH, rst.sig.VID_RESETn, vid.WIN_MODE_LATCH2);
  }
...

  {
    ...
    /*p21.ROPO*/ next.LYC_MATCH.tock(clk.TALU_xxxxEFGH, rst.sig.WESY_RESET, LY_MATCHa);
  }
...

  {
    ...
    /*p21.ROXE*/ next.INT_HBL_EN.tock(CLK_STAT, rst.sig.WESY_RESET, bus.D3);
    /*p21.RUFO*/ next.INT_VBL_EN.tock(CLK_STAT, rst.sig.WESY_RESET, bus.D4);
    /*p21.REFE*/ next.INT_OAM_EN.tock(CLK_STAT, rst.sig.WESY_RESET, bus.D5);
    /*p21.RUGU*/ next.INT_LYC_EN.tock(CLK_STAT, rst.sig.WESY_RESET, bus.D6);
    ...
    /*p21.PAGO*/ wire STAT_LYC_MATCH1 = nor(rst.sig.WESY_RESET, FF41_WRn);  // schematic wrong, this is NOR
...

  {
    ...
    /*p27.SARY*/ next.WY_MATCH_SYNC.tock(clk.TALU_xxxxEFGH, rst.sig.VID_RESETn, WY_MATCH);
  }
...

  {
    ...
    /*p27.REPU*/ wire IN_FRAME_Y  = nor(lcd.VBLANK_d4b, rst.sig.VID_RESET4);   // schematic wrong, this is NOR
...

  {
    ...
    /*p27.XOFO*/ wire X_RST = nand(regs.LCDC_WINEN, lcd.VID_LINE_TRIG_d4o, rst.sig.VID_RESETn);
    /*p27.REPU*/ wire Y_RST  = nor(lcd.VBLANK_d4b, rst.sig.VID_RESET4);   // schematic wrong, this is NOR
  ...
  {
    /*p21.TADY*/ next.X_RST = nor(lcd.VID_LINE_TRIG_d4n, rst.sig.VID_RESET3);
