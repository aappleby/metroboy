  ...
  void sim_slow(int phases) {
    for (int p = 0; p < phases; p++) {
      ...
      for (int pass = 0; pass < 12; pass++) {
        ...
        ResetSignals1 rst_sig = ResetSignals1::tick(prev.rst_reg, MODE_DBG1, MODE_DBG2, RST, clk_sig1.CLK_BAD1, clk_sig1.CPUCLK_REQn, clk_sig1.BOGA_AxCDEFGH, DIV_15, LCDC_EN);

        clk_sig2 = ClockSignals2::tick_slow(prev.clk_reg);


        LCD::tick_slow(clk_sig1, clk_sig2, rst_sig, prev.lcd, prev.vid, prev.spr.SCAN_DONE_d0_TRIG, DIV_06n, DIV_07n, LCDC_EN, lcd);
        Sprites_tickScanner(clk_sig1, clk_sig2, prev.lcd, rst_sig, prev.spr, spr);
        ...
        ResetRegisters::tock(rst_sig, prev.rst_reg, MODE_PROD, MODE_DBG1, MODE_DBG2, RST, clk_sig1.CLK_BAD1, clk_sig1.CPUCLK_REQn, clk_sig1.BOGA_AxCDEFGH, DIV_15, rst_reg);
      }
    }
  }
  ...
  void sim_fast(int phases) {
    for (int p = 0; p < phases; p++) {
      ...
      for (int pass = 0; pass < 8; pass++) {
        ...
        clk_sig2 = ClockSignals2::tick_fast(clk_phase, rst_sig.VID_RESETn);


        LCD::tick_fast(clk_sig1, clk_sig2, rst_sig, prev.vid, prev.spr.SCAN_DONE_d0_TRIG, DIV_06n, DIV_07n, LCDC_EN, lcd);
        Sprites_tickScanner(clk_sig1, clk_sig2, prev.lcd, rst_sig, prev.spr, spr);
        dec.tick(bus, prev.clk_reg, BOOT_BIT, MODE_DBG2, ADDR_VALID);
