...
void GateBoyApp::app_render_frame(Viewport view) {
  ...
  */
  ...
  d("\002===== Disasm =====\001\n");
  {
    ...
    if (!bit(gb->cpu_bus.TEPU_BOOT_BITn_h.qp_old2())) {
  ...
  gb_blitter.blit_map   (view, 52 * 32 - 8,   0 * 32 + 8,  1, vid_ram, bit(gb->reg_lcdc.XAFO_LCDC_BGMAPn.qn_old2()),  bit(gb->reg_lcdc.WEXU_LCDC_BGTILEn.qn_old2()));
  gb_blitter.blit_map   (view, 52 * 32 - 8,   8 * 32 + 16, 1, vid_ram, bit(gb->reg_lcdc.WOKY_LCDC_WINMAPn.qn_old2()), bit(gb->reg_lcdc.WEXU_LCDC_BGTILEn.qn_old2()));
