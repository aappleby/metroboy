...
void GateBoyBootrom::read_boot_bit(GateBoyCpuBus& cpu_bus)
{
  ...
  /* p07.SYPU*/ cpu_bus.BUS_CPU_D00p.tri6_pn(_TEXE_FF50_RDp, cpu_bus.TEPU_BOOT_BITn_h.qp_new2());
}
...
void GateBoyBootrom::write_boot_bit_sync(GateBoyResetDebug& rst, GateBoyCpuBus& cpu_bus)
{
  ...
  /* p07.SATO*/ wire _SATO_BOOT_BITn_old = or2(cpu_bus.BUS_CPU_D00p.qp_old2(), cpu_bus.TEPU_BOOT_BITn_h.qp_old2());
...
void GateBoyBootrom::read_bootrom(GateBoyResetDebug& rst, GateBoyCpuBus& cpu_bus, uint8_t* boot_buf)
{
  ...
  /* p07.ZORO*/ wire _ZORO_0000xxxx_XX = nor4b(cpu_bus.BUS_CPU_A15p.qp_new2(), cpu_bus.BUS_CPU_A14p.qp_new2(), cpu_bus.BUS_CPU_A13p.qp_new2(), cpu_bus.BUS_CPU_A12p.qp_new2());
  /* p07.ZADU*/ wire _ZADU_xxxx0000_XX = nor4b(cpu_bus.BUS_CPU_A11p.qp_new2(), cpu_bus.BUS_CPU_A10p.qp_new2(), cpu_bus.BUS_CPU_A09p.qp_new2(), cpu_bus.BUS_CPU_A08p.qp_new2());
