...
void GateBoy::tock_clocks() {
  ...
  /* p01.ARYS*/ wire ARYS = not1(clk.PIN_74_CLK.clock());
  /* p01.AVET*/ clk.AVET = nand2(clk.ANOS, ARYS);
  /* p01.ANOS*/ clk.ANOS = nand2(clk.PIN_74_CLK.clock(), clk.AVET);
  /* p01.AVET*/ clk.AVET = nand2(clk.ANOS, ARYS);
  /* p01.ANOS*/ clk.ANOS = nand2(clk.PIN_74_CLK.clock(), clk.AVET);
  ...
  /*PIN_75*/ clk.PIN_75_CLK_OUT.pin_out_dp(clk.BUDE_xxxxEFGH());
  /*SIG_CPU_BOWA_Axxxxxxx*/ clk.SIG_CPU_BOWA_Axxxxxxx.sig_out(clk.BOWA_xBCDEFGH());
  /*SIG_CPU_BEDO_xBCDEFGH*/ clk.SIG_CPU_BEDO_xBCDEFGH.sig_out(clk.BEDO_Axxxxxxx());
  /*SIG_CPU_BEKO_ABCDxxxx*/ clk.SIG_CPU_BEKO_ABCDxxxx.sig_out(clk.BEKO_ABCDxxxx());
  /*SIG_CPU_BUDE_xxxxEFGH*/ clk.SIG_CPU_BUDE_xxxxEFGH.sig_out(clk.BUDE_xxxxEFGH());
  /*SIG_CPU_BOLO_ABCDEFxx*/ clk.SIG_CPU_BOLO_ABCDEFxx.sig_out(clk.BOLO_ABCDEFxx());
  /*SIG_CPU_BUKE_AxxxxxGH*/ clk.SIG_CPU_BUKE_AxxxxxGH.sig_out(clk.BUKE_AxxxxxGH());
  /*SIG_CPU_BOMA_xBCDEFGH*/ clk.SIG_CPU_BOMA_xBCDEFGH.sig_out(clk.BOMA_xBCDEFGH());
  /*SIG_CPU_BOGA_Axxxxxxx*/ clk.SIG_CPU_BOGA_Axxxxxxx.sig_out(clk.BOGA_Axxxxxxx());
}

//------------------------------------------------------------------------------------------------------------------------

void GateBoy::tock_vid_clocks() {
  /* p29.WOSU*/ clk.WOSU_AxxDExxH.dff17(clk.XYFY_xBxDxFxH(),        rst.XAPO_VID_RSTn(), clk.WUVU_ABxxEFxx.qn_old());
  /* p29.WUVU*/ clk.WUVU_ABxxEFxx.dff17(clk.XOTA_AxCxExGx(),        rst.XAPO_VID_RSTn(), clk.WUVU_ABxxEFxx.qn_old());
