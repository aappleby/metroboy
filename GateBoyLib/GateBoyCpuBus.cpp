  ...
  Req bus_req)
{
  ...
  BUS_CPU_A[ 0] = wire((bus_addr >>  0) & 1);
  BUS_CPU_A[ 1] = wire((bus_addr >>  1) & 1);
  BUS_CPU_A[ 2] = wire((bus_addr >>  2) & 1);
  BUS_CPU_A[ 3] = wire((bus_addr >>  3) & 1);
  BUS_CPU_A[ 4] = wire((bus_addr >>  4) & 1);
  BUS_CPU_A[ 5] = wire((bus_addr >>  5) & 1);
  BUS_CPU_A[ 6] = wire((bus_addr >>  6) & 1);
  BUS_CPU_A[ 7] = wire((bus_addr >>  7) & 1);
  BUS_CPU_A[ 8] = wire((bus_addr >>  8) & 1);
  BUS_CPU_A[ 9] = wire((bus_addr >>  9) & 1);
  BUS_CPU_A[10] = wire((bus_addr >> 10) & 1);
  BUS_CPU_A[11] = wire((bus_addr >> 11) & 1);
  BUS_CPU_A[12] = wire((bus_addr >> 12) & 1);
  BUS_CPU_A[13] = wire((bus_addr >> 13) & 1);
  BUS_CPU_A[14] = wire((bus_addr >> 14) & 1);
  BUS_CPU_A[15] = wire((bus_addr >> 15) & 1);

  wire bus_oe = (DELTA_DE || DELTA_EF || DELTA_FG || DELTA_GH) && bus_req.write;
  BUS_CPU_D[0] = wire(!bus_oe || (bus_req.data_lo >> 0) & 1);
  BUS_CPU_D[1] = wire(!bus_oe || (bus_req.data_lo >> 1) & 1);
  BUS_CPU_D[2] = wire(!bus_oe || (bus_req.data_lo >> 2) & 1);
  BUS_CPU_D[3] = wire(!bus_oe || (bus_req.data_lo >> 3) & 1);
  BUS_CPU_D[4] = wire(!bus_oe || (bus_req.data_lo >> 4) & 1);
  BUS_CPU_D[5] = wire(!bus_oe || (bus_req.data_lo >> 5) & 1);
  BUS_CPU_D[6] = wire(!bus_oe || (bus_req.data_lo >> 6) & 1);
  BUS_CPU_D[7] = wire(!bus_oe || (bus_req.data_lo >> 7) & 1);
