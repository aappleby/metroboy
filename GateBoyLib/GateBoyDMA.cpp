...
uint8_t GateBoyDMA::LUMA_DMA_CARTp() const {
  /*#p04.LEBU*/ uint8_t _LEBU_DMA_A15n  = not1(MARU_DMA_A15n.qn_new());
  /*#p04.MUDA*/ uint8_t _MUDA_DMA_VRAMp = nor3(PULA_DMA_A13n.qn_new(), POKU_DMA_A14n.qn_new(), _LEBU_DMA_A15n);
  /* p04.LOGO*/ uint8_t _LOGO_DMA_VRAMn = not1(_MUDA_DMA_VRAMp);
  /* p04.MORY*/ uint8_t _MORY_DMA_CARTn = nand2(MATU_DMA_RUNNINGp.qp_new(), _LOGO_DMA_VRAMn);
  /* p04.LUMA*/ uint8_t _LUMA_DMA_CARTp = not1(_MORY_DMA_CARTn);
  return _LUMA_DMA_CARTp;
}

//------------------------------------------------------------------------------------------------------------------------

uint8_t GateBoyDMA::LUFA_DMA_VRAMp() const {
  /*#p04.LEBU*/ uint8_t _LEBU_DMA_A15n  = not1(MARU_DMA_A15n.qn_new());
  /*#p04.MUDA*/ uint8_t _MUDA_DMA_VRAMp = nor3(PULA_DMA_A13n.qn_new(), POKU_DMA_A14n.qn_new(), _LEBU_DMA_A15n);
  /* p04.MUHO*/ uint8_t _MUHO_DMA_VRAMp = nand2(MATU_DMA_RUNNINGp.qp_new(), _MUDA_DMA_VRAMp);
  /* p04.LUFA*/ uint8_t _LUFA_DMA_VRAMp = not1(_MUHO_DMA_VRAMp);
...
void GateBoyDMA::write_dma_sync(GateBoyCpuBus& cpu_bus) {
  /*#p04.LAVY*/ uint8_t LAVY_FF46_WRp = and2(CUPA_CPU_WRp(cpu_bus.TAPU_CPU_WRp.qp_new()), XEDA_FF46p(cpu_bus.BUS_CPU_A));
  /*#p04.LORU*/ uint8_t LORU_FF46_WRn = not1(LAVY_FF46_WRp);
...
{
  /*#p04.LAVY*/ uint8_t LAVY_FF46_WRp = and2(CUPA_CPU_WRp(cpu_bus.TAPU_CPU_WRp.qp_new()), XEDA_FF46p(cpu_bus.BUS_CPU_A));
  /*#p04.LUPA*/ uint8_t _LUPA_DMA_TRIG_old = nor2(LAVY_FF46_WRp, LYXE_DMA_LATCHp.qn_old());
  ...
  /*#p04.LOKO*/ uint8_t _LOKO_DMA_RSTp = nand2(LENE_DMA_TRIG_d4.qn_new(), rst.CUNU_SYS_RSTn());
  /*#p04.LYXE*/ LYXE_DMA_LATCHp.nor_latch(LAVY_FF46_WRp, _LOKO_DMA_RSTp);

  /*#p04.LAPA*/ uint8_t _LAPA_DMA_RSTn = not1(_LOKO_DMA_RSTp);

  /*#p04.NAVO*/ uint8_t _NAVO_DMA_DONEn_old = nand6(NAKY_DMA_A00p.qp_old(), PYRO_DMA_A01p.qp_old(),  // 128+16+8+4+2+1 = 159
                                                  NEFY_DMA_A02p.qp_old(), MUTY_DMA_A03p.qp_old(),
                                                  NYKO_DMA_A04p.qp_old(), MUGU_DMA_A07p.qp_old());
  /*#p04.NOLO*/ uint8_t _NOLO_DMA_DONEp_old = not1(_NAVO_DMA_DONEn_old);
  ...
  /*#p04.META*/ uint8_t _META_DMA_CLKp = and2(clk.UVYT_ABCDxxxx(), LOKY_DMA_LATCHp.qp_new());
...
void GateBoyDMA::read_dma(GateBoyCpuBus& cpu_bus) {
  /*#p04.MOLU*/ uint8_t MOLU_FF46_RDp = and2(ASOT_CPU_RDp(cpu_bus.TEDO_CPU_RDp.qp_new()), XEDA_FF46p(cpu_bus.BUS_CPU_A));
  /*#p04.NYGO*/ uint8_t NYGO_FF46_RDn = not1(MOLU_FF46_RDp);
  /*#p04.PUSY*/ uint8_t PUSY_FF46_RDp = not1(NYGO_FF46_RDn);
