...
struct ExtDataLatch {
  void reset_to_cart() {
    ...
    SELO_EXT_DATA_LATCH_D3n.reset(REG_D1C0);
...
struct ExtAddrLatch {
  void reset_to_cart() {
    ...
    ARET_EXT_ADDR_LATCH_03p.reset(REG_D0C0);
    AVYS_EXT_ADDR_LATCH_04p.reset(REG_D0C0);
    ATEV_EXT_ADDR_LATCH_05p.reset(REG_D0C0);
    AROS_EXT_ADDR_LATCH_06p.reset(REG_D0C0);
    ARYM_EXT_ADDR_LATCH_07p.reset(REG_D0C0);
    LUNO_EXT_ADDR_LATCH_08p.reset(REG_D1C0);
...
struct GateBoyExtBus {
  ...
  void pins_to_data_latch(const GateBoyCpuBus& cpu_bus);
  void data_latch_to_cpu_bus(GateBoyCpuBus& cpu_bus);
