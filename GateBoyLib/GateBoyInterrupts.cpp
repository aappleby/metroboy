...
void GateBoyInterrupts::read_intf(GateBoyCpuBus& cpu_bus) {
  /* p07.ROLO*/ wire2 _ROLO_FF0F_RDn = nand4b(cpu_bus.TEDO_CPU_RDp.qp_new(), cpu_bus.SYKE_ADDR_HIp(), cpu_bus.SEMY_XX_0000xxxxp(), cpu_bus.SAPA_XX_xxxx1111p()); // schematic wrong, is NAND
...
void GateBoyInterrupts::read_ie(GateBoyCpuBus& cpu_bus)
{
  ...
  wire2 FFFF_RDn_ext = nand2b(cpu_bus.TEDO_CPU_RDp.qp_new(), FFFF_HIT_ext);
...
void GateBoyInterrupts::write_ie(const GateBoyResetDebug& rst, GateBoyCpuBus& cpu_bus)
{
  uint16_t cpu_addr = (uint16_t)BitBase::pack_new(16, cpu_bus.BUS_CPU_A);
  wire2 FFFF_HIT_ext = cpu_addr == 0xFFFF;
  wire2 FFFF_WRn_ext = nand2b(cpu_bus.TAPU_CPU_WRp.qp_new(), FFFF_HIT_ext);
  ...
  wire2 WODU_HBLANKp)
{
  ...
  /* p07.REFA*/ wire2 _REFA_FF0F_WRn = nand4b(cpu_bus.TAPU_CPU_WRp.qp_new(), cpu_bus.SYKE_ADDR_HIp(), cpu_bus.SEMY_XX_0000xxxxp(), cpu_bus.SAPA_XX_xxxx1111p()); // schematic wrong, is NAND
  ...
  /* p02.LETY*/ wire2 _LETY_INT_VBL_ACKn  = not1b(SIG_CPU_ACK_VBLANK.qp_new());
  /* p02.LEJA*/ wire2 _LEJA_INT_STAT_ACKn = not1b(SIG_CPU_ACK_STAT.qp_new());
  /* p02.LESA*/ wire2 _LESA_INT_TIM_ACKn  = not1b(SIG_CPU_ACK_TIMER.qp_new());
  /* p02.LUFE*/ wire2 _LUFE_INT_SER_ACKn  = not1b(SIG_CPU_ACK_SERIAL.qp_new());
  /* p02.LAMO*/ wire2 _LAMO_INT_JOY_ACKn  = not1b(SIG_CPU_ACK_JOYPAD.qp_new());

  /* p02.ROTU*/ wire2 _ROTU_FF0F_WRp   = not1b(_REFA_FF0F_WRn);
  /* p02.MYZU*/ wire2 _MYZU_FF0F_SET0n = nand3b(_ROTU_FF0F_WRp, _LETY_INT_VBL_ACKn,  cpu_bus.BUS_CPU_D[0].qp_new());
  /* p02.MODY*/ wire2 _MODY_FF0F_SET1n = nand3b(_ROTU_FF0F_WRp, _LEJA_INT_STAT_ACKn, cpu_bus.BUS_CPU_D[1].qp_new());
  /* p02.PYHU*/ wire2 _PYHU_FF0F_SET2n = nand3b(_ROTU_FF0F_WRp, _LESA_INT_TIM_ACKn,  cpu_bus.BUS_CPU_D[2].qp_new());
  /* p02.TOME*/ wire2 _TOME_FF0F_SET3n = nand3b(_ROTU_FF0F_WRp, _LUFE_INT_SER_ACKn,  cpu_bus.BUS_CPU_D[3].qp_new());
  /* p02.TOGA*/ wire2 _TOGA_FF0F_SET4n = nand3b(_ROTU_FF0F_WRp, _LAMO_INT_JOY_ACKn,  cpu_bus.BUS_CPU_D[4].qp_new());
