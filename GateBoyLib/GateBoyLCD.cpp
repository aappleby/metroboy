...
wire GateBoyLCD::ATEJ_LINE_RSTp_old() const {
  /* p28.ABAF*/ wire _ABAF_LINE_P000n_old = not1b(CATU_LINE_P000p.qp_old2());
  /* p28.BYHA*/ wire _BYHA_LINE_TRIGn_old = or_and3(ANEL_LINE_P002p.qp_old2(), _ABAF_LINE_P000n_old, ABEZ_VID_RSTn(_XODO_VID_RSTp.qp_old2())); // so if this is or_and, BYHA should go low on 910 and 911
  /* p28.ATEJ*/ wire _ATEJ_LINE_RSTp_old = not1b(_BYHA_LINE_TRIGn_old);
...
wire GateBoyLCD::ATEJ_LINE_RSTp_new() const {
  /* p28.ABAF*/ wire _ABAF_LINE_P000n_new = not1b(CATU_LINE_P000p.qp_new2());
  /* p28.BYHA*/ wire _BYHA_LINE_TRIGn_new = or_and3(ANEL_LINE_P002p.qp_new2(), _ABAF_LINE_P000n_new, ABEZ_VID_RSTn(_XODO_VID_RSTp.qp_new2())); // so if this is or_and, BYHA should go low on 910 and 911
  /* p28.ATEJ*/ wire _ATEJ_LINE_RSTp_new = not1b(_BYHA_LINE_TRIGn_new);
  ...
  wire RUTU_x113p_old)
{
  ...
  /*#p21.PURE*/ wire _PURE_LINE_ENDn_old = not1b(RUTU_x113p_old);
  /*#p21.SELA*/ wire _SELA_LINE_P908p_old = not1b(_PURE_LINE_ENDn_old);
  /*#p29.ALES*/ wire _ALES_y144n_old = not1b(XYVO_y144p_old);
...
void GateBoyLCD::set_pin_flip(GateBoyResetDebug& rst, wire TULU_DIV07p, wire XONA_LCDC_LCDENp) {
  ...
  /*#p24.MAGU*/ wire _MAGU = xor2(NAPO_FRAME_EVENp.qp_new2(), LUCA_LINE_EVENp.qn_new2());
  /*#p24.MECO*/ wire _MECO = not1b(_MAGU);
  /*#p24.KEBO*/ wire _KEBO = not1b(_MECO);
  /* p01.UREK*/ wire _UREK_DIV07n = not1b(TULU_DIV07p);
  /*#p24.USEC*/ wire _USEC_DIV07p = not1b(_UREK_DIV07n);
...
void GateBoyLCD::set_pin_hsync(GateBoyResetDebug& rst, wire TYFA_CLKPIPE_odd, wire XYMU_RENDERINGp, wire XYDO_PX3p, wire AVAP_SCAN_DONE_TRIGp) {
  ...
  /*#p24.POME*/ POME = nor2b(AVAP_SCAN_DONE_TRIGp, POFY.qp_old2());
  /*#p24.RUJU*/ RUJU = or3(PAHO_X_8_SYNC.qp_new2(), rst.TOFU_VID_RSTp(), POME.qp_new2());
  /*#p24.POFY*/ POFY = not1b(RUJU.qp_new2());
  /*#p24.POME*/ POME = nor2b(AVAP_SCAN_DONE_TRIGp, POFY.qp_new2());
  /*#p24.RUJU*/ RUJU = or3(PAHO_X_8_SYNC.qp_new2(), rst.TOFU_VID_RSTp(), POME.qp_new2());
  /*#p24.POFY*/ POFY = not1b(RUJU.qp_new2());
...
void GateBoyLCD::set_pin_latch(GateBoyDiv& div, RegLCDC& reg_lcdc) {
  /* p01.UMEK*/ wire _UMEK_DIV06n = not1b(div.UGOT_DIV06p.qp_new2());
  /*#p24.KASA*/ wire _KASA_LINE_ENDp = not1b(reg_lx.PURE_LINE_ENDn());
  /*#p24.UMOB*/ wire _UMOB_DIV_06p = not1b(_UMEK_DIV06n);
...
void GateBoyLCD::set_pin_clock(PixCount& pix_count, FineScroll& fine_scroll, wire WEGO_HBLANKp, wire SACU_CLKPIPE_evn) {
  /*#p21.XAJO*/ wire _XAJO_X_009p = and2(pix_count.XEHO_PX0p.qp_new2(), pix_count.XYDO_PX3p.qp_new2());
