...
wire2 GateBoyLCD::ATEJ_LINE_RSTp_old() const {
  /* p28.ABAF*/ wire2 _ABAF_LINE_P000n_old = not1(CATU_LINE_P000p.qp_old());
  /* p28.BYHA*/ wire2 _BYHA_LINE_TRIGn_old = or_and3(ANEL_LINE_P002p.qp_old(), _ABAF_LINE_P000n_old, ABEZ_VID_RSTn(_XODO_VID_RSTp.qp_old())); // so if this is or_and, BYHA should go low on 910 and 911
  /* p28.ATEJ*/ wire2 _ATEJ_LINE_RSTp_old = not1(_BYHA_LINE_TRIGn_old);
...
wire2 GateBoyLCD::ATEJ_LINE_RSTp_new() const {
  /* p28.ABAF*/ wire2 _ABAF_LINE_P000n_new = not1(CATU_LINE_P000p.qp_new());
  /* p28.BYHA*/ wire2 _BYHA_LINE_TRIGn_new = or_and3(ANEL_LINE_P002p.qp_new(), _ABAF_LINE_P000n_new, ABEZ_VID_RSTn(_XODO_VID_RSTp.qp_new())); // so if this is or_and, BYHA should go low on 910 and 911
  /* p28.ATEJ*/ wire2 _ATEJ_LINE_RSTp_new = not1(_BYHA_LINE_TRIGn_new);
  ...
  wire2 RUTU_x113p_old)
{
  ...
  /*#p21.PURE*/ wire2 _PURE_LINE_ENDn_old = not1(RUTU_x113p_old);
  /*#p21.SELA*/ wire2 _SELA_LINE_P908p_old = not1(_PURE_LINE_ENDn_old);
  /*#p29.ALES*/ wire2 _ALES_y144n_old = not1(XYVO_y144p_old);
...
void GateBoyLCD::set_pin_ctrl(GateBoyResetDebug& rst, GateBoyClock& clk) {
  ...
  /*#p21.RYNO*/ wire2 _RYNO = or2(SYGU_LINE_STROBE.qp_new(), reg_lx.RUTU_x113p_new());
  /*#p21.POGU*/ wire2 _POGU = not1(_RYNO);
...
void GateBoyLCD::set_pin_flip(GateBoyResetDebug& rst, wire2 TULU_DIV07p, wire2 XONA_LCDC_LCDENp) {
  ...
  /*#p24.MAGU*/ wire2 _MAGU = xor2(NAPO_FRAME_EVENp.qp_new(), LUCA_LINE_EVENp.qn_new());
  /*#p24.MECO*/ wire2 _MECO = not1(_MAGU);
  /*#p24.KEBO*/ wire2 _KEBO = not1(_MECO);
  /* p01.UREK*/ wire2 _UREK_DIV07n = not1(TULU_DIV07p);
  /*#p24.USEC*/ wire2 _USEC_DIV07p = not1(_UREK_DIV07n);
  /*#p24.KEDY*/ wire2 _KEDY_LCDC_ENn = not1(XONA_LCDC_LCDENp);
  /*#p24.KUPA*/ wire2 _KUPA = amux2(XONA_LCDC_LCDENp, _KEBO, _KEDY_LCDC_ENn, _USEC_DIV07p);
  /*#p24.KOFO*/ wire2 _KOFO = not1(_KUPA);
...
void GateBoyLCD::set_pin_vsync(GateBoyResetDebug& rst) {
  ...
  /*#p24.MURE*/ wire2 _MURE_VSYNC = not1(MEDA_VSYNC_OUTn.qp_new());
...
void GateBoyLCD::set_pin_hsync(GateBoyResetDebug& rst, wire2 TYFA_CLKPIPE_odd, wire2 XYMU_RENDERINGp, wire2 XYDO_PX3p, wire2 AVAP_SCAN_DONE_TRIGp) {
  ...
  /*#p24.RUZE*/ wire2 _RUZE_HSYNCn = not1(POFY.qp_new());
...
void GateBoyLCD::set_pin_latch(GateBoyDiv& div, RegLCDC& reg_lcdc) {
  /* p01.UMEK*/ wire2 _UMEK_DIV06n = not1(div.UGOT_DIV06p.qp_new());
  /*#p24.KASA*/ wire2 _KASA_LINE_ENDp = not1(reg_lx.PURE_LINE_ENDn());
  /*#p24.UMOB*/ wire2 _UMOB_DIV_06p = not1(_UMEK_DIV06n);
  /*#p24.KEDY*/ wire2 _KEDY_LCDC_ENn = not1(reg_lcdc.XONA_LCDC_LCDENn.qn_new());
  /*#p24.KAHE*/ wire2 _KAHE_LINE_ENDp = amux2(reg_lcdc.XONA_LCDC_LCDENn.qn_new(), _KASA_LINE_ENDp, _KEDY_LCDC_ENn, _UMOB_DIV_06p);
  /*#p24.KYMO*/ wire2 _KYMO_LINE_ENDn = not1(_KAHE_LINE_ENDp);
...
void GateBoyLCD::set_pin_clock(PixCount& pix_count, FineScroll& fine_scroll, wire2 WEGO_HBLANKp, wire2 SACU_CLKPIPE_evn) {
  /*#p21.XAJO*/ wire2 _XAJO_X_009p = and2(pix_count.XEHO_PX0p.qp_new(), pix_count.XYDO_PX3p.qp_new());
  /*#p21.WUSA*/ WUSA_LCD_CLOCK_GATE.nor_latch(_XAJO_X_009p, WEGO_HBLANKp);
  /*#p21.TOBA*/ wire2 _TOBA_LCD_CLOCK = and2(WUSA_LCD_CLOCK_GATE.qp_new(), SACU_CLKPIPE_evn);
  /*#p21.SEMU*/ wire2 _SEMU_LCD_CLOCK = or2(_TOBA_LCD_CLOCK, fine_scroll.POVA_FINE_MATCH_TRIGp());
  /*#p21.RYPO*/ wire2 _RYPO_LCD_CLOCK = not1(_SEMU_LCD_CLOCK);
