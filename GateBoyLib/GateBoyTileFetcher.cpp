...
void TileFetcher::tock(GateBoyClock& clk, Bus BUS_VRAM_Dp[8], wire2 XYMU_RENDERINGp, wire2 NYXU_BFETCH_RSTn, wire2 MOCE_BFETCH_DONEn_old)
{
  for (int feedback = 0; feedback < 2; feedback++) {
    ...
    /* p27.LEBO*/ wire2 _LEBO_AxCxExGx = nand2(clk.ALET_xBxDxFxH(), _MOCE_BFETCH_DONEn);
  ...
  /* p27.LYRY*/ wire2 _LYRY_BFETCH_DONEp_old = not1(MOCE_BFETCH_DONEn_old);
...
wire2 TileFetcher::LOMA_LATCH_TILE_DAn() const {
  /* p24.LOBY*/ wire2 _LOBY_RENDERINGn_new_xxx = not1(_XYMU_RENDERINGp.qp_new());
  ...
  /*#p27.LAXE*/ wire2 _LAXE_BFETCH_S0n = not1(LAXU_BFETCH_S0p);
  /*#p27.NOFU*/ wire2 _NOFU_BFETCH_S2n = not1(NYVA_BFETCH_S2p);

  /*#p27.MYSO*/ wire2 _MYSO_STORE_VRAM_DATA_TRIGp = nor3(_LOBY_RENDERINGn_new_xxx, _LAXE_BFETCH_S0n, _LYZU_BFETCH_S0p_D1.qp_new()); // MYSO fires on fetch phase 2, 6, 10

  /*#p27.NYDY*/ wire2 _NYDY_LATCH_TILE_DAn = nand3(_MYSO_STORE_VRAM_DATA_TRIGp, MESU_BFETCH_S1p, _NOFU_BFETCH_S2n); // NYDY on fetch phase 6
  /*#p32.METE*/ wire2 _METE_LATCH_TILE_DAp = not1(_NYDY_LATCH_TILE_DAn);
  /*#p32.LOMA*/ wire2 _LOMA_LATCH_TILE_DAn = not1(_METE_LATCH_TILE_DAp);
...
wire2 TileFetcher::LABU_LATCH_TILE_DBn() const {
  ...
  /*#p27.LAXE*/ wire2 _LAXE_BFETCH_S0n = not1(LAXU_BFETCH_S0p);
  /*#p27.NAKO*/ wire2 _NAKO_BFETCH_S1n = not1(MESU_BFETCH_S1p);

  /*#p27.MYSO*/ wire2 _MYSO_STORE_VRAM_DATA_TRIGp = nor3(_LOBY_RENDERINGn, _LAXE_BFETCH_S0n, _LYZU_BFETCH_S0p_D1.qp_new()); // MYSO fires on fetch phase 2, 6, 10

  /* p27.MOFU*/ wire2 _MOFU_LATCH_TILE_DBp = and2(_MYSO_STORE_VRAM_DATA_TRIGp, _NAKO_BFETCH_S1n); // MOFU fires on fetch phase 2 and 10
  /* p32.LESO*/ wire2 _LESO_LATCH_TILE_DBn = not1(_MOFU_LATCH_TILE_DBp);
  /* p??.LUVE*/ wire2 _LUVE_LATCH_TILE_DBp = not1(_LESO_LATCH_TILE_DBn); // Schematic wrong, was labeled AJAR
  /* p32.LABU*/ wire2 _LABU_LATCH_TILE_DBn = not1(_LUVE_LATCH_TILE_DBp);
