...
void GateBoyVramBus::cpu_addr_to_vram_addr(Signal BUS_CPU_A[16], wire2 XYMU_RENDERINGp, wire2 LUFA_DMA_VRAMp) {
  ...
  /*#p25.XANE*/ wire2 _XANE_VRAM_LOCKn_new_evn = nor2(LUFA_DMA_VRAMp, XYMU_RENDERINGp);
  /* p25.XEDU*/ wire2 _XEDU_VRAM_LOCKp_new_evn = not1(_XANE_VRAM_LOCKn_new_evn);
...
void GateBoyVramBus::dma_addr_to_vram_addr(const GateBoyDMA& dma) {
  ...
  /* p04.AHOC*/ wire2 _AHOC_DMA_VRAMn_new = not1(dma.LUFA_DMA_VRAMp());
...
void GateBoyVramBus::scroll_to_addr(BGScrollX scroll_x, BGScrollY scroll_y, wire2 POTU_BGW_MAP_READp, wire2 AXAD_WIN_MODEn, wire2 XAFO_LCDC_BGMAPp) {
  ...
  /* p26.BAFY*/ wire2 _BAFY_BG_MAP_READn = not1(_ACEN_BG_MAP_READp);
...
void GateBoyVramBus::win_to_addr(const WinMapX& win_map_x, const WinLineY& win_line_y, wire2 POTU_BGW_MAP_READp, wire2 PORE_WIN_MODEp, wire2 WOKY_LCDC_WINMAPp) {
  ...
  /*#p25.WUKO*/ wire2 _WUKO_WIN_MAP_READn = not1(_XEZE_WIN_MAP_READp);
...
void GateBoyVramBus::tile_to_addr(const BGScrollY scroll_y, const WinLineY win_line_y, const TileTempB tile_temp_b, wire2 NETA_BGW_TILE_READp, wire2 XUHA_FETCH_HILOp, wire2 WEXU_LCDC_BGTILEp, wire2 PORE_WIN_MODEp, wire2 AXAD_WIN_MODEn) {
  ...
  /* p26.BEJE*/ wire2 _BEJE_BG_TILE_READn = not1(_ASUL_BG_TILE_READp);
  /* p25.XUCY*/ wire2 _XUCY_WIN_TILE_READn = nand2(NETA_BGW_TILE_READp, PORE_WIN_MODEp);
...
void GateBoyVramBus::sprite_to_addr(Bus SPR_TRI_L[4], const OamTempA& oam_temp_a, const OamTempB& oam_temp_b, wire2 XUQU_SPRITE_AB, wire2 SAKY_SFETCHn, wire2 XYMU_RENDERINGp, wire2 XYMO_LCDC_SPSIZEp) {
  /*#p29.WUKY*/ wire2 _WUKY_FLIP_Yp = not1(oam_temp_b.YZOS_OAM_DB6p.qp_new());
  /*#p29.FUFO*/ wire2 _FUFO_LCDC_SPSIZEn = not1(XYMO_LCDC_SPSIZEp);
  ...
  /* p29.TEPA*/ wire2 _TEPA_RENDERINGp = not1(XYMU_RENDERINGp);
  ...
  /* p29.ABON*/ wire2 _ABON_SFETCHINGn = not1(_TEXY_SFETCHINGp);
...
void GateBoyVramBus::addr_to_pins() {
  /* p25.MYFU*/ wire2 _MYFUp = not1(BUS_VRAM_An[ 0].qp_new());
  /* p25.MASA*/ wire2 _MASAp = not1(BUS_VRAM_An[ 1].qp_new());
  /* p25.MYRE*/ wire2 _MYREp = not1(BUS_VRAM_An[ 2].qp_new());
  /* p25.MAVU*/ wire2 _MAVUp = not1(BUS_VRAM_An[ 3].qp_new());
  /* p25.MEPA*/ wire2 _MEPAp = not1(BUS_VRAM_An[ 4].qp_new());
  /* p25.MYSA*/ wire2 _MYSAp = not1(BUS_VRAM_An[ 5].qp_new());
  /* p25.MEWY*/ wire2 _MEWYp = not1(BUS_VRAM_An[ 6].qp_new());
  /* p25.MUME*/ wire2 _MUMEp = not1(BUS_VRAM_An[ 7].qp_new());
  /* p25.VOVA*/ wire2 _VOVAp = not1(BUS_VRAM_An[ 8].qp_new());
  /* p25.VODE*/ wire2 _VODEp = not1(BUS_VRAM_An[ 9].qp_new());
  /* p25.RUKY*/ wire2 _RUKYp = not1(BUS_VRAM_An[10].qp_new());
  /* p25.RUMA*/ wire2 _RUMAp = not1(BUS_VRAM_An[11].qp_new());
  /* p25.REHO*/ wire2 _REHOp = not1(BUS_VRAM_An[12].qp_new());

  /* p25.LEXE*/ wire2 _LEXEn = not1(_MYFUp);
  /* p25.LOZU*/ wire2 _LOZUn = not1(_MASAp);
  /* p25.LACA*/ wire2 _LACAn = not1(_MYREp);
  /* p25.LUVO*/ wire2 _LUVOn = not1(_MAVUp);
  /* p25.LOLY*/ wire2 _LOLYn = not1(_MEPAp);
  /* p25.LALO*/ wire2 _LALOn = not1(_MYSAp);
  /* p25.LEFA*/ wire2 _LEFAn = not1(_MEWYp);
  /* p25.LUBY*/ wire2 _LUBYn = not1(_MUMEp);
  /* p25.TUJY*/ wire2 _TUJYn = not1(_VOVAp);
  /* p25.TAGO*/ wire2 _TAGOn = not1(_VODEp);
  /* p25.NUVA*/ wire2 _NUVAn = not1(_RUKYp);
  /* p25.PEDU*/ wire2 _PEDUn = not1(_RUMAp);
  /* p25.PONY*/ wire2 _PONYn = not1(_REHOp);
...
void GateBoyVramBus::cpu_data_to_bus_data(const GateBoyCpuBus& cpu_bus, wire2 SERE_CPU_VRAM_RDp, wire2 SALE_CPU_VRAM_WRn) {
  /*#p25.RUVY*/ wire2 _RUVY_CPU_VRAM_WRp = not1(SALE_CPU_VRAM_WRn);
  /*#p25.SAZO*/ wire2 _SAZO_CBD_TO_VPDp = and2(SERE_CPU_VRAM_RDp, _RUVY_CPU_VRAM_WRp);

  /*#p25.RYJE*/ wire2 _RYJE_CBD_TO_VPDn = not1(_SAZO_CBD_TO_VPDp);
  /*#p25.REVO*/ wire2 _REVO_CBD_TO_VPDp = not1(_RYJE_CBD_TO_VPDn);
  /* p25.ROCY*/ wire2 _ROCY_CBD_TO_VPDp = and2(_REVO_CBD_TO_VPDp, _SAZO_CBD_TO_VPDp);
  /* p25.RAHU*/ wire2 _RAHU_CBD_TO_VPDn = not1(_ROCY_CBD_TO_VPDp);
...
void GateBoyVramBus::bus_data_to_pins(wire2 SERE_CPU_VRAM_RDp, wire2 SALE_CPU_VRAM_WRn) {
  ...
  /* p25.RAHU*/ wire2 _RAHU_CBD_TO_VPDn = not1(_ROCY_CBD_TO_VPDp);
  /*#p25.RENA*/ wire2 _RENA_CBD_TO_VPDn = not1(_RELA_CBD_TO_VPDp);

  /*#p25.ROVE*/ wire2 _ROVE_CBD_TO_VPDp = not1(_RAHU_CBD_TO_VPDn);
  /*#p25.ROFA*/ wire2 _ROFA_CBD_TO_VPDp = not1(_RENA_CBD_TO_VPDn);
  ...
  /*#p25.REGE*/ wire2 _REGE_D0n = not1(_SEFA_D0p);
  /* p25.RYKY*/ wire2 _RYKY_D1n = not1(_SOGO_D1p);
  /* p25.RAZO*/ wire2 _RAZO_D2n = not1(_SEFU_D2p);
  /* p25.RADA*/ wire2 _RADA_D3n = not1(_SUNA_D3p);
  /* p25.RYRO*/ wire2 _RYRO_D4n = not1(_SUMO_D4p);
  /* p25.REVU*/ wire2 _REVU_D5n = not1(_SAZU_D5p);
  /* p25.REKU*/ wire2 _REKU_D6n = not1(_SAMO_D6p);
  /* p25.RYZE*/ wire2 _RYZE_D7n = not1(_SUKE_D7p);

  /*#p25.RURA*/ wire2 _RURA_D0n = not1(_SYNU_D0p);
  /* p25.RULY*/ wire2 _RULY_D1n = not1(_SYMA_D1p);
  /* p25.RARE*/ wire2 _RARE_D2n = not1(_ROKO_D2p);
  /* p25.RODU*/ wire2 _RODU_D3n = not1(_SYBU_D3p);
  /* p25.RUBE*/ wire2 _RUBE_D4n = not1(_SAKO_D4p);
  /* p25.RUMU*/ wire2 _RUMU_D5n = not1(_SEJY_D5p);
  /* p25.RYTY*/ wire2 _RYTY_D6n = not1(_SEDO_D6p);
  /* p25.RADY*/ wire2 _RADY_D7n = not1(_SAWU_D7p);
...
void GateBoyVramBus::set_pin_cs(wire2 TUTO_VRAM_DBGp, wire2 SERE_CPU_VRAM_RDp, wire2 LUFA_DMA_VRAMp, wire2 LENA_BFETCHINGp, wire2 TEXY_SFETCHINGp) {
  /*#p25.SUTU*/ wire2 _SUTU_MCSn = nor4(LENA_BFETCHINGp, LUFA_DMA_VRAMp, TEXY_SFETCHINGp, SERE_CPU_VRAM_RDp);

  /*#p25.RACO*/ wire2 _RACO_DBG_VRAMn = not1(TUTO_VRAM_DBGp);
  /* p25.TODE*/ wire2 _TODE_MCSn_A = and2(_SUTU_MCSn, _RACO_DBG_VRAMn);
  /* p25.SEWO*/ wire2 _SEWO_MCSn_D =  or2(_SUTU_MCSn, TUTO_VRAM_DBGp);
  /* p25.SOKY*/ wire2 _SOKY_MCSp_A = not1(_TODE_MCSn_A);
  /* p25.SETY*/ wire2 _SETY_MCSp_D = not1(_SEWO_MCSn_D);
...
void GateBoyVramBus::set_pin_wr(wire2 TUTO_VRAM_DBGp, wire2 SERE_CPU_VRAM_RDp, wire2 TUJA_CPU_VRAM_WRp) {
  /* p25.SUDO*/ wire2 _SUDO_MWRp = not1(/*vram_bus.PIN_VRAM_WRn.qn_new()*/ 1); // Ignoring debug stuff for now
  /* p25.TYJY*/ wire2 _TYJY_VRAM_WRp = mux2p(TUTO_VRAM_DBGp, _SUDO_MWRp, TUJA_CPU_VRAM_WRp);
  /* p25.SOHY*/ wire2 _SOHY_MWRn   = nand2(_TYJY_VRAM_WRp, SERE_CPU_VRAM_RDp);

  /*#p25.RACO*/ wire2 _RACO_DBG_VRAMn = not1(TUTO_VRAM_DBGp);
  /* p25.TAXY*/ wire2 _TAXY_MWRn_A = and2(_SOHY_MWRn, _RACO_DBG_VRAMn);
  /* p25.SOFY*/ wire2 _SOFY_MWRn_D =  or2(_SOHY_MWRn, TUTO_VRAM_DBGp);
  /* p25.SYSY*/ wire2 _SYSY_MWRp_A = not1(_TAXY_MWRn_A);
  /* p25.RAGU*/ wire2 _RAGU_MWRp_D = not1(_SOFY_MWRn_D);
...
void GateBoyVramBus::set_pin_oe(wire2 TUTO_VRAM_DBGp, wire2 SALE_CPU_VRAM_WRn, wire2 LUFA_DMA_VRAMp, wire2 XYMU_RENDERINGp, wire2 LONY_BFETCHINGp, wire2 SOHO_SPR_VRAM_RDp) {
  /*#p25.XANE*/ wire2 _XANE_VRAM_LOCKn = nor2(LUFA_DMA_VRAMp, XYMU_RENDERINGp);
  /* p25.RYLU*/ wire2 _RYLU_CPU_VRAM_RDn = nand2(SALE_CPU_VRAM_WRn, _XANE_VRAM_LOCKn);
  /* p25.RAWA*/ wire2 _RAWA_SPR_VRAM_RDn = not1(SOHO_SPR_VRAM_RDp);
  /* p27.MYMA*/ wire2 _MYMA_BGW_VRAM_RDn = not1(LONY_BFETCHINGp);
  /* p25.APAM*/ wire2 _APAM_DMA_VRAMn    = not1(LUFA_DMA_VRAMp);
  ...
  /* p25.REFO*/ wire2 _REFO_MOEn_A = not1(_SEMA_MOEn_A);
  /* p25.SAHA*/ wire2 _SAHA_MOEn_D = not1(_RUTE_MOEn_D);
...
void GateBoyVramBus::read_vram(const uint8_t* vid_ram) {
  ...
  PIN25_VRAM_DATA[0].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 0) & 1);
  PIN25_VRAM_DATA[1].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 1) & 1);
  PIN25_VRAM_DATA[2].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 2) & 1);
  PIN25_VRAM_DATA[3].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 3) & 1);
  PIN25_VRAM_DATA[4].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 4) & 1);
  PIN25_VRAM_DATA[5].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 5) & 1);
  PIN25_VRAM_DATA[6].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 6) & 1);
  PIN25_VRAM_DATA[7].pin_in_oedp(!PIN45_VRAM_OEn.ext_qp_new(), (data >> 7) & 1);
}
...
void GateBoyVramBus::data_bus_to_cpu_bus(GateBoyCpuBus& cpu_bus, wire2 SERE_CPU_VRAM_RDp) {
  /* p25.RERY*/ wire2 _RERY_VBUS_D0n = not1(BUS_VRAM_Dp[0].qp_new());
  /* p25.RUNA*/ wire2 _RUNA_VBUS_D1n = not1(BUS_VRAM_Dp[1].qp_new());
  /* p25.RONA*/ wire2 _RONA_VBUS_D2n = not1(BUS_VRAM_Dp[2].qp_new());
  /* p25.RUNO*/ wire2 _RUNO_VBUS_D3n = not1(BUS_VRAM_Dp[3].qp_new());
  /* p25.SANA*/ wire2 _SANA_VBUS_D4n = not1(BUS_VRAM_Dp[4].qp_new());
  /* p25.RORO*/ wire2 _RORO_VBUS_D5n = not1(BUS_VRAM_Dp[5].qp_new());
  /* p25.RABO*/ wire2 _RABO_VBUS_D6n = not1(BUS_VRAM_Dp[6].qp_new());
  /* p25.SAME*/ wire2 _SAME_VBUS_D7n = not1(BUS_VRAM_Dp[7].qp_new());

  /* p25.TYVY*/ wire2 _TYVY_VBD_TO_CBDn = nand2(SERE_CPU_VRAM_RDp, cpu_bus.LEKO_CPU_RDp());
  /* p25.SEBY*/ wire2 _SEBY_VBD_TO_CBDp = not1(_TYVY_VBD_TO_CBDn);
