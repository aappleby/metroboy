...
void GateBoyZramBus::read(GateBoyCpuBus& cpu_bus, uint8_t* zero_ram)
{
  ...
  cpu_bus.BUS_CPU_D00p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 0));
  cpu_bus.BUS_CPU_D01p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 1));
  cpu_bus.BUS_CPU_D02p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 2));
  cpu_bus.BUS_CPU_D03p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 3));
  cpu_bus.BUS_CPU_D04p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 4));
  cpu_bus.BUS_CPU_D05p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 5));
  cpu_bus.BUS_CPU_D06p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 6));
  cpu_bus.BUS_CPU_D07p.tri(CSp && bit(cpu_bus.TEDO_CPU_RDp.qp_new()), bit(data, 7));
}
