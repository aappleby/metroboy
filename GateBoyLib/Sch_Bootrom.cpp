...
void Bootrom::tock(const SchematicTop& top, CpuBus& cpu_bus) {
  ...
  {
    /*p07.TYRO*/ wire _TYFO_ADDR_0x0x0000p = nor6(top.cpu_bus.BUS_CPU_A07.tp(), top.cpu_bus.BUS_CPU_A05.tp(), top.cpu_bus.BUS_CPU_A03.tp(),
                                                  top.cpu_bus.BUS_CPU_A02.tp(), top.cpu_bus.BUS_CPU_A01.tp(), top.cpu_bus.BUS_CPU_A00.tp());
    /*p07.TUFA*/ wire _TUFA_ADDR_x1x1xxxxp = and2(top.cpu_bus.BUS_CPU_A04.tp(), top.cpu_bus.BUS_CPU_A06.tp());

    /*p07.TEXE*/ wire _TEXE_FF50_RDp = and4(top.TEDO_CPU_RDp, top.cpu_bus.SYKE_FF00_FFFFp(), _TYFO_ADDR_0x0x0000p, _TUFA_ADDR_x1x1xxxxp);
    /*p07.SYPU*/ cpu_bus.BUS_CPU_D0p = tribuf_6pn(_TEXE_FF50_RDp, BOOT_BITn.qp());
    ...
    /*p07.SATO*/ wire _SATO_BOOT_BIT_IN = or2(top.cpu_bus.BUS_CPU_D0p.tp(), BOOT_BITn.qp());
...

  {
    ...
    cpu_bus.BUS_CPU_D0p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x01));
    cpu_bus.BUS_CPU_D1p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x02));
    cpu_bus.BUS_CPU_D2p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x04));
    cpu_bus.BUS_CPU_D3p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x08));
    cpu_bus.BUS_CPU_D4p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x10));
    cpu_bus.BUS_CPU_D5p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x20));
    cpu_bus.BUS_CPU_D6p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x40));
    cpu_bus.BUS_CPU_D7p = tribuf_6pn(_ZERY_BOOT_CSp, !bool(data & 0x80));
#endif
  }
}
