...
int GateBoyTests::test_clk() {
  ...
  for (int i = 0; i < 32; i++) {
    ...
    EXPECT_CLK(vclk_reg.WUVU_ABxxEFxxp.qp_old(), 0b11001100);
    EXPECT_CLK(vclk_reg.VENA_xxCDEFxxp.qp_old(), 0b00111100);
    EXPECT_CLK(vclk_reg.WOSU_AxxDExxHp.qp_old(), 0b10011001);
