...
void Gameboy::tick() {
  ...
  if (tphase == 0) {
    ...
    if (imask & 0x02) {
      ...
      if (ppu.stat & EI_OAM)    z80.unhalt |= old_vblank;
      if (ppu.stat & EI_VBLANK) z80.unhalt |= vblank;
...
void Gameboy::tock() {
  ...
  if (!lcd_on || vblankP2) {
    ...
    ppu.stat = ubit8_t(0x80 | (ppu.stat & 0b01111000) | (lyc_match << 2) | 1);
  }
  else {
