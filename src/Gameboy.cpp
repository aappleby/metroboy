...
void Gameboy::tick() {
  ...
  if (tphase == 0 || tphase == 2) {
    ...
    if (tphase == 0) {
      ...
      if (!ppu.vblank_phase && ppu.counterP2 == 0) {
        ppu.oam_phase = !weird_line && !(ppu.lineP2 > 143);
        ppu.hblank_phase = false;
        ppu.sprite_index = -1;
        ppu.sprite_count = 0;

        //ppu.state = (ppu.lineP2 == 0) ? PPU_STATE_HBLANK : PPU_STATE_OAM;
        ppu.state = PPU_STATE_HBLANK;
      }

      if (!ppu.vblank_phase && ppu.counterP2 == 4) {
        if (!weird_line) ppu.state = PPU_STATE_OAM;
        ppu.pix_count = 0;
        ppu.hblank_delay = HBLANK_DELAY_START;
      }

      if (!ppu.vblank_phase && ppu.counterP2 == 84) {
        ppu.hblank_phase = false;
        ppu.oam_phase = false;
  ...
  if (tphase == 0) {
    ...
    if ((stat_int2 & EI_HBLANK) && (ppu.stat & EI_HBLANK)) new_stat_int = 1;
    if ((stat_int2 & EI_VBLANK) && (ppu.stat & EI_VBLANK)) new_stat_int = 1;
    if ((stat_int2 & EI_LYC) && (ppu.stat & EI_LYC)) new_stat_int = 1;
    if ((stat_int2 & EI_OAM) && (ppu.stat & EI_OAM)) new_stat_int = 1;
...
void Gameboy::tock() {
  ...
  int new_stat_int = 0;
  if ((stat_int & EI_HBLANK) && (ppu.stat & EI_HBLANK)) new_stat_int = 1;
  if ((stat_int & EI_VBLANK) && (ppu.stat & EI_VBLANK)) new_stat_int = 1;
  if ((stat_int & EI_LYC) && (ppu.stat & EI_LYC)) new_stat_int = 1;
  if ((stat_int & EI_OAM) && (ppu.stat & EI_OAM)) new_stat_int = 1;
  if (stat_int & 0x80) new_stat_int = 1;
  if (new_stat_int && !old_stat_int) intf |= INT_STAT;
  ...
  uint8_t dma_data = 0;
  if (dma_mode_b == DMA_CART) dma_data = mmu.bus_out;
  if (dma_mode_b == DMA_VRAM) dma_data = vram.bus_out;
  if (dma_mode_b == DMA_IRAM) dma_data = iram.bus_out;
