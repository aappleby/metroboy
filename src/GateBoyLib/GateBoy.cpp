...
void GateBoy::tock_logic(const blob& cart_blob) {
  ...
  bool int_joy_old = !joy_int.APUG_JP_GLITCH3.state || !joy_int.BATU_JP_GLITCH0.state;
  bool int_tim_old = int_ctrl.MOBA_TIMER_OVERFLOWp.state;
  //wire int_ser_old = serial.CALY_SER_CNT3.state;
  bool int_ser_old = 0;


  bool UKAP_CLK_MUXa_old = tac.SOPU_TAC0p.state ? div.TAMA_DIV05p.state : div.TERO_DIV03p.state;
  bool TEKO_CLK_MUXb_old = tac.SOPU_TAC0p.state ? div.UFOR_DIV01p.state : div.TULU_DIV07p.state;
  bool TECY_CLK_MUXc_old = tac.SAMY_TAC1p.state ? UKAP_CLK_MUXa_old : TEKO_CLK_MUXb_old;
  bool SOGU_TIMA_CLKn_old = TECY_CLK_MUXc_old && tac.SABO_TAC2p.state;
  ...
  uint16_t cpu_addr_old = (uint16_t)bit_pack(cpu_abus_old);
  ...
  uint8_t cpu_data_old = (uint8_t)bit_pack(cpu_dbus_old);
  ...
  uint8_t cpu_data_new = (uint8_t)bit_pack(cpu_dbus_new);
  ...
  clk.SIG_CPU_BOWA_Axxxxxxx.state = !!(phase_new & 0b10000000);
  clk.SIG_CPU_BEDO_xBCDEFGH.state = !!(phase_new & 0b01111111);
  clk.SIG_CPU_BEKO_ABCDxxxx.state = !!(phase_new & 0b11110000);
  clk.SIG_CPU_BUDE_xxxxEFGH.state = !!(phase_new & 0b00001111);
  clk.SIG_CPU_BOLO_ABCDEFxx.state = !!(phase_new & 0b11111100);
  clk.SIG_CPU_BUKE_AxxxxxGH.state = !!(phase_new & 0b10000011);
  clk.SIG_CPU_BOMA_xBCDEFGH.state = !!(phase_new & 0b01111111);
  clk.SIG_CPU_BOGA_Axxxxxxx.state = !!(phase_new & 0b10000000);
  ...
  auto pack_oam_temp_a = bit_pack(oam_temp_a);
  ...
  if (!FEPO_STORE_MATCHp.state) {
    bit_unpack(&sprite_lbus, 4, pack_ydiff);
  }
...

  {
    ...
    if (cpu_signals.SIG_IN_CPU_WRp.state && gen_clk_new(0b00000001)) {
  ...
  if (!(dma_running_new && !dma_addr_vram_new) && cpu_signals.SIG_IN_CPU_EXT_BUSp.state && cpu_signals.SIG_IN_CPU_WRp.state) {
  ...
  else if (!cpu_signals.TEPU_BOOT_BITn.state && cpu_addr_new <= 0x00FF) {
...

  {
    ...
    if (dma_running_new) {
      ...
      oam_ctrl.SIG_OAM_OEn  .state = !(dbus_busy && addr_oam && cpu_rd);
    }
      ...
      oam_ctrl.SIG_OAM_CLKn .state = (!vid_rst_new && gen_clk_new(0b10011001)) && (!vid_rst_new && gen_clk_new(0b11001100)) && (!addr_oam || !CLK_xxxxEFGH);
      oam_ctrl.SIG_OAM_WRn_A.state = 1;
      oam_ctrl.SIG_OAM_WRn_B.state = 1;
      oam_ctrl.SIG_OAM_OEn  .state = (!vid_rst_new && gen_clk_new(0b10011001)) && !(dbus_busy && addr_oam && cpu_rd);
    }
...

  {
    ...
    if (cpu_signals.SIG_IN_CPU_WRp.state & (cpu_addr_new == 0xFF0F) & gen_clk_new(0b00001110)) {
