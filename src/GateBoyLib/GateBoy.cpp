...
void GateBoy::tock_logic(const blob& cart_blob) {
  ...
  if (!vid_rst_new) {
    ...
    lcd.LUCA_LINE_EVENp. dff_pp(LOFU_x113n,           ~lcd.LUCA_LINE_EVENp.state);
    lcd.NAPO_FRAME_EVENp.dff_pp(lcd.POPU_y144p.state, ~lcd.NAPO_FRAME_EVENp.state);
  ...
  //tock_interrupts_gates();
  tock_interrupts_logic(cpu_addr_old, cpu_addr_new, cpu_rd_old, cpu_rd_new, cpu_wr_old, cpu_wr_new);
