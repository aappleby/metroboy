...
void GateBoy::tock_clocks_gates() {
  /*_PIN_73*/ reg.sys_clk.PIN_73_CLK_DRIVE.pin_out(reg.sys_clk.PIN_74_CLK.clk(), reg.sys_clk.PIN_74_CLK.clk());

  /*_p01.ARYS*/ wire ARYS_CLKIN = not1(reg.sys_clk.PIN_74_CLK.clk());
  /*_p01.AVET*/ reg.sys_clk.AVET_DEGLITCH = nand2(reg.sys_clk.ANOS_DEGLITCH.out_mid(), ARYS_CLKIN);
  /*_p01.ANOS*/ reg.sys_clk.ANOS_DEGLITCH = nand2(reg.sys_clk.PIN_74_CLK.clk(), reg.sys_clk.AVET_DEGLITCH.out_mid());
  /*_p01.AVET*/ reg.sys_clk.AVET_DEGLITCH = nand2(reg.sys_clk.ANOS_DEGLITCH.out_mid(), ARYS_CLKIN);
  /*_p01.ANOS*/ reg.sys_clk.ANOS_DEGLITCH = nand2(reg.sys_clk.PIN_74_CLK.clk(), reg.sys_clk.AVET_DEGLITCH.out_mid());

  /*_p01.ATAL*/ wire ATAL_xBxDxFxH = not1(reg.sys_clk.AVET_DEGLITCH.out_new());
  /*_p01.ATAN*/ wire ATAN_AxCxExGx = not1(ATAL_xBxDxFxH); // cell not marked on die but it's next to ATAL

  DFF9 ADYK_ABCxxxxH_old = reg.sys_clk.ADYK_ABCxxxxH;
  DFF9 AFUR_xxxxEFGH_old = reg.sys_clk.AFUR_xxxxEFGH;
