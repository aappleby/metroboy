...
void GateBoy::tock_bootrom_gates() {
  /*_p07.TUGE*/ wire TUGE_FF50_WRn = nand4(reg.cpu_signals.TAPU_CPU_WRp.out_new(), reg.cpu_abus_new.SYKE_ADDR_HIp(), reg.cpu_abus_new.TYRO_XX_0x0x0000p(), reg.cpu_abus_new.TUFA_XX_x1x1xxxxp());
  ...
  auto cpu_addr_new = bit_pack(reg.cpu_abus_new);
  ...
  /*_p07.ZORO*/ wire ZORO_0000xxxx_XX = nor4(reg.cpu_abus_new.BUS_CPU_A15p.out_new(), reg.cpu_abus_new.BUS_CPU_A14p.out_new(), reg.cpu_abus_new.BUS_CPU_A13p.out_new(), reg.cpu_abus_new.BUS_CPU_A12p.out_new());
  /*_p07.ZADU*/ wire ZADU_xxxx0000_XX = nor4(reg.cpu_abus_new.BUS_CPU_A11p.out_new(), reg.cpu_abus_new.BUS_CPU_A10p.out_new(), reg.cpu_abus_new.BUS_CPU_A09p.out_new(), reg.cpu_abus_new.BUS_CPU_A08p.out_new());
  ...
  reg.cpu_dbus_new.BUS_CPU_D00p.tri_bus(boot_d0);
  reg.cpu_dbus_new.BUS_CPU_D01p.tri_bus(boot_d1);
  reg.cpu_dbus_new.BUS_CPU_D02p.tri_bus(boot_d2);
  reg.cpu_dbus_new.BUS_CPU_D03p.tri_bus(boot_d3);
  reg.cpu_dbus_new.BUS_CPU_D04p.tri_bus(boot_d4);
  reg.cpu_dbus_new.BUS_CPU_D05p.tri_bus(boot_d5);
  reg.cpu_dbus_new.BUS_CPU_D06p.tri_bus(boot_d6);
  reg.cpu_dbus_new.BUS_CPU_D07p.tri_bus(boot_d7);

  /*_p07.TEXE*/ wire TEXE_FF50_RDp =  and4(reg.cpu_signals.TEDO_CPU_RDp.out_new(), reg.cpu_abus_new.SYKE_ADDR_HIp(), reg.cpu_abus_new.TYRO_XX_0x0x0000p(), reg.cpu_abus_new.TUFA_XX_x1x1xxxxp());
  /*_p07.SYPU*/ triwire SYPU_BOOT_TO_CD0 = tri6_pn(TEXE_FF50_RDp, reg.cpu_signals.TEPU_BOOT_BITn.qp_new());
  /*_BUS_CPU_D00p*/ reg.cpu_dbus_new.BUS_CPU_D00p.tri_bus(SYPU_BOOT_TO_CD0);

  /*_p07.SATO*/ reg.SATO_BOOT_BITn = or2(reg.cpu_dbus_new.BUS_CPU_D00p.out_new(), reg.cpu_signals.TEPU_BOOT_BITn.qp_new());
}
