...
void GateBoy::get_flat_blob(const blob& cart_blob, int addr, int size, uint8_t* out) const {
  if (addr + size >= 0xFFFF) {
    size = 0xFFFF - addr;
  }
...
void GateBoy::tock_ext_gates(const GateBoyState& reg_old, const blob& cart_blob)
{
  ...
  /*_p08.LAGU*/ wire LAGU_new = and_or3(reg_new.cpu_signals.SIG_IN_CPU_RDp.out_new(), LEVO_ADDR_VRAMn_new, reg_new.cpu_signals.SIG_IN_CPU_WRp.out_new());
...

  {
    ...
    /*_p08.TYHO*/ wire TYHO_CS_A_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.MARU_DMA_A15p.qp_new(), TOZA_CS_A_new);
  ...
  /*#p08.MANO*/ wire MANO_A08p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.NAFA_DMA_A08p.qp_new(),   reg_new.ext_addr_latch.LUNO_EXT_ADDR_LATCH_08p.qp_new());
  /*_p08.MASU*/ wire MASU_A09p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.PYNE_DMA_A09p.qp_new(),   reg_new.ext_addr_latch.LYSA_EXT_ADDR_LATCH_09p.qp_new());
  /*_p08.PAMY*/ wire PAMY_A10p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.PARA_DMA_A10p.qp_new(),   reg_new.ext_addr_latch.PATE_EXT_ADDR_LATCH_10p.qp_new());
  /*_p08.MALE*/ wire MALE_A11p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.NYDO_DMA_A11p.qp_new(),   reg_new.ext_addr_latch.LUMY_EXT_ADDR_LATCH_11p.qp_new());
  /*_p08.MOJY*/ wire MOJY_A12p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.NYGY_DMA_A12p.qp_new(),   reg_new.ext_addr_latch.LOBU_EXT_ADDR_LATCH_12p.qp_new());
  /*_p08.MUCE*/ wire MUCE_A13p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.PULA_DMA_A13p.qp_new(),   reg_new.ext_addr_latch.LONU_EXT_ADDR_LATCH_13p.qp_new());
  /*_p08.PEGE*/ wire PEGE_A14p_new = mux2p(LUMA_DMA_CARTp_new, reg_new.reg_dma.POKU_DMA_A14p.qp_new(),   reg_new.ext_addr_latch.NYRE_EXT_ADDR_LATCH_14p.qp_new());
  ...
  /*_p08.TAZY*/ wire TAZY_A15p_new = mux2p (LUMA_DMA_CARTp_new, reg_new.reg_dma.MARU_DMA_A15p.qp_new(), SEPY_A15p_new);
