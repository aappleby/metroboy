...
{
  uint16_t cpu_addr = (uint16_t)pack(16, (BitBase*)&new_bus.BUS_CPU_A00p);
  ...
  /*_p21.ROXE*/ reg_stat.ROXE_STAT_HBI_ENn.dff9(RYVE_FF41_WRn, WESY_SYS_RSTn(), old_bus.BUS_CPU_D03p.out_old());
  /*_p21.RUFO*/ reg_stat.RUFO_STAT_VBI_ENn.dff9(RYVE_FF41_WRn, WESY_SYS_RSTn(), old_bus.BUS_CPU_D04p.out_old());
  /*_p21.REFE*/ reg_stat.REFE_STAT_OAI_ENn.dff9(RYVE_FF41_WRn, WESY_SYS_RSTn(), old_bus.BUS_CPU_D05p.out_old());
  /*_p21.RUGU*/ reg_stat.RUGU_STAT_LYI_ENn.dff9(RYVE_FF41_WRn, WESY_SYS_RSTn(), old_bus.BUS_CPU_D06p.out_old());

  /*_p21.TOBE*/ wire TOBE_FF41_RDp = and2(ASOT_CPU_RDp(), new_bus.VARY_FF41p());
  ...
  /*_BUS_CPU_D00p*/ new_bus.BUS_CPU_D00p.tri_bus(TEBY_STAT0_TO_CD0);
  /*_BUS_CPU_D01p*/ new_bus.BUS_CPU_D01p.tri_bus(WUGA_STAT1_TO_CD1);
  /*_BUS_CPU_D02p*/ new_bus.BUS_CPU_D02p.tri_bus(SEGO_STAT2_TO_CD2);
  /*_BUS_CPU_D03p*/ new_bus.BUS_CPU_D03p.tri_bus(PUZO_STAT3_TO_CD3);
  /*_BUS_CPU_D04p*/ new_bus.BUS_CPU_D04p.tri_bus(POFO_STAT4_TO_CD4);
  /*_BUS_CPU_D05p*/ new_bus.BUS_CPU_D05p.tri_bus(SASY_STAT5_TO_CD5);
  /*_BUS_CPU_D06p*/ new_bus.BUS_CPU_D06p.tri_bus(POTE_STAT6_TO_CD6);
  ...
  /*_p07.REFA*/ wire REFA_FF0F_WRn = nand4(cpu_signals.TAPU_CPU_WRp.out_new(), new_bus.SYKE_ADDR_HIp(), new_bus.SEMY_XX_0000xxxxp(), new_bus.SAPA_XX_xxxx1111p()); // schematic wrong, is NAND
  ...
  /*_p02.MYZU*/ wire MYZU_FF0F_SET0n = nand3(ROTU_FF0F_WRp, LETY_INT_VBL_ACKn,  new_bus.BUS_CPU_D00p.out_new());
  /*_p02.MODY*/ wire MODY_FF0F_SET1n = nand3(ROTU_FF0F_WRp, LEJA_INT_STAT_ACKn, new_bus.BUS_CPU_D01p.out_new());
  /*_p02.PYHU*/ wire PYHU_FF0F_SET2n = nand3(ROTU_FF0F_WRp, LESA_INT_TIM_ACKn,  new_bus.BUS_CPU_D02p.out_new());
  /*_p02.TOME*/ wire TOME_FF0F_SET3n = nand3(ROTU_FF0F_WRp, LUFE_INT_SER_ACKn,  new_bus.BUS_CPU_D03p.out_new());
  /*_p02.TOGA*/ wire TOGA_FF0F_SET4n = nand3(ROTU_FF0F_WRp, LAMO_INT_JOY_ACKn,  new_bus.BUS_CPU_D04p.out_new());

  /*#p02.MUXE*/ wire MUXE_INT0_WRn = or2(new_bus.BUS_CPU_D00p.out_new(), REFA_FF0F_WRn);
  /*_p02.NABE*/ wire NABE_INT1_WRn = or2(new_bus.BUS_CPU_D01p.out_new(), REFA_FF0F_WRn);
  /*_p02.RAKE*/ wire RAKE_INT2_WRn = or2(new_bus.BUS_CPU_D02p.out_new(), REFA_FF0F_WRn);
  /*_p02.SULO*/ wire SULO_INT3_WRn = or2(new_bus.BUS_CPU_D03p.out_new(), REFA_FF0F_WRn);
  /*_p02.SEME*/ wire SEME_INT4_WRn = or2(new_bus.BUS_CPU_D04p.out_new(), REFA_FF0F_WRn);
  ...
  /*_p02.NYBO*/ interrupts.NYBO_FF0F_D2p.dff22(timer.MOBA_TIMER_OVERFLOWp.qp_new(), PYHU_FF0F_SET2n, PYGA_FF0F_RST2n, SIG_VCC.out_new());
  ...
  /*_p07.ROLO*/ wire ROLO_FF0F_RDn = nand4(cpu_signals.TEDO_CPU_RDp.out_new(), new_bus.SYKE_ADDR_HIp(), new_bus.SEMY_XX_0000xxxxp(), new_bus.SAPA_XX_xxxx1111p()); // schematic wrong, is NAND
  ...
  /*_BUS_CPU_D00p*/ new_bus.BUS_CPU_D00p.tri_bus(NELA_IF0_TO_CD0);
  /*_BUS_CPU_D01p*/ new_bus.BUS_CPU_D01p.tri_bus(NABO_IF1_TO_CD1);
...
void GateBoy::tock_interrupts_logic()
{
  ...
  if (new_addr == 0xFFFF && bit(cpu_signals.SIG_IN_CPU_WRp.state) && DELTA_GH) {
    interrupts.IE_D0.state = old_bus.BUS_CPU_D00p.state;
    interrupts.IE_D1.state = old_bus.BUS_CPU_D01p.state;
    interrupts.IE_D2.state = old_bus.BUS_CPU_D02p.state;
    interrupts.IE_D3.state = old_bus.BUS_CPU_D03p.state;
    interrupts.IE_D4.state = old_bus.BUS_CPU_D04p.state;
  }

  if (new_addr == 0xFF41 && bit(cpu_signals.SIG_IN_CPU_WRp.state) && DELTA_GH) {
    reg_stat.ROXE_STAT_HBI_ENn.state = ~old_bus.BUS_CPU_D03p.state;
    reg_stat.RUFO_STAT_VBI_ENn.state = ~old_bus.BUS_CPU_D04p.state;
    reg_stat.REFE_STAT_OAI_ENn.state = ~old_bus.BUS_CPU_D05p.state;
    reg_stat.RUGU_STAT_LYI_ENn.state = ~old_bus.BUS_CPU_D06p.state;
  }
  ...
  if (cpu_signals.SIG_IN_CPU_WRp.state & (new_addr == 0xFF0F) & CLK_xxxxEFGx) {
    ...
    interrupts.UBUL_FF0F_D3p.set_data(new_bus.BUS_CPU_D03p.out_new());
    interrupts.ULAK_FF0F_D4p.set_data(new_bus.BUS_CPU_D04p.out_new());
  }

  if (bit(interrupts.SIG_CPU_ACK_VBLANK.out_new())) interrupts.LOPE_FF0F_D0p.state &= ~1;
  if (bit(interrupts.SIG_CPU_ACK_STAT.out_new()))   interrupts.LALU_FF0F_D1p.state &= ~1;
  if (bit(interrupts.SIG_CPU_ACK_TIMER.out_new()))  interrupts.NYBO_FF0F_D2p.state &= ~1;
  if (bit(interrupts.SIG_CPU_ACK_SERIAL.out_new())) interrupts.UBUL_FF0F_D3p.state &= ~1;
  if (bit(interrupts.SIG_CPU_ACK_JOYPAD.out_new())) interrupts.ULAK_FF0F_D4p.state &= ~1;
