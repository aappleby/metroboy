...
void GateBoy::tock_zram_gates()
{
  ...
  reg.cpu_dbus_new.BUS_CPU_D00p.tri_bus(tri0);
  reg.cpu_dbus_new.BUS_CPU_D01p.tri_bus(tri1);
  reg.cpu_dbus_new.BUS_CPU_D02p.tri_bus(tri2);
  reg.cpu_dbus_new.BUS_CPU_D03p.tri_bus(tri3);
  reg.cpu_dbus_new.BUS_CPU_D04p.tri_bus(tri4);
  reg.cpu_dbus_new.BUS_CPU_D05p.tri_bus(tri5);
  reg.cpu_dbus_new.BUS_CPU_D06p.tri_bus(tri6);
  reg.cpu_dbus_new.BUS_CPU_D07p.tri_bus(tri7);
}
