...
void LogicBoy::tock_logic(const blob& cart_blob) {
  ...
  */
  ...
  if (vid_rst_evn_new) {
    ...
    state_new.win_ctrl.NOPA_WIN_MODE_Bp_evn.state = state_old.win_ctrl.PYNU_WIN_MODE_LATCHp.state;   
  }
  else if (DELTA_ODD) {
  }
  ...
  //--------------------------------------------------------------------------------
    ...
    if (state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state && !state_new.win_ctrl.NOPA_WIN_MODE_Bp_evn.state) state_new.win_ctrl.RYDY_WIN_HITp.state = !line_rst_odd_new;
    ...
    if (!state_old.win_ctrl.RYDY_WIN_HITp.state && state_new.tfetch_control.POKY_PRELOAD_LATCHp_evn.state && !state_old.FEPO_STORE_MATCHp && (state_old.pix_count != 167)) {
    ...
    if (!state_new.tfetch_control.POKY_PRELOAD_LATCHp_evn.state && fetch_done_new) {
  ...
  //--------------------------------------------------------------------------------
    ...
    if (state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state && !state_new.win_ctrl.NOPA_WIN_MODE_Bp_evn.state) {
      state_new.tfetch_control.NYKA_FETCH_DONEp_evn.state = 0;
      state_new.tfetch_control.PORY_FETCH_DONEp_odd.state = 0;
    }

    if (state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state && !state_new.win_ctrl.NOPA_WIN_MODE_Bp_evn.state) state_new.win_ctrl.RYDY_WIN_HITp.state = 1;
    if (state_new.tfetch_control.PORY_FETCH_DONEp_odd.state) state_new.win_ctrl.RYDY_WIN_HITp.state = 0;
    ...
    if (!state_old.win_ctrl.RYDY_WIN_HITp.state && state_old.tfetch_control.POKY_PRELOAD_LATCHp_evn.state && !state_old.FEPO_STORE_MATCHp && !WODU_HBLANK_old) {
      ...
      if (!state_old.fine_scroll.ROXY_FINE_SCROLL_DONEn.state) state_new.pix_count++;
    }

    wire SEKO_WIN_FETCH_TRIG_old     = and2(!state_old.win_ctrl.RENE_WIN_FETCHn_B.state,         state_old.win_ctrl.RYFA_WIN_FETCHn_A.state);
    wire SUZU_WIN_HIT_TRIG_old       = and2(!state_old.win_ctrl.RYDY_WIN_HITp.state,             state_old.win_ctrl.SOVY_WIN_HITp.state);
    wire TAVE_PRELOAD_DONE_TRIGp_old = and2(state_old.tfetch_control.NYKA_FETCH_DONEp_evn.state, state_old.tfetch_control.PORY_FETCH_DONEp_odd.state) && !state_old.tfetch_control.POKY_PRELOAD_LATCHp_evn.state;
    ...
    wire TAVE_PRELOAD_DONE_TRIGp_new = and2(state_new.tfetch_control.NYKA_FETCH_DONEp_evn.state, state_new.tfetch_control.PORY_FETCH_DONEp_odd.state) && !state_new.tfetch_control.POKY_PRELOAD_LATCHp_evn.state;

    wire TEVO_WIN_FETCH_TRIGp_old = or3(SEKO_WIN_FETCH_TRIG_old, SUZU_WIN_HIT_TRIG_old, TAVE_PRELOAD_DONE_TRIGp_old);
    wire VETU_WIN_MAPp_old = and2(TEVO_WIN_FETCH_TRIGp_old, state_old.win_ctrl.PYNU_WIN_MODE_LATCHp.state);

    wire TEVO_WIN_FETCH_TRIGp_new = or3(SEKO_WIN_FETCH_TRIG_new, SUZU_WIN_HIT_TRIG_new, TAVE_PRELOAD_DONE_TRIGp_new);
    wire VETU_WIN_MAPp_new = and2(TEVO_WIN_FETCH_TRIGp_new, state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state);
  ...
  wire clkpipe_gate = !state_old.win_ctrl.RYDY_WIN_HITp.state && state_new.tfetch_control.POKY_PRELOAD_LATCHp_evn.state && !state_old.FEPO_STORE_MATCHp && (state_old.pix_count != 167);
  ...
  wire NYXU_BFETCH_RSTn_new = (line_rst_odd_new || vid_rst_evn_new || !scan_done_trig_new) && !(state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state && !state_new.win_ctrl.NOPA_WIN_MODE_Bp_evn.state) && !TEVO_WIN_FETCH_TRIGp_new;
  ...
  if (XYMU_RENDERINGn_new || ceno_scan_donen_odd_new) {
    ...
    state_new.sprite_match_flags = 0;
    state_new.sprite_ibus = 0x3F;
    state_new.sprite_lbus = 0x0F;

    if (spr_en_new) {
      if      (state_new.pix_count == state_new.store_x[0]) { state_new.sprite_match_flags = 0x001;  state_new.sprite_ibus = state_new.store_i[0] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[0] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[1]) { state_new.sprite_match_flags = 0x002;  state_new.sprite_ibus = state_new.store_i[1] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[1] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[2]) { state_new.sprite_match_flags = 0x004;  state_new.sprite_ibus = state_new.store_i[2] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[2] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[3]) { state_new.sprite_match_flags = 0x008;  state_new.sprite_ibus = state_new.store_i[3] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[3] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[4]) { state_new.sprite_match_flags = 0x010;  state_new.sprite_ibus = state_new.store_i[4] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[4] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[5]) { state_new.sprite_match_flags = 0x020;  state_new.sprite_ibus = state_new.store_i[5] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[5] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[6]) { state_new.sprite_match_flags = 0x040;  state_new.sprite_ibus = state_new.store_i[6] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[6] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[7]) { state_new.sprite_match_flags = 0x080;  state_new.sprite_ibus = state_new.store_i[7] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[7] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[8]) { state_new.sprite_match_flags = 0x100;  state_new.sprite_ibus = state_new.store_i[8] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[8] ^ 0x0F; }
      else if (state_new.pix_count == state_new.store_x[9]) { state_new.sprite_match_flags = 0x200;  state_new.sprite_ibus = state_new.store_i[9] ^ 0x3F;  state_new.sprite_lbus = state_new.store_l[9] ^ 0x0F; }
    }
  ...
  if (((state_new.phase_sfetch >= 4) && (state_new.phase_sfetch < 12) && !vid_rst_evn_old && !state_old.XYMU_RENDERINGn)) {
    ...
    if (state_new.phase_tfetch >= 4 && state_new.phase_tfetch <= 11) {
      ...
      const auto tile_y = (state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state ? state_new.win_y.tile : (sum_y & 0b111));
    ...
    else if (state_new.win_ctrl.PYNU_WIN_MODE_LATCHp.state) {
  ...
  if (!config_fastmode) {
    state_new.sfetch_counter_evn = uint8_t(state_new.phase_sfetch / 2);
