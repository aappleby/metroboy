...
int main(int argc, char** argv) {
  ...
  if (0) {
    ...
    //results << t.test_fuzz_reg(proto.get(), ADDR_IF  , reps); // FIXME DONOTCHECKIN
...
TestResults GateBoyTests::test_regression_cart(const char* filename, int cycles) {
  ...
  for (int i = 0; i < cycles; i++) {
    ...
    ASSERT_EQ(false, gb->next_phase(cart_blob).is_err());
  }
...
TestResults GateBoyTests::test_fuzz_reg(const IGateBoy* proto, uint16_t addr, int reps) {
  ...
  for (int j = 0; j < reps; j++) {
    ...
    for (int i = 0; i < 1000; i++) {
      if ((r % 100) >= 90) {
        auto res = gb->dbg_write(dummy_cart, addr, uint8_t(xorshift32(r)));
        if (res.is_err()) {
          TEST_FAIL("test_fuzz_reg failed at %04d:%04d - write 0x%02x to 0x%04x\n", j, i, uint8_t(r), addr);
          break;
        }
      }
      else {
        ...
        if (res.is_err()) {
          TEST_FAIL("test_fuzz_reg failed at %04d:%04d - read 0x%04x\n", j, i, addr);
          break;
        }
      }
    }
...
void GateBoyTests::save_fuzz_dump(const IGateBoy* proto, int rep_j, int rep_i) {
  uint32_t r = rep_j;
  for (int k = 0; k < 10; k++) xorshift32(r);
  ...
  for (int i = 0; i <= rep_i; i++) {
    ...
    if (i == rep_i) {
      LOG_R("last write was to 0x%04x 0x%02x\n", addr, data);
      BlobStream bs;
      gb->save_raw_dump(bs);
      bs.write(dummy_cart.data(), dummy_cart.size());
      char filename[256];
      sprintf(filename, "mismatch_%d_%d.dump", rep_j, rep_i);
    ...
    for (int phase = 0; phase < 8; phase++) {
      ...
      if (res1.is_err()) {
        LOG_R("save_fuzz_dump failed at %04d:%04d - write 0x%02x to 0x%04x\n", rep_j, i, data, addr);
      }
    }
  }

  LOG_R("save_fuzz_dump failed - should not get here\n");
}

TestResults GateBoyTests::test_fuzz_reg2(const IGateBoy* proto, int reps) {
  TEST_INIT();
    ...
    for (int i = 0; i < 100000; i++) {
      ...
      for (int phase = 0; phase < 8; phase++) {
        ...
        if (res1.is_err()) {
          ...
          LOG_R("Dump saved @ %04d:%04d\n", j, i);
          if (write) {
            TEST_FAIL("test_fuzz_spu failed at %04d:%04d - write 0x%02x to 0x%04x\n", j, i, data, addr);
          }
          else {
            TEST_FAIL("test_fuzz_spu failed at %04d:%04d - read 0x%04x\n", j, i, addr);
          }
        }
      }
    }
...
TestResults GateBoyTests::diff_gb(IGateBoy* gb1, IGateBoy* gb2, uint8_t mask) {
  ...
  EXPECT_EQ(true, pass, "GateBoyTests::diff_gb fail");
...
TestResults GateBoyTests::run_microtest(const IGateBoy* proto, const char* filename, bool verbose) {
  ...
  blob cart_blob;
  load_blob((std::string("symlinks/GBMicrotests/bin/") + filename).c_str(), cart_blob);

  if (cart_blob.empty()) {
    TEST_FAIL("%-30s FILE NOT FOUND\n", filename);
  }
...
TestResults GateBoyTests::run_mooneye_test(const IGateBoy* proto, const char* path, const char* filename) {
  ...
  ASSERT_EQ(false, cart_blob.empty(), "FILE NOT FOUND - %-30s\n", filename);
