...
void LCD::tock(int phase, const Req& /*req*/, bool XONA_LCDC_EN) {
  ...
  bool VID_LINE_d0 = _RUTU_LINE_p000p && !IN_VBLANK;
  bool LINE_000n = Y != 0;
  ...
  if (PHASE_B) {
    X++;
    if (_RUTU_LINE_p000p && !NYPE_LINE_STARTp) {
      MYTA_LINE_153p = LINE_153;
      MEDA_VSYNC_OUTn = LINE_000n;
      POPU_IN_VBLANKp = IN_VBLANK;
    }
    NYPE_LINE_STARTp = _RUTU_LINE_p000p;
  }

  if (PHASE_F) {
    if (LINE_END && !_RUTU_LINE_p000p) Y++;
    _RUTU_LINE_p000p = LINE_END;
  }
  ...
  if (PHASE_D || PHASE_H) {
    _ANEL_LINE_p004p = CATU_VID_LINE_ENDp;
  }

  if (_RUTU_LINE_p000p) X = 0;
  ...
  VID_LINE_TRIG_d4 = (CATU_VID_LINE_ENDp && !_ANEL_LINE_p004p) || !XONA_LCDC_EN;
}
...
void LCD::dump(Dumper& dump) const {
  ...
  dump("X            = %d\n", X);
  dump("Y            = %d\n", Y);
  dump("_RUTU_LINE_p000p  = %d\n", _RUTU_LINE_p000p);
  dump("_NYPE_LINE_p004p  = %d\n", NYPE_LINE_STARTp);
  dump("CATU_VID_LINE_ENDp  = %d\n", CATU_VID_LINE_ENDp);
  dump("_ANEL_LINE_p004p  = %d\n", _ANEL_LINE_p004p);
