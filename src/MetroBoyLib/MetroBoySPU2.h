...
public:
  ...
  void tock_out() {
    ...
    if (s1_running && s1_env_vol) {
      ...
      if (mix_l1 && s1_out) out_l = out_l + s1_env_vol;
      if (mix_r1 && s1_out) out_r = out_r + s1_env_vol;
    }
    ...
    if (s2_running && s2_env_vol) {
      ...
      if (mix_l2 && s2_out) out_l = out_l + s2_env_vol;
      if (mix_r2 && s2_out) out_r = out_r + s2_env_vol;
    }
    ...
    if (s3_running && s3_power) {
      ...
      if (mix_l3) out_l = out_l + s3_out;
      if (mix_r3) out_r = out_r + s3_out;
    }

    if (s4_running && s4_env_vol) {
      logic<1> s4_out = b1(s4_lfsr, 15);
      if (mix_l4 && s4_out) out_l = out_l + s4_env_vol;
      if (mix_r4 && s4_out) out_r = out_r + s4_env_vol;
    }
