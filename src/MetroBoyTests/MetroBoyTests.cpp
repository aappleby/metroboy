...
std::string microtest_path = "symlinks/GBMicrotests/bin";
...
void MetroBoyApp::post() {
  ...
  //load(microtest_path, "dma_basic");
  //load(microtest_path, "dma_0x8000");
  //load(microtest_path, "dma_0xA000");
  //load(microtest_path, "dma_0xE000");
  //load(microtest_path, "dma_0xFF00");
  //load(microtest_path, "poweron_006_oam");
...
TestResults run_microtest(std::string filename) {
  ...
  std::string path = microtest_path + filename;
  blob rom;
  ...
  ASSERT_EQ(false, phase >= timeout,
    "%-30s TIMEOUT\n",
    filename.c_str());

  ASSERT_EQ(0x31, result_c,
    "%-30s 0x%02x 0x%02x 0x%02x ERROR @ %d\n",
    filename.c_str(), result_a, result_b, result_c, phase);

  ASSERT_EQ(result_a, result_b,
    "%-30s 0x%02x 0x%02x 0x%02x FAIL @ %d\n",
    filename.c_str(), result_a, result_b, result_c, phase);
