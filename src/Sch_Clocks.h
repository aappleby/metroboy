...
struct ClockRegisters {
  ...
  uint8_t get_phase_clock() {
    return (uint8_t)pack(AFUR_ABCDxxxx.q(), ALEF_AxxxxFGH.qn(), APUK_xxCDEFxx.q(), ADYK_ABCxxxxH.qn());
  }

  //-----------------------------------------------------------------------------
  // Clock input & 4 mhz clocks

  /*p01.UCOB*/ wire UCOB_CLKBADp()  const { return not1(_SYS_PIN_CLK_A.q()); }
  /*p01.ATAL*/ wire ATAL_xBxDxFxH() const { return _SYS_PIN_CLK_B.q(); } // ignoring the deglitcher here
  ...
  /*p29.XOCE*/ wire XOCE_AxxDExxH() const { return not1(WOSU_xBCxxFGx.q());  } // def q, bus mux
  ...
  wire SYS_PIN_RSTp() const { return _SYS_PIN_RSTp.q(); }

  wire ALUR_SYS_RSTn() const { // used everywhere
    /*p01.AVOR*/ wire AVOR_SYS_RSTp = or2(_AFER_SYS_RSTp.q(), _ASOL_POR_DONEn.q());
  ...
  /*p07.UBET*/ wire UBET_T1p()        const { return not1(_SYS_PIN_T1n.q()); }
  /*p07.UVAR*/ wire UVAR_T2p()        const { return not1(_SYS_PIN_T2n.q()); }
  /*p07.UMUT*/ wire UMUT_MODE_DBG1p() const { return and2(_SYS_PIN_T1n.q(), UVAR_T2p()); }
  /*p07.UNOR*/ wire UNOR_MODE_DBG2p() const { return and2(_SYS_PIN_T2n.q(), UBET_T1p()); }
  /*p07.UPOJ*/ wire UPOJ_MODE_PRODn() const { return nand3(UBET_T1p(), UVAR_T2p(), _SYS_PIN_RSTp.q()); }
