...
void ResetRegisters::tick(SchematicTop& gb) {
  tick(gb.clk_reg,
       gb.dbg_reg.sig(gb).UPOJ_MODE_PROD,
       gb.rst_reg.sig(gb).TABA_RSTp,
       gb.cpu_bus.CPU_PIN_CLKREQ,
       gb.EXT_PIN_RST,
       gb.EXT_PIN_CLK_GOOD);
}
...
void ResetRegisters::tick(
...

  {
    ...
    /*p01.AFER*/ AFER_RSTp.set(BOMA_xBxxxxxx, UPOJ_MODE_PROD, ASOL_RST_LATCHp);
  }

  {
    ...
    /*p01.TUBO*/ TUBO_CLKREQn_LATCH.nor_latch(CPU_PIN_CLKREQ, UPYF);
  }
}
