...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && STM_A16_SP)             /**/ { pc = addr + 1; bus = 0;              /**/ alu_x = bus; alu_y = p;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ data_out    = alu_o;     state_ = 1; }
    if (state == 1 && STM_A16_SP)             /**/ { pc = addr + 1; bus = data_in;        /**/ alu_x = bus; alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
