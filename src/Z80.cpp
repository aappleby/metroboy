...
#define PASS(A)     { addr = A;           write = 0; read = 0; }
    ...
    if (LDM_A_A8          && state == 0) { pc = addr + 1;   ao = alu(0,      0,      data,        ao.f);           x = 0xFF;                             ao.f = f;                                           READ(pc);       state_ = 1; break; }
    ...
    if (STM_A8_A          && state == 0) { pc = addr + 1;   ao = alu(0,      0,      data,        ao.f);           x = 0xFF;                             ao.f = f;                                           READ(pc);       state_ = 1; break; }
    ...
    if (LDM_A_C           && state == 0) { pc = addr + 1;   ao = alu(0,      0,      c,           ao.f);           x = 0xFF; y = ao.x;                   ao.f = f;                                           READ(xy);       state_ = 1; break; }
    if (LDM_A_C           && state == 1) {                  ao = alu(0,      0,      data,        ao.f);           a = ao.x;                             ao.f = f;                                           READ(pc);       state_ = 0; break; }
    if (STM_C_A           && state == 0) { pc = addr + 1;   ao = alu(0,      0,      c,           ao.f);           x = 0xFF; y = ao.x;                   ao.f = f;                                           WRITE(xy, a);   state_ = 1; break; }
