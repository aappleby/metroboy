...
void Z80::tock(const int tcycle_, const uint8_t imask_, const uint8_t intf_) {
  ...
  do {
    ...
    if (                     state == INT4)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    
    if (CALL_A16          && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (CALL_A16          && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (CALL_A16          && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = PUSH0;  break; }
    ...
    if (CALL_A16          && state == PUSH2)  {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (CALL_CC_A16       && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (CALL_CC_A16       && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 0;     state_ = PUSH0;  break; }
    ...
    if (CALL_CC_A16       && state == PUSH2)  {                                                                                                                                         pc = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }

    if (nb) {
    if (CALL_CC_A16       && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (CALL_CC_A16       && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (CALL_CC_A16       && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }
    ...
    if (JP_CC_A16         && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (JP_CC_A16         && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = PTR1;   break; }
    if (JP_CC_A16         && state == PTR1)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }

    if (nb) {
    if (JP_CC_A16         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (JP_CC_A16         && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (JP_CC_A16         && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }
    ...
    if (JR_CC_R8          && state == ALU1)   { ao = alu(1, pcl, bus_in, 1);                    y = ao.x;                                                                                                    addr = pc;               bus_out = 0;              write = 0; read = 0;     state_ = ALU2;   break; }
    if (JR_CC_R8          && state == ALU2)   { ao = alu(1, pch, sxt(bus_in), ao.f & F_CARRY);  x = ao.x;                                                                                                    addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }

    if (nb) {
    if (JR_CC_R8          && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ALU1;   break; }
    if (JR_CC_R8          && state == ALU1)   {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }
    ...
    if (RET               && state == PTR1)   {                                                                                                                                         pc = addr;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (tb) {
    if (RET_CC            && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = POP0;   break; }
    if (RET_CC            && state == POP0)   {                                                                                                                                         pc = addr;           addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    if (RET_CC            && state == POP1)   {                                                 y = bus_in;                                                                             sp = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP2;   break; }
    if (RET_CC            && state == POP2)   {                                                 x = bus_in;                                                                             sp = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    if (RET_CC            && state == PTR1)   {                                                                                                                                         pc = addr;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }

    if (nb) {
    if (RET_CC            && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = POP0;   break; }
    if (RET_CC            && state == POP0)   {                                                                                                                                         pc = addr;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }
    ...
    if (RETI              && state == PTR1)   {                                                                                                                                         pc = addr;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
