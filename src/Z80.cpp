...
void Z80::tock_b(const uint8_t imask_, const uint8_t intf_, const Ack& ack) {
  ...
  if (INT) {
    ...
    if (state == 1 && OP_CB_R)                /**/ { alu_x = reg(CB_COL); pcl = inc(pcl, 1);  /**/ reg(CB_COL) = alu_cb(cb, f); pch = inc(pch, inc_c); set_addr(pc, 0); /**/                                                 set_f(mask); state_ = 0; }
    if (state == 1 && OP_CB_HL)               /**/ {                              y = l;            /**/                                      x = h;               set_addr(xy, 0); /**/                                                              state_ = 2; }
    if (state == 2)                           /**/ { alu_x = in;                  y = l;            /**/ out = alu_cb(cb, f);                 x = h;               set_addr(xy, 1); /**/                                                 set_f(mask); state_ = 3; }
