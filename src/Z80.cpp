...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && JP_HL)                  /**/ { pc = addr + 1; bus = z;              /**/ alu_x    = bus; alu_f = z;     alu_op = z;      bus = z;           /**/ addr = hl;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/                                        state_ = 0; }
    if (state == 0 && NOP)                    /**/ { pc = addr + 1; bus = z;              /**/ alu_x    = bus; alu_f = z;     alu_op = z;      bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/                                        state_ = 0; }
    if (state == 0 && STOP)                   /**/ { pc = addr + 1; bus = z;              /**/ alu_x    = bus; alu_f = z;     alu_op = z;      bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/                                        state_ = 0; }
    if (state == 0 && DI)                     /**/ { pc = addr + 1; bus = z;              /**/ alu_x    = bus; alu_f = z;     alu_op = z;      bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/                                        state_ = 0; }
    if (state == 0 && EI)                     /**/ { pc = addr + 1; bus = z;              /**/ alu_x    = bus; alu_f = z;     alu_op = z;      bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/                                        state_ = 0; }
    if (state == 0 && MV_R_R)                 /**/ { pc = addr + 1; bus = 0;              /**/ alu_x    = bus; alu_f = z;     alu_op = 0;      bus = reg(OP_COL); /**/ addr = pc;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/ reg(OP_ROW) = bus;                     state_ = 0; }
    ...
    if (state == 0 && RLC_A)                  /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0xB0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0x60); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0x70); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { pc = addr + 1; bus = a;              /**/ alu_x    = bus; alu_f = f;     alu_op = OP_ROW; bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; rlu(); bus = alu_o; /**/ a           = bus; update_flags(0x70); state_ = 0; }
    ...
    if (state == 2 && STM_A8_A)               /**/ {                bus = z;              /**/ alu_x    = bus; alu_f = z;     alu_op = z;      bus = z;           /**/ addr = pc;   write = 0; alu_y = bus; alu(); bus = alu_o; /**/                                        state_ = 0; }
    ...
    if (state == 2 && STM_A16_A)              /**/ { pc = addr + 1; bus = data_in;        /**/ alu_x    = bus; alu_f = z;     alu_op = 0;      bus = 0;           /**/ alu_y = bus; alu(); bus = alu_o; /**/ x = bus;  addr = xy;   write = 1;  state_ = 6; }
    ...
    if (state == 2 && JP_A16)                 /**/ { pc = addr + 1; bus = data_in;        /**/ alu_x    = bus; alu_f = z;     alu_op = 0;      bus = 0;           /**/ alu_y = bus; alu(); bus = alu_o; /**/ x = bus;  addr = xy;   write = 0;  state_ = 5; }
    ...
    if (state == 0 && STM_A16_A)              /**/ { pc = addr + 1; alu_x = 0;            /**/ alu_y = a;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ data_out    = alu_o;     state_ = 1; }
    if (state == 0 && STM_A16_SP)             /**/ { pc = addr + 1; alu_x = 0;            /**/ alu_y = p;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ data_out    = alu_o;     state_ = 1; }
    ...
    if (state == 1 && STM_A16_SP)             /**/ { pc = addr + 1; alu_x = data_in;      /**/ alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    ...
    if (state == 1 && STM_A16_A)              /**/ { pc = addr + 1; alu_x = data_in;      /**/ alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    if (state == 1 && LDM_A_A16)              /**/ { pc = addr + 1; alu_x = data_in;      /**/ alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    if (state == 1 && JP_A16)                 /**/ { pc = addr + 1; alu_x = data_in;      /**/ alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { pc = addr + 1; alu_x = data_in;      /**/ alu_y = p;           alu_f = f;     alu_op = 0;      alu(); /**/ addr = addr; write = 0;  /**/ p           = alu_o;     update_flags(0xF0); f &= ~(F_ZERO | F_NEGATIVE); state_ = 2; }
    if (state == 1 && LD_HL_SP_R8)            /**/ { pc = addr + 1; alu_x = data_in;      /**/ alu_y = p;           alu_f = f;     alu_op = 0;      alu(); /**/ addr = addr; write = 0;  /**/ l           = alu_o;     update_flags(0xF0); f &= ~(F_ZERO | F_NEGATIVE); state_ = 2; }
    ...
    if (state == 1 && RST_NN)                 /**/ { sp = addr - 1; alu_x = 0;            /**/ alu_y = pch;         alu_f = z;     alu_op = 0;      alu(); /**/ addr = sp;   write = 1;  /**/ data_out    = alu_o;     state_ = 2; }
    ...
    if (state == 2 && RST_NN)                 /**/ { sp = addr - 1; alu_x = 0;            /**/ alu_y = pcl;         alu_f = z;     alu_op = 0;      alu(); /**/ addr = sp;   write = 1;  /**/ data_out    = alu_o;     state_ = 3; }
    ...
    if (state == 3 && STM_A16_SP)             /**/ { xy = addr + 1; alu_x = 0;            /**/ alu_y = s;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = xy;   write = 1;  /**/ data_out    = alu_o;     state_ = 4; }
    ...
    if (state == 3 && RET_CC)                 /**/ { sp = addr + 1; alu_x = data_in;      /**/ alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = sp;   write = 0;  /**/ y           = alu_o;     state_ = 4; }
    if (state == 3 && LDM_A_A16)              /**/ {                alu_x = data_in;      /**/ alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ a           = alu_o;     state_ = 0; }
