...
void Z80::tock_b(const uint8_t imask_, const uint8_t intf_, const Ack& ack) {
  ...
  if (INT) {
    ...
    if (state == 1 && OP_CB_R)                /**/ { alu_x = R_CB;                               /**/                  pcl = inc(pcl, 1);           /**/ R_CB = alu_cb(cb, f);  pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; set_f(mask); }
    if (state == 1 && OP_CB_HL)               /**/ {                                             /**/ DBUS_BUSY;       xyl = l;                     /**/ DBUS_BUSY;             xyh = h;                  set_addr(xy, 0); state_ = 2; }
    if (state == 2)                           /**/ { alu_x = in;                                 /**/                                               /**/ out = alu_cb(cb, f);                             set_addr(xy, 1); state_ = 3; set_f(mask); }
    if (state == 3)                           /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                        pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
  }                                                                                                                                                                                                    
  else {                                                                                                                                                                                               
    bool branch = false;                                                                                                                                                                               
    switch (OP_ROW & 3) {                                                                                                                                                                              
    ...
    if (state == 0 && MV_R_R)                 /**/ { R_ROW = R_COL;                              /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && LD_SP_HL)               /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 1; }
    if (state == 1 && LD_SP_HL)               /**/ {                                             /**/ DBUS_BUSY;       spl = l;                     /**/ DBUS_BUSY;                   sph = h;                  set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && LD_R_D8)                /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 1; }
    if (state == 1 && LD_R_D8)                /**/ { R_ROW = in;                                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    // 8-bit alu                                                                                                                                                                                                                 
                                                                                                                                                                                      
    if (state == 0 && ALU_A_R)                /**/ { alu_y = R_COL;                              /**/ alu_x = a;       pcl = inc(pcl, 1);           /**/ a = alu(OP_ROW, f);          pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; set_f(0xF0); }
    if (state == 0 && INC_R)                  /**/ { alu_x = R_ROW;                              /**/                  pcl = inc(pcl, 1);           /**/ R_ROW = alu(1, F_CARRY);     pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; set_f(0xE0); }
    if (state == 0 && DEC_R)                  /**/ { alu_x = R_ROW;                              /**/                  pcl = inc(pcl, 1);           /**/ R_ROW = alu(3, F_CARRY);     pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; set_f(0xE0); }
    ...
    if (state == 1 && ALU_A_D8)               /**/ { alu_y = in;                                 /**/ alu_x = a;       pcl = inc(pcl, 1);           /**/ a = alu(OP_ROW, f);          pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; set_f(0xF0); }
                                                                                                                                                                                      
    if (state == 0 && ALU_A_HL)               /**/ {                                             /**/ DBUS_BUSY;       xyl = l;                     /**/                              xyh = h;                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && ALU_A_HL)               /**/ { alu_y = in;                                 /**/ alu_x = a;       pcl = inc(pcl, 1);           /**/ a = alu(OP_ROW, f);          pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; set_f(0xF0); }
                                                                                                                                                                                      
                                                                                                                                                                                      
    if (state == 0 && INC_AT_HL)              /**/ {                                             /**/ DBUS_BUSY;       xyl = l;                     /**/                              xyh = h;                  set_addr(xy, 0); state_ = 1; }
    if (state == 0 && DEC_AT_HL)              /**/ {                                             /**/ DBUS_BUSY;       xyl = l;                     /**/                              xyh = h;                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && INC_AT_HL)              /**/ { alu_x = in;                                 /**/                                               /**/ out = alu(1, F_CARRY);                                 set_addr(xy, 1); state_ = 2; set_f(0xE0); }
    if (state == 1 && DEC_AT_HL)              /**/ { alu_x = in;                                 /**/                                               /**/ out = alu(3, F_CARRY);                                 set_addr(xy, 1); state_ = 2; set_f(0xE0); }
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { alu_x = in;                                 /**/ DBUS_BUSY;       alu_y = spl;                 /**/ DBUS_BUSY;                   spl = alu(0, f);          set_addr(pc, 0); state_ = 2; set_f(0xF0); }
    if (state == 2 && ADD_SP_R8)              /**/ { alu_x = sxt(in);                            /**/ DBUS_BUSY;       alu_y = sph;                 /**/ DBUS_BUSY;                   sph = alu(1, f);          set_addr(pc, 0); state_ = 3; }
    ...
    if (state == 1 && LD_HL_SP_R8)            /**/ { alu_x = in;                                 /**/ DBUS_BUSY;       alu_y = spl;                 /**/ l = alu(0, f);                                         set_addr(pc, 0); state_ = 2; set_f(0xF0); }
    if (state == 2 && LD_HL_SP_R8)            /**/ { alu_x = sxt(in);                            /**/ DBUS_BUSY;       alu_y = sph;                 /**/ h = alu(1, f);                                         set_addr(pc, 0); state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && INC_BC)                 /**/ {                                             /**/ DBUS_BUSY;       c = inc(  c, 1);             /**/ DBUS_BUSY;                   b = inc(  b, inc_c);      set_addr(pc, 0); state_ = 1; }
    if (state == 0 && DEC_BC)                 /**/ {                                             /**/ DBUS_BUSY;       c = dec(  c, 1);             /**/ DBUS_BUSY;                   b = dec(  b, inc_c);      set_addr(pc, 0); state_ = 1; }
    if (state == 0 && INC_DE)                 /**/ {                                             /**/ DBUS_BUSY;       e = inc(  e, 1);             /**/ DBUS_BUSY;                   d = inc(  d, inc_c);      set_addr(pc, 0); state_ = 1; }
    if (state == 0 && DEC_DE)                 /**/ {                                             /**/ DBUS_BUSY;       e = dec(  e, 1);             /**/ DBUS_BUSY;                   d = dec(  d, inc_c);      set_addr(pc, 0); state_ = 1; }
    if (state == 0 && INC_HL)                 /**/ {                                             /**/ DBUS_BUSY;       l = inc(  l, 1);             /**/ DBUS_BUSY;                   h = inc(  h, inc_c);      set_addr(pc, 0); state_ = 1; }
    if (state == 0 && DEC_HL)                 /**/ {                                             /**/ DBUS_BUSY;       l = dec(  l, 1);             /**/ DBUS_BUSY;                   h = dec(  h, inc_c);      set_addr(pc, 0); state_ = 1; }
    ...
    if (state == 0 && STM_A16_SP)             /**/ { DBUS_BUSY;        out = spl;                /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 1; }
    if (state == 1 && STM_A16_SP)             /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && STM_A16_SP)             /**/ { DBUS_BUSY;        xyh = in;                 /**/                                               /**/                                                        set_addr(xy, 1); state_ = 3; }
    if (state == 3 && STM_A16_SP)             /**/ { DBUS_BUSY;        out = sph;                /**/                  xyl = inc(xyl, 1);           /**/                              xyh = inc(xyh, inc_c);    set_addr(xy, 1); state_ = 4; }
    ...
    if (state == 1 && STM_A16_A)              /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && STM_A16_A)              /**/ { DBUS_BUSY;        xyh = in;                 /**/                                               /**/                                                        set_addr(xy, 1); state_ = 3; }
    ...
    if (state == 1 && LDM_A_A16)              /**/ { xyl = in;                                   /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && LDM_A_A16)              /**/ { xyh = in;                                   /**/                                               /**/                                                        set_addr(xy, 0); state_ = 3; }
    if (state == 3 && LDM_A_A16)              /**/ { a = in;                                     /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    // indirect load/store                                                                                                                                                            
                                                                                                                                                                                      
    if (state == 0 && LDM_A_BC)               /**/ { DBUS_BUSY;        xyl = c;                  /**/ DBUS_BUSY;       xyh = b;                     /**/                                                        set_addr(xy, 0); state_ = 1; }
    if (state == 1 && LDM_A_BC)               /**/ { a = in;                                     /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && LDM_A_DE)               /**/ { DBUS_BUSY;        xyl = e;                  /**/ DBUS_BUSY;       xyh = d;                     /**/                                                        set_addr(xy, 0); state_ = 1; }
    if (state == 1 && LDM_A_DE)               /**/ { a = in;                                     /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && STM_BC_A)               /**/ { out = a;                                    /**/ DBUS_BUSY;       xyl = c;                     /**/ DBUS_BUSY;                   xyh = b;                  set_addr(xy, 1); state_ = 1; }
    if (state == 1 && STM_BC_A)               /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && STM_DE_A)               /**/ { out = a;                                    /**/ DBUS_BUSY;       xyl = e;                     /**/ DBUS_BUSY;                   xyh = d;                  set_addr(xy, 1); state_ = 1; }
    if (state == 1 && STM_DE_A)               /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && LDM_R_HL)               /**/ {                                             /**/ DBUS_BUSY;       xyl = l;                     /**/ DBUS_BUSY;                   xyh = h;                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && LDM_R_HL)               /**/ { reg(OP_ROW) = in;                           /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && STM_HL_R)               /**/ { out = reg(OP_COL);                          /**/ DBUS_BUSY;       xyl = l;                     /**/ DBUS_BUSY;                   xyh = h;                  set_addr(xy, 1); state_ = 1; }
    ...
    if (state == 1 && STM_HL_D8)              /**/ { out = in;                                   /**/ DBUS_BUSY;       xyl = l;                     /**/ DBUS_BUSY;                   xyh = h;                  set_addr(hl, 1); state_ = 2; }
    if (state == 2 && STM_HL_D8)              /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && LDM_A_HLP)              /**/ {                                             /**/ alu_x = l;       xyl = l;                     /**/ l = alu(1, F_CARRY);         xyh = h;                  set_addr(xy, 0); state_ = 1; }
    ...
    if (state == 0 && STM_HLM_A)              /**/ { out = a;                                    /**/ alu_x = l;       xyl = l;                     /**/ l = alu(3, F_CARRY);         xyh = h;                  set_addr(xy, 1); state_ = 1; }
    ...
    if (state == 0 && LDM_A_C)                /**/ {                                             /**/ DBUS_BUSY;       xyl = c;                     /**/ DBUS_BUSY;                   xyh = 0xFF;               set_addr(xy, 0); state_ = 1; }
    if (state == 0 && STM_C_A)                /**/ { out = a;                                    /**/ DBUS_BUSY;       xyl = c;                     /**/ DBUS_BUSY;                   xyh = 0xFF;               set_addr(xy, 1); state_ = 1; }
    if (state == 1 && LDM_A_A8)               /**/ {                                             /**/ DBUS_BUSY;       xyl = in;                    /**/ DBUS_BUSY;                   xyh = 0xFF;               set_addr(xy, 0); state_ = 2; }
    if (state == 1 && STM_A8_A)               /**/ { out = a;                                    /**/ DBUS_BUSY;       xyl = in;                    /**/ DBUS_BUSY;                   xyh = 0xFF;               set_addr(xy, 1); state_ = 2; }
    ...
    if (state == 0 && ADD_HL_SP)              /**/ { DBUS_BUSY;        alu_x = spl;              /**/ alu_y = l;       pcl = inc(pcl, 1);           /**/ l = alu(0, f);               pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 1; }
    ...
    if (state == 1 && ADD_HL_SP)              /**/ { DBUS_BUSY;        alu_x = sph;              /**/ alu_y = h;                                    /**/ h = alu(1, alu_f);                                     set_addr(pc, 0); state_ = 0; set_f(0x70); }
                                                                                                                                                                                                                                
    // conditional branches                                                                                                                                                                                                     
                                                                                                                                                                                                                                
    if (state == 0 && JR_R8)                  /**/ {                                             /**/                  xyl = inc(pcl, 1);           /**/                              xyh = inc(pch, inc_c);    set_addr(xy, 0); state_ = 1; }
    if (state == 1 && JR_R8)                  /**/ { DBUS_BUSY;        alu_x = xyl;              /**/ alu_y = in;                                   /**/ DBUS_BUSY;                   pcl = alu(1, F_CARRY);    set_addr(xy, 0); state_ = 2; }                                                         
    if (state == 2 && JR_R8)                  /**/ { DBUS_BUSY;        alu_x = xyh;              /**/ alu_y = sxt(in);                              /**/ DBUS_BUSY;                   pch = alu(1, alu_f);      set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && JR_CC_R8    &&  branch) /**/ {                                             /**/                  xyl = inc(pcl, 1);           /**/                              xyh = inc(pch, inc_c);    set_addr(xy, 0); state_ = 1; }
    if (state == 1 && JR_CC_R8    &&  branch) /**/ { DBUS_BUSY;        alu_x = xyl;              /**/ alu_y = in;                                   /**/ DBUS_BUSY;                   pcl = alu(1, F_CARRY);    set_addr(xy, 0); state_ = 2; }                                                         
    if (state == 2 && JR_CC_R8    &&  branch) /**/ { DBUS_BUSY;        alu_x = xyh;              /**/ alu_y = sxt(in);                              /**/ DBUS_BUSY;                   pch = alu(1, alu_f);      set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 0 && JP_CC_A16   && !branch) /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 1; }
    if (state == 1 && JP_CC_A16   &&  branch) /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 1 && JP_CC_A16   && !branch) /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && JP_CC_A16   &&  branch) /**/ { DBUS_BUSY;        xyh = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 3; }
    if (state == 2 && JP_CC_A16   && !branch) /**/ { DBUS_BUSY;        xyh = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 1 && JP_A16)                 /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && JP_A16)                 /**/ { DBUS_BUSY;        xyh = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 3; }
    ...
    if (state == 1 && CALL_CC_A16 &&  branch) /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && CALL_CC_A16 &&  branch) /**/ { DBUS_BUSY;        xyh = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(sp, 0); state_ = 3; }
    if (state == 3 && CALL_CC_A16 &&  branch) /**/ { DBUS_BUSY;        out = pch;                /**/                  spl = dec(spl, 1);           /**/                              sph = dec(sph, inc_c);    set_addr(sp, 1); state_ = 4; }
    if (state == 4 && CALL_CC_A16 &&  branch) /**/ { DBUS_BUSY;        out = pcl;                /**/                  spl = dec(spl, 1);           /**/                              sph = dec(sph, inc_c);    set_addr(sp, 1); state_ = 5; }
    ...
    if (state == 1 && CALL_A16)               /**/ { DBUS_BUSY;        xyl = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(pc, 0); state_ = 2; }
    if (state == 2 && CALL_A16)               /**/ { DBUS_BUSY;        xyh = in;                 /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(sp, 0); state_ = 3; }
    ...
    if (state == 2 && RETI)                   /**/ { DBUS_BUSY;        pch = in;                 /**/                  spl = inc(spl, 1);           /**/                              sph = inc(sph, inc_c);    set_addr(sp, 0); state_ = 3; }
    if (state == 3 && RETI)                   /**/ {                                             /**/                                               /**/                                                        set_addr(pc, 0); state_ = 0; }
                                                                                                                                                                                      
    if (state == 0 && RET)                    /**/ {                                             /**/                  pcl = inc(pcl, 1);           /**/                              pch = inc(pch, inc_c);    set_addr(sp, 0); state_ = 1; }
    if (state == 1 && RET)                    /**/ { DBUS_BUSY;        pcl = in;                 /**/                  spl = inc(spl, 1);           /**/                              sph = inc(sph, inc_c);    set_addr(sp, 0); state_ = 2; }
    if (state == 2 && RET)                    /**/ { DBUS_BUSY;        pch = in;                 /**/                                               /**/                                                        set_addr(sp, 0); state_ = 3; }
