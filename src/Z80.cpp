...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && STM_A16_A)              /**/ { pc = addr + 1; bus = 0;              /**/ alu_x = bus; alu_y = a;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ data_out    = alu_o;     state_ = 1; }
    if (state == 1 && STM_A16_A)              /**/ { pc = addr + 1; bus = data_in;        /**/ alu_x = bus; alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    ...
    if (state == 3 && STM_A16_A)              /**/ {                bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = pc;   write = 0;  /**/                          state_ = 0; }

    if (state == 0 && LDM_A_A16)              /**/ { pc = addr + 1; bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = pc;   write = 0;  /**/                          state_ = 1; }
    if (state == 1 && LDM_A_A16)              /**/ { pc = addr + 1; bus = data_in;        /**/ alu_x = bus; alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    ...
    if (state == 3 && LDM_A_A16)              /**/ {                bus = data_in;        /**/ alu_x = bus; alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ a           = alu_o;     state_ = 0; }

    if (state == 0 && JP_A16)                 /**/ { pc = addr + 1; bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = pc;   write = 0;  /**/                          state_ = 1; }
    if (state == 1 && JP_A16)                 /**/ { pc = addr + 1; bus = data_in;        /**/ alu_x = bus; alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = pc;   write = 0;  /**/ y           = alu_o;     state_ = 2; }
    ...
    if (state == 3 && JP_A16)                 /**/ {                bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = xy;   write = 0;  /**/                          state_ = 0; }

    if (state == 0 && RST_NN)                 /**/ { pc = addr + 1; bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = sp;   write = 0;  /**/                          state_ = 1; }
    if (state == 1 && RST_NN)                 /**/ { sp = addr - 1; bus = 0;              /**/ alu_x = bus; alu_y = pch;         alu_f = z;     alu_op = 0;      alu(); /**/ addr = sp;   write = 1;  /**/ data_out    = alu_o;     state_ = 2; }
    if (state == 2 && RST_NN)                 /**/ { sp = addr - 1; bus = 0;              /**/ alu_x = bus; alu_y = pcl;         alu_f = z;     alu_op = 0;      alu(); /**/ addr = sp;   write = 1;  /**/ data_out    = alu_o;     state_ = 3; }
    if (state == 3 && RST_NN)                 /**/ {                bus = op;             /**/ alu_x = bus; alu_f = z;     alu_op = 4;      bus = 0x38;        /**/ alu_y = bus; alu(); bus = alu_o; /**/ y = bus;  addr = xy;   write = 0;  state_ = 0; }

    if (state == 0 && RET_CC && !branch)      /**/ { pc = addr + 1; bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = pc;   write = 0;  /**/                          state_ = 6; }
    if (state == 0 && RET_CC && branch)       /**/ { pc = addr + 1; bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = pc;   write = 0;  /**/                          state_ = 1; }
    if (state == 1 && RET_CC && branch)       /**/ {                bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = pc;   write = 0;  /**/                          state_ = 2; }
    if (state == 2 && RET_CC)                 /**/ {                bus = z;              /**/ alu_x = bus; alu_y = z;           alu_f = z;     alu_op = z;      alu(); /**/ addr = sp;   write = 0;  /**/                          state_ = 3; }
    if (state == 3 && RET_CC)                 /**/ { sp = addr + 1; bus = data_in;        /**/ alu_x = bus; alu_y = 0;           alu_f = z;     alu_op = 0;      alu(); /**/ addr = sp;   write = 0;  /**/ y           = alu_o;     state_ = 4; }
