...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && OP_CB_R)                /**/ { pc = addr + 1;   alu_x = reg(CB_COL);        /**/ alu_cb(); reg(CB_COL) = alu_o;                                    addr = pc; write = 0; /**/                                  /**/ update_flags(mask);                    state_ = 0; }
    if (state == 1 && OP_CB_HL)               /**/ { pc = addr + 1;                               /**/                                                                   addr = hl; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && PREFIX_CB)              /**/ { alu_x = data_in;                             /**/ alu_cb(); data_out = alu_o;                                       addr = hl; write = 1; /**/                                  /**/ update_flags(mask);                    state_ = 3; }
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { pc = addr + 1; bus = data_in;                /**/ alu_x = bus; alu(0, f);                                           addr = pc; write = 0; /**/ p = bus; bus = sxt(data_in);     /**/                    update_flags(0xF0); state_ = 2; }
    ...
    if (state == 1 && LD_HL_SP_R8)            /**/ { pc = addr + 1; bus = data_in;                /**/ alu_x = bus; alu(0, f);                                                      write = 0; /**/ l = bus; bus = s;                /**/ alu_y = bus;       update_flags(0xF0); state_ = 2; }
    ...
    if (state == 1 && ALU_A_D8)               /**/ { pc = addr + 1; bus = data_in;                /**/ alu_y = bus; alu(OP_ROW, f);                                      addr = pc; write = 0; /**/ a = bus;                         /**/                    update_flags(0xF0); state_ = 0; }
    ...
    if (state == 0 && INC_AT_HL)              /**/ { pc = addr + 1; bus = 1;                      /**/ alu_y = bus;                                                      addr = hl; write = 0; /**/                                  /**/                                        state_ = 1; }
