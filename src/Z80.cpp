...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && STM_A8_A)               /**/ { bus = a;              data_out = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    ...
    if (state == 0 && STM_A16_SP)             /**/ { bus = p;              data_out = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_A16_SP)             /**/ { bus = data_in;               y = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && STM_A16_SP)             /**/ { bus = data_in;               x = bus; pc = addr + 1; /**/                                                                   addr = xy; write = 1; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && STM_A16_SP)             /**/ { bus = s;              data_out = bus; xy = addr + 1; /**/                                                                   addr = xy; write = 1; /**/                                  /**/                                        state_ = 4; }
    if (state == 4 && STM_A16_SP)             /**/ {                                                      /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                           
    if (state == 0 && STM_A16_A)              /**/ { bus = a;              data_out = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    ...
    if (state == 2 && RST_NN)                 /**/ { bus = pcl;            data_out = bus; sp = addr - 1; /**/ bus = 0x38;                                          alu_y = bus; addr = sp; write = 1; /**/ bus = op;           alu_x = bus; /**/                                        state_ = 3; }
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { bus = data_in;           alu_x = bus; pc = addr + 1; /**/ alu(0, f);                                              p = bus;  addr = pc; write = 0; /**/ bus = sxt(data_in);              /**/                    update_flags(0xF0); state_ = 2; }
    if (state == 2 && ADD_SP_R8)              /**/ { alu_x = bus;   bus = s;  alu_y = bus;                /**/ alu(1, f);                                              s = bus;  addr = pc; write = 0; /**/                                  /**/                                        state_ = 3; }
