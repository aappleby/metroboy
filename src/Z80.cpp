...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && INC_R)                  /**/ { pc = addr + 1; bus = reg(OP_ROW);                 /**/ alu_x = bus;                             bus = 1;                 addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ reg(OP_ROW) = bus; update_flags(0xE0); state_ = 0; }
    if (state == 0 && DEC_R)                  /**/ { pc = addr + 1; bus = reg(OP_ROW);                 /**/ alu_x = bus;                             bus = 1;                 addr = pc; write = 0; /**/ alu_y = bus; alu(2, f);          /**/ reg(OP_ROW) = bus; update_flags(0xE0); state_ = 0; }
    if (state == 0 && ALU_A_R)                /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                             bus = reg(OP_COL);       addr = pc; write = 0; /**/ alu_y = bus; alu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RLC_A)                  /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xB0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x60); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x70); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { pc = addr + 1; bus = a;                           /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x70); state_ = 0; }
    ...
    if (state == 0 && ADD_HL_BC)              /**/ { pc = addr + 1; bus = c;                           /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_DE)              /**/ { pc = addr + 1; bus = e;                           /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_HL)              /**/ { pc = addr + 1; bus = l;                           /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_SP)              /**/ { pc = addr + 1; bus = p;                           /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
