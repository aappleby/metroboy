...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { bus = data_in;           alu_x = bus;                /**/ alu(0, f);               p = bus;  addr = pc; write = 0; /**/                                  /**/                    update_flags(0xF0); state_ = 2; }
    if (state == 2 && ADD_SP_R8)              /**/ { bus = sxt(data_in);      alu_x = bus; pc = addr + 1; /**/                                    addr = pc; write = 0; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && ADD_SP_R8)              /**/ { bus = s;                 alu_y = bus;                /**/ alu(1, f);               s = bus;  addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
