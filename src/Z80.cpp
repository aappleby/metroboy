...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && JP_A16)                 /**/ { pc = addr + 1; bus = data_in;        /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && JP_A16)                 /**/ { pc = addr + 1; bus = data_in;        /**/ x = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 3; }
    ...
    if (state == 2 && RST_NN)                 /**/ { sp = addr - 1; bus = pcl;            /**/ data_out = bus; bus = 0x38;                                       addr = sp; write = 1; /**/ alu_y = bus; bus = op;           /**/ alu_x = bus;                           state_ = 3; }
    if (state == 3 && RST_NN)                 /**/ {                                      /**/ alu_op = 4; alu(); bus = alu_o; y = bus;                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 3 && RET_CC)                 /**/ { sp = addr + 1; bus = data_in;        /**/ y = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 4; }
    if (state == 4 && RET_CC)                 /**/ { sp = addr + 1; bus = data_in;        /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 0 && MV_R_R)                 /**/ { pc = addr + 1; bus = reg(OP_COL);    /**/ reg(OP_ROW) = bus;                                                addr = pc; write = 0; /**/                                ; /**/                                        state_ = 0; }
    ...
    if (state == 2 && PUSH_BC)                /**/ { sp = addr - 1; bus = c;              /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && PUSH_DE)                /**/ { sp = addr - 1; bus = e;              /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && PUSH_HL)                /**/ { sp = addr - 1; bus = l;              /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && PUSH_AF)                /**/ { sp = addr - 1; bus = f;              /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    ...
    if (state == 2 && JP_CC_A16 && branch)    /**/ { pc = addr + 1; bus = data_in;        /**/ x = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 5; }
