...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && ADD_SP_R8)              /**/ { pcl = adl = abus = inc(pcl, 1); /**/ pch = adh = abus = inc(pch, inc_c); set_addr(pc, 0); /**/                                              state_ = 1; }
    if (state == 1 && ADD_SP_R8)              /**/ { alu_x = dbus = in;              /**/ alu_y = dbus = abus = spl;          set_addr(pc, 0); /**/ spl = abus = dbus = alu(0, f);  set_f(0xF0); state_ = 2; }
    if (state == 2 && ADD_SP_R8)              /**/ { alu_x = dbus = sxt(in);         /**/ alu_y = dbus = abus = sph;          set_addr(pc, 0); /**/ sph = abus = dbus = alu(1, f);               state_ = 3; }
    if (state == 3 && ADD_SP_R8)              /**/ { pcl = adl = abus = inc(pcl, 1); /**/ pch = adh = abus = inc(pch, inc_c); set_addr(pc, 0); /**/                                              state_ = 0; }

    if (state == 0 && LD_HL_SP_R8)            /**/ { pcl = adl = abus = inc(pcl, 1); /**/ pch = adh = abus = inc(pch, inc_c); set_addr(pc, 0); /**/                                              state_ = 1; }
    if (state == 1 && LD_HL_SP_R8)            /**/ { alu_x = dbus = in;              /**/ alu_y = dbus = abus = spl;          set_addr(pc, 0); /**/ l = dbus = alu(0, f);           set_f(0xF0); state_ = 2; }
    if (state == 2 && LD_HL_SP_R8)            /**/ { alu_x = dbus = sxt(in);         /**/ alu_y = dbus = abus = sph;          set_addr(pc, 0); /**/ h = dbus = alu(1, f);                        state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ { pcl = adl = abus = inc(pcl, 1); /**/ pch = adh = abus = inc(pch, inc_c); set_addr(pc, 0); /**/                                              state_ = 0; }
