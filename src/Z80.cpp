...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && LD_HL_SP_R8)            /**/ { bus = data_in;           alu_x = bus; pc = addr + 1; /**/ alu(0, f);                l = bus;            write = 0; /**/                                  /**/                    update_flags(0xF0); state_ = 2; }
    if (state == 2 && LD_HL_SP_R8)            /**/ { bus = s;                 alu_y = bus;                /**/                                               write = 0; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ { bus = sxt(data_in);      alu_x = bus;                /**/ alu(1, f);                h = bus; addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                                                                                
    if (state == 0 && ALU_A_D8)               /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                    addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ALU_A_D8)               /**/ { bus = data_in;           alu_y = bus; pc = addr + 1; /**/ alu(OP_ROW, f);           a = bus; addr = pc; write = 0; /**/                                  /**/                    update_flags(0xF0); state_ = 0; }
    ...
    if (state == 1 && DEC_AT_HL)              /**/ { bus = data_in;           alu_x = bus;                /**/ bus = 1;              alu_y = bus; addr = hl; write = 1; /**/ alu(2, f);       data_out = bus; /**/                    update_flags(0xE0); state_ = 2; }
    ...
    if (state == 0 && INC_R)                  /**/ { bus = reg(OP_ROW);       alu_x = bus; pc = addr + 1; /**/ bus = 1;              alu_y = bus; addr = pc; write = 0; /**/ alu(0, f);   reg(OP_ROW) = bus; /**/                     update_flags(0xE0); state_ = 0; }
    if (state == 0 && DEC_R)                  /**/ { bus = reg(OP_ROW);       alu_x = bus; pc = addr + 1; /**/ bus = 1;              alu_y = bus; addr = pc; write = 0; /**/ alu(2, f);   reg(OP_ROW) = bus; /**/                     update_flags(0xE0); state_ = 0; }
    if (state == 0 && ALU_A_R)                /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/ bus = reg(OP_COL);    alu_y = bus; addr = pc; write = 0; /**/ alu(OP_ROW, f);        a = bus; /**/                     update_flags(0xF0); state_ = 0; }
    if (state == 0 && RLC_A)                  /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0xF0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0xF0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0xF0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0xF0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0xB0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0x60); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0x70); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                       alu_y = bus; addr = pc; write = 0; /**/ rlu(OP_ROW, f);        a = bus; /**/                     update_flags(0x70); state_ = 0; }
    ...
    if (state == 0 && ADD_HL_BC)              /**/ { bus = c;                 alu_x = bus; pc = addr + 1; /**/ bus = l;              alu_y = bus; addr = pc; write = 0; /**/ alu(0, f);              l = bus; /**/                                        state_ = 1; }
    if (state == 0 && ADD_HL_DE)              /**/ { bus = e;                 alu_x = bus; pc = addr + 1; /**/ bus = l;              alu_y = bus; addr = pc; write = 0; /**/ alu(0, f);              l = bus; /**/                                        state_ = 1; }
    if (state == 0 && ADD_HL_HL)              /**/ { bus = l;                 alu_x = bus; pc = addr + 1; /**/ bus = l;              alu_y = bus; addr = pc; write = 0; /**/ alu(0, f);              l = bus; /**/                                        state_ = 1; }
    if (state == 0 && ADD_HL_SP)              /**/ { bus = p;                 alu_x = bus; pc = addr + 1; /**/ bus = l;              alu_y = bus; addr = pc; write = 0; /**/ alu(0, f);              l = bus; /**/                                        state_ = 1; }
    if (state == 1 && ADD_HL_BC)              /**/ { bus = b;                 alu_x = bus;                /**/ bus = h;              alu_y = bus; addr = pc; write = 0; /**/ alu(1, alu_f);          h = bus; /**/                    update_flags(0x70); state_ = 0; }
    if (state == 1 && ADD_HL_DE)              /**/ { bus = d;                 alu_x = bus;                /**/ bus = h;              alu_y = bus; addr = pc; write = 0; /**/ alu(1, alu_f);          h = bus; /**/                    update_flags(0x70); state_ = 0; }
    if (state == 1 && ADD_HL_HL)              /**/ { bus = h;                 alu_x = bus;                /**/ bus = h;              alu_y = bus; addr = pc; write = 0; /**/ alu(1, alu_f);          h = bus; /**/                    update_flags(0x70); state_ = 0; }
    if (state == 1 && ADD_HL_SP)              /**/ { bus = s;                 alu_x = bus;                /**/ bus = h;              alu_y = bus; addr = pc; write = 0; /**/ alu(1, alu_f);          h = bus; /**/                    update_flags(0x70); state_ = 0; }
