...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && JR_R8)                  /**/ { bus = data_in;           alu_y = bus;                /**/ alu(0, f);                                               y = bus; addr = pc; write = 0; /**/                                  /**/ bus = pch; alu_x = bus;                state_ = 2; }                                                         
    if (state == 2 && JR_R8)                  /**/ { bus = sxt(alu_y);        alu_y = bus;                /**/ alu(1, alu_f);                                           x = bus; addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                                          
    if (state == 0 && JR_CC_R8 && branch)     /**/ {                                       pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ pc = addr + 1;                   /**/ bus = pcl;                alu_x = bus; state_ = 1; }
    if (state == 1 && JR_CC_R8 && branch)     /**/ { bus = data_in;           alu_y = bus;                /**/ alu(0, f);                                               y = bus; addr = pc; write = 0; /**/                                  /**/ bus = pch;                alu_x = bus; state_ = 2; }
    if (state == 2 && JR_CC_R8 && branch)     /**/ { bus = sxt(alu_y);        alu_y = bus;                /**/ alu(1, alu_f);                                           x = bus; addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
