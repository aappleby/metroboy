...
#define PASS(A)     { addr = A;           write = 0; read = 0; }
    ...
    if (CALL_A16          && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (CALL_A16          && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 PASS(sp);       state_ = 3; break; }
    ...
    if (MV_R_R            && state == 0) { pc = addr + 1;   ao = alu(0,      r,      0,           f);              reg_put8(ao.x);                                                                           READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                               
    if (DEC_R             && state == 0) { pc = addr + 1;   ao = alu(2,      r,      1,           f);              reg_put8(ao.x);                       set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (INC_R             && state == 0) { pc = addr + 1;   ao = alu(0,      r,      1,           f);              reg_put8(ao.x);                       set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (INC_AT_HL         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (INC_AT_HL         && state == 1) {                  ao = alu(0,      data,   1,           f);              y = ao.x;                             set_flag(ao.f);                                     WRITE(hl, y);   state_ = 2; break; }
    if (INC_AT_HL         && state == 2) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (DEC_AT_HL         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (DEC_AT_HL         && state == 1) {                  ao = alu(2,      data,   1,           f);              y = ao.x;                             set_flag(ao.f);                                     WRITE(hl, y);   state_ = 2; break; }
    if (DEC_AT_HL         && state == 2) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                             
    if (RLU_R             && state == 0) { pc = addr + 1;   ao = rlu(OP_ROW, r,                   f);              a = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (ALU_A_R           && state == 0) { pc = addr + 1;   ao = alu(OP_ROW, a,      r,           f);              a = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (ADD_HL_BC         && state == 0) { pc = addr + 1;   ao = alu(0,      l,      c,           ao.f);           l = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 1; break; }
    if (ADD_HL_BC         && state == 1) {                  ao = alu(1,      h,      b,           ao.f);           h = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (ADD_HL_DE         && state == 0) { pc = addr + 1;   ao = alu(0,      l,      e,           ao.f);           l = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 1; break; }
    if (ADD_HL_DE         && state == 1) {                  ao = alu(1,      h,      d,           ao.f);           h = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (ADD_HL_HL         && state == 0) { pc = addr + 1;   ao = alu(0,      l,      l,           ao.f);           l = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 1; break; }
    if (ADD_HL_HL         && state == 1) {                  ao = alu(1,      h,      h,           ao.f);           h = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (ADD_HL_SP         && state == 0) { pc = addr + 1;   ao = alu(0,      l,      p,           ao.f);           l = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 1; break; }
    if (ADD_HL_SP         && state == 1) {                  ao = alu(1,      h,      s,           ao.f);           h = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }

    if (ADD_SP_R8         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (ADD_SP_R8         && state == 1) { pc = addr + 1;   ao = alu(0,      p,      data,        f);              y = ao.x;                             set_flag(ao.f);                                     PASS(pc);       state_ = 2; break; }
    ...
    if (ALU_A_D8          && state == 1) { pc = addr + 1;   ao = alu(OP_ROW, a,      data,        f);              a = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (ALU_A_HL          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (ALU_A_HL          && state == 1) {                  ao = alu(OP_ROW, a,      data,        f);              a = ao.x;                             set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    ...
    if (JR_CC_R8 && tb    && state == 2) {                  ao = alu(1,      pch,    sxt(data),   ao.f);           x = ao.x;                                                                                 READ(xy);       state_ = 0; break; }
    ...
    if (JR_R8             && state == 2) {                  ao = alu(1,      pch,    sxt(data),   ao.f);           x = ao.x;                                                                                 READ(xy);       state_ = 0; break; }
                                                                    
    if (LD_HL_SP_R8       && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_HL_SP_R8       && state == 1) { pc = addr + 1;                                                                                                                                                    PASS(pc);       state_ = 2; break; }
    if (LD_HL_SP_R8       && state == 2) {                  ao = alu(0,      p,      data,        f);              l = ao.x;                             set_flag(ao.f);                                     PASS(pc);       state_ = 3; break; }
    if (LD_HL_SP_R8       && state == 3) {                  ao = alu(1,      s,      sxt(data),   f);              h = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                             

    if (CALL_CC_A16 && tb && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (CALL_CC_A16 && tb && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (CALL_CC_A16 && tb && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 PASS(sp);       state_ = 3; break; }
    ...
    if (CALL_CC_A16 && nb && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (CALL_CC_A16 && nb && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (RST_NN            && state == 0) { pc = addr + 1;                                                                                                                                                    PASS(sp);       state_ = 1; break; }
    if (RST_NN            && state == 1) { sp = addr - 1;                                                                                                                                                    WRITE(sp, pch); state_ = 2; break; }
    if (RST_NN            && state == 2) { sp = addr - 1;   ao = alu(0,      0,      0,           f);              x = ao.x;                                                                                 WRITE(sp, pcl); state_ = 3; break; }
    if (RST_NN            && state == 3) {                  ao = alu(0,      0,      op & 0x38,   f);              y = ao.x;                                                                                 READ(xy);       state_ = 0; break; }
    ...
    if (JP_A16            && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (JP_A16            && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 PASS(pc);       state_ = 3; break; }
    if (JP_A16            && state == 3) {                                                                                                                                                                   READ(xy);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (JP_CC_A16 && tb   && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (JP_CC_A16 && tb   && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (JP_CC_A16 && tb   && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 READ(pc);       state_ = 3; break; }
    if (JP_CC_A16 && tb   && state == 3) {                                                                                                                                                                   READ(xy);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (JP_CC_A16 && nb   && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (JP_CC_A16 && nb   && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (JP_CC_A16 && nb   && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
                                                                                                                                                                                                                                  
    //----------                                                                                                                                                                                                                  
                                                                                                                                                                                                                                  
    if (LD_BC_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_BC_D16         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              c = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_BC_D16         && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              b = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_DE_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_DE_D16         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              e = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_DE_D16         && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              d = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_HL_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_HL_D16         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              l = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_HL_D16         && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              h = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_SP_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_SP_D16         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              p = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_SP_D16         && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              s = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_BC          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(bc);       state_ = 1; break; }
    if (LDM_A_BC          && state == 1) { bc = addr;       ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_DE          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(de);       state_ = 1; break; }
    if (LDM_A_DE          && state == 1) { de = addr;       ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_HLP         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (LDM_A_HLP         && state == 1) { hl = addr + 1;   ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_HLM         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (LDM_A_HLM         && state == 1) { hl = addr - 1;   ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_R_D8           && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_R_D8           && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              reg_put8(ao.x);                                                                           READ(pc);       state_ = 0; break; }
    ...
    if (LDM_A_A16         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (LDM_A_A16         && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 READ(xy);       state_ = 3; break; }
    if (LDM_A_A16         && state == 3) {                  ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_R_HL          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (LDM_B_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              b = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    if (LDM_C_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              c = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    if (LDM_D_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              d = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    if (LDM_E_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              e = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    if (LDM_H_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              h = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    if (LDM_L_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              l = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    if (LDM_A_HL          && state == 1) {                  ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }

    if (POP_BC            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_BC            && state == 1) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              c = ao.x;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_BC            && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              b = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_DE            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_DE            && state == 1) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              e = ao.x;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_DE            && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              d = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_HL            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_HL            && state == 1) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              l = ao.x;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_HL            && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              h = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_AF            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_AF            && state == 1) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              f = ao.x;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_AF            && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
    ...
    if (RET               && state == 1) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(sp);       state_ = 2; break; }
    if (RET               && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 PASS(xy);       state_ = 3; break; }
    ...
    if (RET_CC && tb      && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(sp);       state_ = 3; break; }
    if (RET_CC && tb      && state == 3) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 PASS(xy);       state_ = 4; break; }
    ...
    if (RETI              && state == 1) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(sp);       state_ = 2; break; }
    if (RETI              && state == 2) { sp = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 PASS(xy);       state_ = 3; break; }
    if (RETI              && state == 3) {                                                                                                                                                                   READ(xy);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_A16_A         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (STM_A16_A         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (STM_A16_A         && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 WRITE(xy, a);   state_ = 3; break; }
    if (STM_A16_A         && state == 3) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_A8          && state == 0) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = 0xFF;                                                                                 READ(pc);       state_ = 1; break; }
    if (LDM_A_A8          && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(xy);       state_ = 2; break; }
    if (LDM_A_A8          && state == 2) {                  ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_A8_A          && state == 0) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = 0xFF;                                                                                 READ(pc);       state_ = 1; break; }
    if (STM_A8_A          && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 WRITE(xy, a);   state_ = 2; break; }
    if (STM_A8_A          && state == 2) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_C           && state == 0) { pc = addr + 1;   ao = alu(0,      0,      c,           f);              x = 0xFF; y = ao.x;                                                                       READ(xy);       state_ = 1; break; }
    if (LDM_A_C           && state == 1) {                  ao = alu(0,      0,      data,        f);              a = ao.x;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_C_A           && state == 0) { pc = addr + 1;   ao = alu(0,      0,      c,           f);              x = 0xFF; y = ao.x;                                                                       WRITE(xy, a);   state_ = 1; break; }
    if (STM_C_A           && state == 1) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_HL_D8         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (STM_HL_D8         && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 WRITE(hl, y);   state_ = 2; break; }
    ...
    if (STM_A16_SP        && state == 1) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              y = ao.x;                                                                                 READ(pc);       state_ = 2; break; }
    if (STM_A16_SP        && state == 2) { pc = addr + 1;   ao = alu(0,      0,      data,        f);              x = ao.x;                                                                                 WRITE(xy, p);   state_ = 3; break; }
    ...
    if (OP_CB_R           && state == 1) { pc = addr + 1;   ao = alu_cb(cb, reg_get8(CB_COL), f);                  reg_put8(CB_COL, ao.x);               set_flag(ao.f);                                     READ(pc);       state_ = 0; break; }
    if (OP_CB_HL          && state == 1) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 2; break; }
    if (OP_CB_HL          && state == 2) {                  ao = alu_cb(cb, data, f);                              y = ao.x;                             set_flag(ao.f);                                     WRITE(hl, y);   state_ = 3; break; }
