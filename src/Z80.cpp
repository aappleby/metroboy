...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && JR_R8)                  /**/ { bus = data_in;                                    /**/ alu_y = bus; alu(0, f);                                           addr = pc; write = 0; /**/ y = bus; bus = pch;              /**/ alu_x = bus; bus = sxt(data_in);       state_ = 2; }                                                         
    if (state == 2 && JR_R8)                  /**/ { alu_y = bus; alu(1, alu_f);                       /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                                       
    if (state == 0 && JR_CC_R8 && branch)     /**/ {                                    pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ pc = addr + 1; bus = pcl;        /**/ alu_x = bus;                           state_ = 1; }
    if (state == 1 && JR_CC_R8 && branch)     /**/ { bus = data_in;                                    /**/ alu_y = bus; alu(0, f);                                           addr = pc; write = 0; /**/ y = bus; bus = pch;              /**/ alu_x = bus; bus = sxt(data_in);       state_ = 2; }
    if (state == 2 && JR_CC_R8 && branch)     /**/ { alu_y = bus; alu(1, alu_f);                       /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 0 && STM_A8_A)               /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_A8_A)               /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = xy; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && STM_A8_A)               /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && LDM_A_A8)               /**/ {                                    pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && LDM_A_A8)               /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && LDM_A_A8)               /**/ { bus = data_in;                                    /**/ a = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && STM_A16_SP)             /**/ { bus = p;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_A16_SP)             /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && STM_A16_SP)             /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = xy; write = 1; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && STM_A16_SP)             /**/ { bus = s;                           xy = addr + 1; /**/ data_out = bus;                                                   addr = xy; write = 1; /**/                                  /**/                                        state_ = 4; }
    if (state == 4 && STM_A16_SP)             /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && STM_A16_A)              /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_A16_A)              /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && STM_A16_A)              /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = xy; write = 1; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && STM_A16_A)              /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && LDM_A_A16)              /**/ {                                    pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && LDM_A_A16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && LDM_A_A16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 3; }
    ...
    if (state == 1 && JP_A16)                 /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && JP_A16)                 /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && JP_A16)                 /**/ {                                                   /**/                                                                   addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                                                                                                                                                                                               
    if (state == 0 && RST_NN)                 /**/ {                                    pc = addr + 1; /**/                                                                   addr = sp; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && RST_NN)                 /**/ { bus = pch;                         sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && RST_NN)                 /**/ { bus = pcl;                         sp = addr - 1; /**/ data_out = bus; bus = 0x38;                                       addr = sp; write = 1; /**/ alu_y = bus; bus = op;           /**/ alu_x = bus;                           state_ = 3; }
    if (state == 3 && RST_NN)                 /**/ { alu(4, f);                                        /**/ y = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 3 && RET_CC)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ y = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 4; }
    if (state == 4 && RET_CC)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && ADD_SP_R8)              /**/ { bus = p;                           pc = addr + 1; /**/ alu_y = bus;                                                      addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ADD_SP_R8)              /**/ { bus = data_in;                     pc = addr + 1; /**/ alu_x = bus; alu(0, f);                                           addr = pc; write = 0; /**/ p = bus; bus = sxt(data_in);     /**/                    update_flags(0xF0); state_ = 2; }
    if (state == 2 && ADD_SP_R8)              /**/ { alu_x = bus;   bus = s;                           /**/ alu_y = bus; alu(1, f);                                           addr = pc; write = 0; /**/ s = bus;                         /**/                                        state_ = 3; }
    if (state == 3 && ADD_SP_R8)              /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                                       
    if (state == 0 && LD_HL_SP_R8)            /**/ { bus = p;                           pc = addr + 1; /**/ alu_y = bus;                                                      addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && LD_HL_SP_R8)            /**/ { bus = data_in;                     pc = addr + 1; /**/ alu_x = bus; alu(0, f);                                                      write = 0; /**/ l = bus; bus = s;                /**/ alu_y = bus;       update_flags(0xF0); state_ = 2; }
    if (state == 2 && LD_HL_SP_R8)            /**/ { bus = sxt(data_in);                               /**/ alu_x = bus; alu(1, f);                                                      write = 0; /**/ h = bus;                         /**/                                        state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && ALU_A_D8)               /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ALU_A_D8)               /**/ { bus = data_in;                     pc = addr + 1; /**/ alu_y = bus; alu(OP_ROW, f);                                      addr = pc; write = 0; /**/ a = bus;                         /**/                    update_flags(0xF0); state_ = 0; }

    if (state == 0 && ALU_A_HL)               /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = hl; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ALU_A_HL)               /**/ { bus = data_in;                                    /**/ alu_y = bus; alu(OP_ROW, f);                                      addr = pc; write = 0; /**/ a = bus;                         /**/                    update_flags(0xF0); state_ = 0; }

    if (state == 0 && INC_AT_HL)              /**/ { bus = 1;                           pc = addr + 1; /**/ alu_y = bus;                                                      addr = hl; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && INC_AT_HL)              /**/ { bus = data_in;                                    /**/ alu_x = bus; alu(0, f);          data_out = bus;                  addr = hl; write = 1; /**/                                  /**/                    update_flags(0xE0); state_ = 2; }
    if (state == 2 && INC_AT_HL)              /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && DEC_AT_HL)              /**/ {                                    pc = addr + 1; /**/                                                                   addr = hl; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && DEC_AT_HL)              /**/ { bus = data_in;                                    /**/ alu_x = bus;                                 bus = 1;             addr = hl; write = 1; /**/ alu_y = bus; alu(2, f);          /**/ data_out = bus;    update_flags(0xE0); state_ = 2; }
    ...
    if (state == 0 && MV_R_R)                 /**/ { bus = reg(OP_COL);                 pc = addr + 1; /**/ reg(OP_ROW) = bus;                                                addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && INC_R)                  /**/ { bus = reg(OP_ROW);                 pc = addr + 1; /**/ alu_x = bus;                             bus = 1;                 addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ reg(OP_ROW) = bus; update_flags(0xE0); state_ = 0; }
    if (state == 0 && DEC_R)                  /**/ { bus = reg(OP_ROW);                 pc = addr + 1; /**/ alu_x = bus;                             bus = 1;                 addr = pc; write = 0; /**/ alu_y = bus; alu(2, f);          /**/ reg(OP_ROW) = bus; update_flags(0xE0); state_ = 0; }
    if (state == 0 && ALU_A_R)                /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                             bus = reg(OP_COL);       addr = pc; write = 0; /**/ alu_y = bus; alu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RLC_A)                  /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xB0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x60); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x70); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { bus = a;                           pc = addr + 1; /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x70); state_ = 0; }

    if (state == 0 && STM_HL_R)               /**/ { bus = reg(OP_COL);                 pc = addr + 1; /**/ data_out = bus;                                                   addr = hl; write = 1; /**/                                  /**/                                        state_ = 1; }
    ...
    if (state == 1 && POP_BC)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ c = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && POP_DE)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ e = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && POP_HL)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ l = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && POP_AF)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ f = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 2; }
    ...
    if (state == 0 && STM_BC_A )              /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = bc; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 0 && STM_DE_A )              /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = de; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 0 && STM_C_A)                /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = bc; write = 1; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_C_A)                /**/ {                                                   /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 0 && ADD_HL_BC)              /**/ { bus = c;                           pc = addr + 1; /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_DE)              /**/ { bus = e;                           pc = addr + 1; /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_HL)              /**/ { bus = l;                           pc = addr + 1; /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_SP)              /**/ { bus = p;                           pc = addr + 1; /**/ alu_x = bus; bus = l;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 1 && ADD_HL_BC)              /**/ { bus = b;                                          /**/ alu_x = bus; bus = h;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(1, alu_f);      /**/ h = bus; update_flags(0x70);           state_ = 0; }
    if (state == 1 && ADD_HL_DE)              /**/ { bus = d;                                          /**/ alu_x = bus; bus = h;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(1, alu_f);      /**/ h = bus; update_flags(0x70);           state_ = 0; }
    if (state == 1 && ADD_HL_HL)              /**/ { bus = h;                                          /**/ alu_x = bus; bus = h;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(1, alu_f);      /**/ h = bus; update_flags(0x70);           state_ = 0; }
    if (state == 1 && ADD_HL_SP)              /**/ { bus = s;                                          /**/ alu_x = bus; bus = h;                                             addr = pc; write = 0; /**/ alu_y = bus; alu(1, alu_f);      /**/ h = bus; update_flags(0x70);           state_ = 0; }

    if (state == 1 && LD_BC_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ c = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && LD_DE_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ e = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && LD_HL_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ l = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && LD_SP_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ p = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && JP_CC_A16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && CALL_CC_A16)            /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && CALL_A16)               /**/ { bus = data_in;                     pc = addr + 1; /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }



    if (state == 1 && RET)                    /**/ { bus = data_in;                     sp = addr + 1; /**/ pcl = bus;                                                        addr = sp; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && RETI)                   /**/ { bus = data_in;                     sp = addr + 1; /**/ pcl = bus;                                                        addr = sp; write = 0; /**/                                  /**/                                        state_ = 2; }
    ...
    if (state == 1 && PUSH_BC)                /**/ { bus = b;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && PUSH_DE)                /**/ { bus = d;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && PUSH_HL)                /**/ { bus = h;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 1 && PUSH_AF)                /**/ { bus = a;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && PUSH_BC)                /**/ { bus = c;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && PUSH_DE)                /**/ { bus = e;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && PUSH_HL)                /**/ { bus = l;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && PUSH_AF)                /**/ { bus = f;                           sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 6; }

    if (state == 0 && LD_R_D8)                /**/ {                                    pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && LD_R_D8)                /**/ { bus = data_in;                     pc = addr + 1; /**/ reg(OP_ROW) = bus;                                                addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 1 && LDM_A_HLP)              /**/ { bus = data_in;                     hl = addr + 1; /**/ a = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 1 && LDM_A_HLM)              /**/ { bus = data_in;                     hl = addr - 1; /**/ a = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 0 && STM_HLP_A)              /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = hl; write = 1; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_HLP_A)              /**/ {                                    hl = addr + 1; /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && STM_HLM_A)              /**/ { bus = a;                           pc = addr + 1; /**/ data_out = bus;                                                   addr = hl; write = 1; /**/                                  /**/                                        state_ = 1; }
    ...
    if (state == 2 && RET)                    /**/ { bus = data_in;                     sp = addr + 1; /**/ pch = bus;                                                                   write = 0; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && RETI)                   /**/ { bus = data_in;                     sp = addr + 1; /**/ pch = bus;                                                                   write = 0; /**/                                  /**/                                        state_ = 6; }
    if (state == 2 && CALL_CC_A16 && branch)  /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 3; }
    if (state == 2 && CALL_A16)               /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = sp; write = 0; /**/                                  /**/                                        state_ = 3; }
    if (state == 2 && LD_BC_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ b = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && LD_DE_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ d = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && LD_HL_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ h = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && LD_SP_D16)              /**/ { bus = data_in;                     pc = addr + 1; /**/ s = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && POP_BC)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ b = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && POP_DE)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ d = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && POP_HL)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ h = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 2 && POP_AF)                 /**/ { bus = data_in;                     sp = addr + 1; /**/ a = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 2 && JP_CC_A16 && branch)    /**/ { bus = data_in;                     pc = addr + 1; /**/ x = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 5; }

    if (state == 3 && CALL_CC_A16)            /**/ { bus = pch;                         sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 4; }
    if (state == 3 && CALL_A16)               /**/ { bus = pch;                         sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 4; }
    if (state == 4 && CALL_CC_A16)            /**/ { bus = pcl;                         sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 5; }
    if (state == 4 && CALL_A16)               /**/ { bus = pcl;                         sp = addr - 1; /**/ data_out = bus;                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 5; }
