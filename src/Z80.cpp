...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && RST_NN)                 /**/ { bus = pch;            data_out = bus; sp = addr - 1; /**/                                                                   addr = sp; write = 1; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && RST_NN)                 /**/ { bus = pcl;            data_out = bus; sp = addr - 1; /**/ bus = 0x38;                                                       addr = sp; write = 1; /**/ alu_y = bus; bus = op;           /**/ alu_x = bus;                           state_ = 3; }
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { bus = data_in;           alu_x = bus; pc = addr + 1; /**/ alu(0, f);                                                        addr = pc; write = 0; /**/ p = bus; bus = sxt(data_in);     /**/                    update_flags(0xF0); state_ = 2; }
    ...
    if (state == 1 && LD_HL_SP_R8)            /**/ { bus = data_in;           alu_x = bus; pc = addr + 1; /**/ alu(0, f);                                                                   write = 0; /**/ l = bus; bus = s;                /**/ alu_y = bus;       update_flags(0xF0); state_ = 2; }
    ...
    if (state == 1 && ALU_A_D8)               /**/ { bus = data_in;           alu_y = bus; pc = addr + 1; /**/ alu(OP_ROW, f);                                                   addr = pc; write = 0; /**/ a = bus;                         /**/                    update_flags(0xF0); state_ = 0; }
    ...
    if (state == 0 && INC_R)                  /**/ { bus = reg(OP_ROW);       alu_x = bus; pc = addr + 1; /**/ bus = 1;                                                          addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ reg(OP_ROW) = bus; update_flags(0xE0); state_ = 0; }
    if (state == 0 && DEC_R)                  /**/ { bus = reg(OP_ROW);       alu_x = bus; pc = addr + 1; /**/ bus = 1;                                                          addr = pc; write = 0; /**/ alu_y = bus; alu(2, f);          /**/ reg(OP_ROW) = bus; update_flags(0xE0); state_ = 0; }
    if (state == 0 && ALU_A_R)                /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/ bus = reg(OP_COL);                                                addr = pc; write = 0; /**/ alu_y = bus; alu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RLC_A)                  /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xF0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0xB0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x60); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x70); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                                                                   addr = pc; write = 0; /**/ alu_y = bus; rlu(OP_ROW, f);     /**/ a = bus;           update_flags(0x70); state_ = 0; }
                                                                          
    if (state == 0 && STM_HL_R)               /**/ { bus = reg(OP_COL);    data_out = bus; pc = addr + 1; /**/                                                                   addr = hl; write = 1; /**/                                  /**/                                        state_ = 1; }
    ...
    if (state == 0 && STM_BC_A )              /**/ { bus = a;              data_out = bus; pc = addr + 1; /**/                                                                   addr = bc; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 0 && STM_DE_A )              /**/ { bus = a;              data_out = bus; pc = addr + 1; /**/                                                                   addr = de; write = 1; /**/                                  /**/                                        state_ = 6; }
    if (state == 0 && STM_C_A)                /**/ { bus = a;              data_out = bus; pc = addr + 1; /**/                                                                   addr = bc; write = 1; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && STM_C_A)                /**/ {                                                      /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
    if (state == 0 && ADD_HL_BC)              /**/ { bus = c;                 alu_x = bus; pc = addr + 1; /**/ bus = l;                                                          addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_DE)              /**/ { bus = e;                 alu_x = bus; pc = addr + 1; /**/ bus = l;                                                          addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_HL)              /**/ { bus = l;                 alu_x = bus; pc = addr + 1; /**/ bus = l;                                                          addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
    if (state == 0 && ADD_HL_SP)              /**/ { bus = p;                 alu_x = bus; pc = addr + 1; /**/ bus = l;                                                          addr = pc; write = 0; /**/ alu_y = bus; alu(0, f);          /**/ l = bus;                               state_ = 1; }
