...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && ADD_SP_R8)              /**/ { alu_x = in;                                      /**/                                                     set_addr(pc, 0); /**/ p = alu(0, f);                                  set_f(0xF0); state_ = 2; }
    if (state == 2 && ADD_SP_R8)              /**/ { alu_x = sxt(in);                      pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/                                                              state_ = 3; }
    if (state == 3 && ADD_SP_R8)              /**/ { alu_y = s;                                       /**/                                                     set_addr(pc, 0); /**/ s = alu(1, f);                                               state_ = 0; }

    if (state == 0 && LD_HL_SP_R8)            /**/ { alu_y = p;                            pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/                                                              state_ = 1; }
    if (state == 1 && LD_HL_SP_R8)            /**/ { alu_x = in;                           pcl = apl; /**/                                          pch = aph;                  /**/ l = alu(0, f);                                  set_f(0xF0); state_ = 2; }
    if (state == 2 && LD_HL_SP_R8)            /**/ { alu_y = s;                                       /**/                                                                      /**/                                                              state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ { alu_x = sxt(in);                                 /**/                                                     set_addr(pc, 0); /**/ h = alu(1, f);                                               state_ = 0; }
    ...
    if (state == 1 && LD_SP_D16)              /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/ p = in;                                                      state_ = 2; }
    ...
    if (state == 2 && LD_SP_D16)              /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/ s = in;                                                      state_ = 0; }
    ...
    if (state == 1 && CALL_CC_A16 &&  branch) /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/ y = in;                                                      state_ = 2; }
    if (state == 1 && CALL_CC_A16 && !branch) /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/ y = in;                                                      state_ = 2; }
    if (state == 1 && CALL_A16)               /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/ y = in;                                                      state_ = 2; }
    if (state == 2 && CALL_CC_A16 &&  branch) /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(sp, 0); /**/ x = in;                                                      state_ = 3; }
    if (state == 2 && CALL_CC_A16 && !branch) /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(pc, 0); /**/                                                              state_ = 0; }
    if (state == 2 && CALL_A16)               /**/ {                                       pcl = apl; /**/                                          pch = aph; set_addr(sp, 0); /**/ x = in;                                                      state_ = 3; }
