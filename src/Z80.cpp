...
void Z80::tock_b(const uint8_t imask_, const uint8_t intf_, const Ack& ack) {
  ...
  if (INT) {
    ...
    if (state == 0 && CALL_CC_A16 && !branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && CALL_CC_A16 && !branch) /**/ { y = in;           pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 2; }
    ...
    if (state == 1 && RETI)                   /**/ { pcl = in;         spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/                                                  set_addr(sp, 0); state_ = 2; }
    if (state == 2 && RETI)                   /**/ { pch = in;         spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/                                                                   state_ = 3; }
