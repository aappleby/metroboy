...
#define PASS(A)     { addr = A; bus_out = 0; write = 0; read = 0; }
    ...
    if (DEC_R             && state == 0) { ao = alu(2, r, 1, 0);                           reg_put8(ao.x);                         set_flag(ao.f);                                 pc = addr + 1;       READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                             
    if (INC_R             && state == 0) { ao = alu(1, r, 1, 0);                           reg_put8(ao.x);                         set_flag(ao.f);                                 pc = addr + 1;       READ(pc);       state_ = 0; break; }
    ...
    if (ADD_HL_BC         && state == 0) { ao = alu(1, l, c, 0);                           l = ao.x;                               set_flag(ao.f);                                 pc = addr + 1;       READ(pc);       state_ = 1; break; }
    if (ADD_HL_BC         && state == 1) { ao = alu(1, h, b, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                             
    if (ADD_HL_DE         && state == 0) { ao = alu(1, l, e, 0);                           l = ao.x;                               set_flag(ao.f);                                 pc = addr + 1;       READ(pc);       state_ = 1; break; }
    if (ADD_HL_DE         && state == 1) { ao = alu(1, h, d, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                             
    if (ADD_HL_HL         && state == 0) { ao = alu(1, l, l, 0);                           l = ao.x;                               set_flag(ao.f);                                 pc = addr + 1;       READ(pc);       state_ = 1; break; }
    if (ADD_HL_HL         && state == 1) { ao = alu(1, h, h, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                             
    if (ADD_HL_SP         && state == 0) { ao = alu(1, l, p, 0);                           l = ao.x;                               set_flag(ao.f);                                 pc = addr + 1;       READ(pc);       state_ = 1; break; }
