...
void Z80::tock_b(const uint8_t imask_, const uint8_t intf_, const Ack& ack) {
  ...
  if (INT) {
    ...
    if (state == 0 && HALT)                   /**/ {                      pcl = adl = inc(pcl, 1);  /**/                              pch = adh = inc(pch, inc_c); set_addr(ad, 0); /**/                                                  state_ = !no_halt; }
    if (state == 1 && HALT)                   /**/ {                                                /**/                                                           set_addr(ad, 0); /**/                                                  state_ = !unhalt; }
  }                                                                                                                                                                                                                             
  else if (PREFIX_CB) {                                                                                                                                                                                                         
    if (state == 1) cb = in;                                                                                                                                                                                                    
    uint8_t mask = cb_flag_mask[CB_QUAD];                                                                                                                                                                                       

    if (state == 0)                           /**/ {                      pcl = adl = inc(pcl, 1);  /**/                              pch = adh = inc(pch, inc_c); set_addr(ad, 0); /**/                                                              state_ = 1; }
    if (state == 1 && OP_CB_R)                /**/ { alu_x = reg(CB_COL); pcl = adl = inc(pcl, 1);  /**/ reg(CB_COL) = alu_cb(cb, f); pch = adh = inc(pch, inc_c); set_addr(ad, 0); /**/                                                 set_f(mask); state_ = 0; }
    if (state == 1 && OP_CB_HL)               /**/ {                            adl = l;            /**/                                    adh = h;               set_addr(ad, 0); /**/                                                              state_ = 2; }
    if (state == 2)                           /**/ { alu_x = in;                adl = l;            /**/ out = alu_cb(cb, f);               adh = h;               set_addr(ad, 1); /**/                                                 set_f(mask); state_ = 3; }
    if (state == 3)                           /**/ {                      pcl = adl = inc(pcl, 1);  /**/                              pch = adh = inc(pch, inc_c); set_addr(ad, 0); /**/                                                              state_ = 0; }
  }                                                                                                                                                                                                    
  else {                                                                                                                                                                                               
    bool branch = false;                                                                                                                                                                               
    switch (OP_ROW & 3) {                                                                                                                                                                              
    ...
    if (state == 1 && LD_SP_HL)               /**/ {                   spl = l;                  /**/                  sph = h;                     /**/                                                  set_addr(ad, 0); state_ = 0; }
    ...
    if (state == 0 && ALU_A_R)                /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/ alu_y = R_COL;   pch = adh = inc(pch, inc_c); /**/ a = alu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && INC_R)                  /**/ { alu_x = R_ROW;    pcl = adl = inc(pcl, 1);  /**/ alu_y = 1;       pch = adh = inc(pch, inc_c); /**/ R_ROW = alu(0, f);                  set_f(0xE0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && DEC_R)                  /**/ { alu_x = R_ROW;    pcl = adl = inc(pcl, 1);  /**/ alu_y = 1;       pch = adh = inc(pch, inc_c); /**/ R_ROW = alu(2, f);                  set_f(0xE0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && RLC_A)                  /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0xB0); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0x60); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0x70); set_addr(ad, 0); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { alu_x = a;        pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ a = rlu(OP_ROW, f);                 set_f(0x70); set_addr(ad, 0); state_ = 0; }

    if (state == 0 && ALU_A_D8)               /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(ad, 0); state_ = 1; }
    if (state == 1 && ALU_A_D8)               /**/ { alu_y = in;       pcl = adl = inc(pcl, 1);  /**/ alu_x = a;       pch = adh = inc(pch, inc_c); /**/ a = alu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }

    if (state == 0 && ALU_A_HL)               /**/ {                   adl = l;                  /**/                  adh = h;                     /**/                                                  set_addr(ad, 0); state_ = 1; }
    if (state == 1 && ALU_A_HL)               /**/ { alu_y = in;       pcl = adl = inc(pcl, 1);  /**/ alu_x = a;       pch = adh = inc(pch, inc_c); /**/ a = alu(OP_ROW, f);                 set_f(0xF0); set_addr(ad, 0); state_ = 0; }


    if (state == 0 && INC_AT_HL)              /**/ {                   y = l;                    /**/                  x = h;                       /**/ alu_y = 1;                                       set_addr(xy, 0); state_ = 1; }
    if (state == 0 && DEC_AT_HL)              /**/ {                   y = l;                    /**/                  x = h;                       /**/ alu_y = 1;                                       set_addr(xy, 0); state_ = 1; }
    if (state == 1 && INC_AT_HL)              /**/ { alu_x = in;                                 /**/ out = alu(0, f);                              /**/                                     set_f(0xE0); set_addr(xy, 1); state_ = 2; }
    if (state == 1 && DEC_AT_HL)              /**/ { alu_x = in;                                 /**/ out = alu(2, f);                              /**/                                     set_f(0xE0); set_addr(xy, 1); state_ = 2; }
    if (state == 2 && INC_AT_HL)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 2 && DEC_AT_HL)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    // 16-bit alu                                                                                                      

    if (state == 0 && ADD_SP_R8)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    ...
    if (state == 3 && ADD_SP_R8)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    if (state == 0 && LD_HL_SP_R8)            /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && LD_HL_SP_R8)            /**/ { alu_x = in;                                 /**/ alu_y = spl;                                  /**/ l = alu(0, f);                      set_f(0xF0); set_addr(pc, 0); state_ = 2; }
    if (state == 2 && LD_HL_SP_R8)            /**/ { alu_x = sxt(in);                            /**/ alu_y = sph;                                  /**/ h = alu(1, f);                                   set_addr(pc, 0); state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    if (state == 0 && INC_BC)                 /**/ {                           c = inc(  c, 1);  /**/                          b = inc(  b, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 0 && DEC_BC)                 /**/ {                           c = dec(  c, 1);  /**/                          b = dec(  b, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 0 && INC_DE)                 /**/ {                           e = inc(  e, 1);  /**/                          d = inc(  d, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 0 && DEC_DE)                 /**/ {                           e = dec(  e, 1);  /**/                          d = dec(  d, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 0 && INC_HL)                 /**/ {                           l = inc(  l, 1);  /**/                          h = inc(  h, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 0 && DEC_HL)                 /**/ {                           l = dec(  l, 1);  /**/                          h = dec(  h, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    ...
    if (state == 1 && INC_BC)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && DEC_BC)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && INC_DE)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && DEC_DE)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && INC_HL)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && DEC_HL)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && INC_SP)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 1 && DEC_SP)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    // load/store                                                                                                      

    if (state == 0 && STM_A16_SP)             /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && STM_A16_SP)             /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                        y = in;                   set_addr(pc, 0); state_ = 2; }
    ...
    if (state == 4 && STM_A16_SP)             /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    if (state == 0 && STM_A16_A)              /**/ { out = a;          pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && STM_A16_A)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                        y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 2 && STM_A16_A)              /**/ {                                             /**/                                               /**/                        x = in;                   set_addr(xy, 1); state_ = 3; }
    if (state == 3 && STM_A16_A)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    if (state == 0 && LDM_A_A16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && LDM_A_A16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                        y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 2 && LDM_A_A16)              /**/ {                                             /**/                                               /**/                        x = in;                   set_addr(xy, 0); state_ = 3; }
    if (state == 3 && LDM_A_A16)              /**/ { a = in;           pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    // indirect load/store

    if (state == 0 && LDM_A_BC)               /**/ {                   y = c;                    /**/                  x = b;                       /**/                                                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && LDM_A_BC)               /**/ { a = in;           pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    if (state == 0 && LDM_A_DE)               /**/ {                   y = e;                    /**/                  x = d;                       /**/                                                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && LDM_A_DE)               /**/ { a = in;           pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }

    if (state == 0 && STM_BC_A)               /**/ { DBUS_BUSY;        y = c;                    /**/ DBUS_BUSY;       x = b;                       /**/ out = a;                                         set_addr(xy, 1); state_ = 1; }
    if (state == 1 && STM_BC_A)               /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 0 && STM_HL_R)               /**/ { DBUS_BUSY;        y = l;                    /**/ DBUS_BUSY;       x = h;                       /**/ out = reg(OP_COL);                               set_addr(xy, 1); state_ = 1; }
    if (state == 1 && STM_HL_R)               /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 2 && LDM_A_A8)               /**/ { a = in;           pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 0 && PUSH_BC)                /**/ {                         adl = spl;          /**/                        adh = sph;             /**/                                                  set_addr(sp, 0); state_ = 1; }
    if (state == 0 && PUSH_DE)                /**/ {                         adl = spl;          /**/                        adh = sph;             /**/                                                  set_addr(sp, 0); state_ = 1; }
    if (state == 0 && PUSH_HL)                /**/ {                         adl = spl;          /**/                        adh = sph;             /**/                                                  set_addr(sp, 0); state_ = 1; }
    if (state == 0 && PUSH_AF)                /**/ {                         adl = spl;          /**/                        adh = sph;             /**/                                                  set_addr(sp, 0); state_ = 1; }
    ...
    if (state == 3 && PUSH_BC)                /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 3 && PUSH_DE)                /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 3 && PUSH_HL)                /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    if (state == 3 && PUSH_AF)                /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 1 && LD_BC_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ c = in;                                          set_addr(pc, 0); state_ = 2; }
    if (state == 1 && LD_DE_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ e = in;                                          set_addr(pc, 0); state_ = 2; }
    if (state == 1 && LD_HL_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ l = in;                                          set_addr(pc, 0); state_ = 2; }
    if (state == 2 && LD_BC_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ b = in;                                          set_addr(pc, 0); state_ = 0; }
    if (state == 2 && LD_DE_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ d = in;                                          set_addr(pc, 0); state_ = 0; }
    if (state == 2 && LD_HL_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ h = in;                                          set_addr(pc, 0); state_ = 0; }

    if (state == 0 && LD_SP_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && LD_SP_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ spl = in;                                        set_addr(pc, 0); state_ = 2; }
    if (state == 2 && LD_SP_D16)              /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ sph = in;                                        set_addr(pc, 0); state_ = 0; }

    if (state == 0 && ADD_HL_BC)              /**/ { alu_x = c;        pcl = adl = inc(pcl, 1);  /**/ alu_y = l;       pch = adh = inc(pch, inc_c); /**/ l = alu(0, f);                                   set_addr(pc, 0); state_ = 1; }
    if (state == 0 && ADD_HL_DE)              /**/ { alu_x = e;        pcl = adl = inc(pcl, 1);  /**/ alu_y = l;       pch = adh = inc(pch, inc_c); /**/ l = alu(0, f);                                   set_addr(pc, 0); state_ = 1; }
    if (state == 0 && ADD_HL_HL)              /**/ { alu_x = l;        pcl = adl = inc(pcl, 1);  /**/ alu_y = l;       pch = adh = inc(pch, inc_c); /**/ l = alu(0, f);                                   set_addr(pc, 0); state_ = 1; }
    if (state == 0 && ADD_HL_SP)              /**/ { alu_x = spl;      pcl = adl = inc(pcl, 1);  /**/ alu_y = l;       pch = adh = inc(pch, inc_c); /**/ l = alu(0, f);                                   set_addr(pc, 0); state_ = 1; }
    ...
    if (state == 0 && JP_CC_A16   && !branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && JP_CC_A16   &&  branch) /**/ { y = in;           pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 2; }
    if (state == 1 && JP_CC_A16   && !branch) /**/ { y = in;           pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 2; }
    if (state == 2 && JP_CC_A16   &&  branch) /**/ { x = in;           pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 3; }
    if (state == 2 && JP_CC_A16   && !branch) /**/ { x = in;           pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 1 && JP_A16)                 /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 2 && JP_A16)                 /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             x = in;                   set_addr(pc, 0); state_ = 3; }
    ...
    if (state == 1 && CALL_CC_A16 &&  branch) /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ DBUS_BUSY;             y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 2 && CALL_CC_A16 &&  branch) /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/ DBUS_BUSY;             x = in;                   set_addr(sp, 0); state_ = 3; }
    if (state == 3 && CALL_CC_A16 &&  branch) /**/ {                   spl = adl = dec(spl, 1);  /**/                  sph = adh = dec(sph, inc_c); /**/ DBUS_BUSY;             out = pch;                set_addr(sp, 1); state_ = 4; }
    if (state == 4 && CALL_CC_A16 &&  branch) /**/ {                   spl = adl = dec(spl, 1);  /**/                  sph = adh = dec(sph, inc_c); /**/ DBUS_BUSY;             out = pcl;                set_addr(sp, 1); state_ = 5; }
    ...
    if (state == 2 && CALL_CC_A16 && !branch) /**/ {                   pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 2 && CALL_A16)               /**/ { x = in;           pcl = adl = inc(pcl, 1);  /**/                  pch = adh = inc(pch, inc_c); /**/                                                  set_addr(sp, 0); state_ = 3; }
    if (state == 3 && CALL_A16)               /**/ {                   spl = adl = dec(spl, 1);  /**/                  sph = adh = dec(sph, inc_c); /**/ out = pch;                                       set_addr(sp, 1); state_ = 4; }
    if (state == 4 && CALL_A16)               /**/ {                   spl = adl = dec(spl, 1);  /**/                  sph = adh = dec(sph, inc_c); /**/ out = pcl;                                       set_addr(sp, 1); state_ = 5; }
    ...
    if (state == 4 && RET_CC      &&  branch) /**/ { x = in;           spl = adl = inc(spl, 1);  /**/                  sph = adh = inc(sph, inc_c); /**/                                                  set_addr(xy, 0); state_ = 0; }
    ...
    if (state == 3 && RET)                    /**/ {                   spl = adl = inc(spl, 1);  /**/                  sph = adh = inc(sph, inc_c); /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 2 && RST_NN)                 /**/ {                   spl = adl = dec(spl, 1);  /**/ out = pcl;       sph = adh = dec(sph, inc_c); /**/ x = 0x00;                                        set_addr(sp, 1); state_ = 3; }
