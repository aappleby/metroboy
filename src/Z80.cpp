...
#define PASS(A)     { addr = A; bus_out = 0; write = 0; read = 0; }
    ...
    if (INC_AT_HL         && state == WRITE1) {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (DEC_AT_HL         && state == WRITE1) {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (ADD_HL_BC         && state == ALU2)   { ao = alu(1, h, b, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (ADD_HL_DE         && state == ALU2)   { ao = alu(1, h, d, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (ADD_HL_HL         && state == ALU2)   { ao = alu(1, h, h, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (ADD_HL_SP         && state == ALU2)   { ao = alu(1, h, s, f & F_CARRY);                 h = ao.x;                               set_flag(ao.f);                                                      addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (ADD_SP_R8         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ALU1;   break; }
    if (ADD_SP_R8         && state == ALU1)   { ao = alu(1, p, bus_in, 0);                      y = ao.x;                               set_flag(ao.f);                                 pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 0;     state_ = ALU2;   break; }
    ...
    if (ALU_A_HL          && state == ALU1)   { ao = alu(OP_ROW, a, bus_in, f & F_CARRY);       a = ao.x;                               set_flag(ao.f);                                                      addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (CALL_CC_A16 && tb && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (CALL_CC_A16 && tb && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    ...
    if (CALL_CC_A16 && tb && state == PUSH2)  {                                                                                                                                         pc = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (CALL_CC_A16 && nb && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (CALL_CC_A16 && nb && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (CALL_CC_A16 && nb && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (RST_NN            && state == PUSH2)  {                                                 y = op & 0x38;                                                                                               addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (HALT && !unhalt   && state == HALT1)  {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = HALT1;  break; }
    if (HALT && unhalt    && state == HALT1)  {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (JP_A16            && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (JP_A16            && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    if (JP_A16            && state == PTR1)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (JP_CC_A16 && tb   && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (JP_CC_A16 && tb   && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (JP_CC_A16 && tb   && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = PTR1;   break; }
    if (JP_CC_A16 && tb   && state == PTR1)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (JP_CC_A16 && nb   && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (JP_CC_A16 && nb   && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (JP_CC_A16 && nb   && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    //----------

    if (JR_CC_R8 && tb    && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ALU1;   break; }
    if (JR_CC_R8 && tb    && state == ALU1)   { ao = alu(1, pcl, bus_in, 1);                    y = ao.x;                                                                                                    addr = pc;               bus_out = 0;              write = 0; read = 0;     state_ = ALU2;   break; }
    if (JR_CC_R8 && tb    && state == ALU2)   { ao = alu(1, pch, sxt(bus_in), ao.f & F_CARRY);  x = ao.x;                                                                                                    addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (JR_CC_R8 && nb    && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ALU1;   break; }
    ...
    if (JR_R8             && state == ALU1)   { ao = alu(1, pcl, bus_in, 1);                    y = ao.x;                                                                                                    addr = pc;               bus_out = 0;              write = 0; read = 0;     state_ = ALU2;   break; }
    if (JR_R8             && state == ALU2)   { ao = alu(1, pch, sxt(bus_in), ao.f & F_CARRY);  x = ao.x;                                                                                                    addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    //----------

    if (LD_BC_D16         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    ...
    if (LD_DE_D16         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    ...
    if (LD_HL_D16         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    ...
    if (LD_SP_D16         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    ...
    if (LD_HL_SP_R8       && state == ALU2)   { ao = alu(1, s, sxt(bus_in), f & F_CARRY);       h = ao.x;                                                                                                    addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (LDM_A_A16         && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (LDM_A_A16         && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = READ1;  break; }
    if (LDM_A_A16         && state == READ1)  {                                                 a = bus_in;                                                                                                  addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (LDM_A_A8          && state == DECODE) {                                                 x = 0xFF;                                                                               pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (LDM_A_A8          && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = READ1;  break; }
    if (LDM_A_A8          && state == READ1)  {                                                 a = bus_in;                                                                                                  addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (LDM_A_C           && state == DECODE) {                                                 y = c; x = 0xFF;                                                                        pc = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = READ1;  break; }
    if (LDM_A_C           && state == READ1)  {                                                 a = bus_in;                                                                                                  addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (POP_BC            && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    ...
    if (POP_DE            && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    ...
    if (POP_HL            && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    ...
    if (POP_AF            && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    ...
    if (PUSH_BC           && state == PUSH2)  {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (PUSH_DE           && state == PUSH2)  {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (PUSH_HL           && state == PUSH2)  {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (PUSH_AF           && state == PUSH2)  {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (RET               && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    if (RET               && state == POP1)   {                                                 y = bus_in;                                                                             sp = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP2;   break; }
    if (RET               && state == POP2)   {                                                 x = bus_in;                                                                             sp = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    if (RET               && state == PTR1)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (RET_CC && tb      && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = POP0;   break; }
    if (RET_CC && tb      && state == POP0)   {                                                                                                                                         pc = addr;           addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    if (RET_CC && tb      && state == POP1)   {                                                 y = bus_in;                                                                             sp = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP2;   break; }
    if (RET_CC && tb      && state == POP2)   {                                                 x = bus_in;                                                                             sp = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    if (RET_CC && tb      && state == PTR1)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (RET_CC && nb      && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = POP0;   break; }
    if (RET_CC && nb      && state == POP0)   {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (RETI              && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP1;   break; }
    if (RETI              && state == POP1)   {                                                 y = bus_in;                                                                             sp = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = POP2;   break; }
    if (RETI              && state == POP2)   {                                                 x = bus_in;                                                                             sp = addr + 1;       addr = xy;               bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    if (RETI              && state == PTR1)   {                                                                                                                                                              addr = xy;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (STM_A16_A         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (STM_A16_A         && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (STM_A16_A         && state == ARG2)   {                                                 x = bus_in;                                                                             pc = addr + 1;       addr = xy;               bus_out = a;              write = 1; read = 0;     state_ = WRITE1; break; }
    if (STM_A16_A         && state == WRITE1) {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }

    if (STM_A8_A          && state == DECODE) {                                                 x = 0xFF;                                                                               pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    ...
    if (STM_A16_SP        && state == ARG1)   {                                                 y = bus_in;                                                                             pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    ...
    if (STM_A16_SP        && state == WRITE2) {                                                                                                                                                              addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
