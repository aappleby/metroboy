...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 2 && RST_NN)                 /**/ {                                       sp = addr - 1; /**/ bus = pcl;               data_out = bus; addr = sp; write = 1;                /**/                                                              state_ = 3; }
    ...
    if (state == 0 && RLC_A)                  /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0xF0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0xB0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0x60); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0x70); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { bus = a;                 alu_x = bus; pc = addr + 1; /**/                             alu_y = bus; addr = pc; write = 0;                /**/ bus = rlu(OP_ROW, f);           a = bus; update_flags(0x70); state_ = 0; }
