...
void Z80::tock_b(const uint8_t imask_, const uint8_t intf_, const Ack& ack) {
  ...
  if (INT) {
    ...
    if (state == 1 && OP_CB_R)                /**/ { alu_x = R_CB;     pcl = inc(pcl, 1);        /**/ R_CB = alu_cb(cb, f); pch = inc(pch, inc_c);  /**/        set_f(mask);                              set_addr(pc, 0); state_ = 0; }
    if (state == 1 && OP_CB_HL)               /**/ {                   y = l;                    /**/                  x = h;                       /**/                                                  set_addr(xy, 0); state_ = 2; }
    if (state == 2)                           /**/ { alu_x = in;       y = l;                    /**/ out = alu_cb(cb, f);                 x = h;   /**/        set_f(mask);                              set_addr(xy, 1); state_ = 3; }
    ...
    if (state == 0 && ALU_A_R)                /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/ alu_y = R_COL;   pch = inc(pch, inc_c);       /**/ a = alu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && INC_R)                  /**/ { alu_x = R_ROW;    pcl = inc(pcl, 1);        /**/ alu_y = 1;       pch = inc(pch, inc_c);       /**/ R_ROW = alu(0, f);                  set_f(0xE0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && DEC_R)                  /**/ { alu_x = R_ROW;    pcl = inc(pcl, 1);        /**/ alu_y = 1;       pch = inc(pch, inc_c);       /**/ R_ROW = alu(2, f);                  set_f(0xE0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && RLC_A)                  /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && RRC_A)                  /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && RL_A)                   /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && RR_A)                   /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && DAA)                    /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0xB0); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && CPL)                    /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0x60); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && SCF)                    /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0x70); set_addr(pc, 0); state_ = 0; }
    if (state == 0 && CCF)                    /**/ { alu_x = a;        pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ a = rlu(OP_ROW, f);                 set_f(0x70); set_addr(pc, 0); state_ = 0; }

    if (state == 0 && ALU_A_D8)               /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && ALU_A_D8)               /**/ { alu_y = in;       pcl = inc(pcl, 1);        /**/ alu_x = a;       pch = inc(pch, inc_c);       /**/ a = alu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }

    if (state == 0 && ALU_A_HL)               /**/ { DBUS_BUSY;        y = l;                    /**/ DBUS_BUSY;       x = h;                       /**/                                                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && ALU_A_HL)               /**/ { alu_y = in;       pcl = inc(pcl, 1);        /**/ alu_x = a;       pch = inc(pch, inc_c);       /**/ a = alu(OP_ROW, f);                 set_f(0xF0); set_addr(pc, 0); state_ = 0; }


    if (state == 0 && INC_AT_HL)              /**/ { DBUS_BUSY;        y = l;                    /**/ DBUS_BUSY;       x = h;                       /**/                                                  set_addr(xy, 0); state_ = 1; }
    if (state == 0 && DEC_AT_HL)              /**/ { DBUS_BUSY;        y = l;                    /**/ DBUS_BUSY;       x = h;                       /**/                                                  set_addr(xy, 0); state_ = 1; }
    if (state == 1 && INC_AT_HL)              /**/ { alu_x = in;                                 /**/ alu_y = 1;                                    /**/ out = alu(0, f);                    set_f(0xE0); set_addr(xy, 1); state_ = 2; }
    if (state == 1 && DEC_AT_HL)              /**/ { alu_x = in;                                 /**/ alu_y = 1;                                    /**/ out = alu(2, f);                    set_f(0xE0); set_addr(xy, 1); state_ = 2; }
    ...
    if (state == 1 && STM_A16_A)              /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                        y = in;                   set_addr(pc, 0); state_ = 2; }
    ...
    if (state == 1 && LDM_A_A16)              /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                        y = in;                   set_addr(pc, 0); state_ = 2; }
    ...
    if (state == 0 && LDM_R_HL)               /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(hl, 0); state_ = 1; }
    ...
    if (state == 1 && STM_HL_D8)              /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(hl, 1); state_ = 2; }
    ...
    if (state == 1 && JR_R8)                  /**/ { alu_y = in;                                 /**/ DBUS_BUSY;       pcl = alu(0, f);             /**/ alu_x = x;                                       set_addr(xy, 0); state_ = 2; }                                                         
    if (state == 2 && JR_R8)                  /**/ { alu_y = sxt(in);                            /**/ DBUS_BUSY;       pch = alu(1, alu_f);         /**/                                                  set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 0 && JP_CC_A16   && !branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(pc, 0); state_ = 1; }
    if (state == 1 && JP_CC_A16   &&  branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 1 && JP_CC_A16   && !branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 2 && JP_CC_A16   &&  branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             x = in;                   set_addr(pc, 0); state_ = 3; }
    if (state == 2 && JP_CC_A16   && !branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             x = in;                   set_addr(pc, 0); state_ = 0; }
    ...
    if (state == 1 && CALL_CC_A16 &&  branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             y = in;                   set_addr(pc, 0); state_ = 2; }
    if (state == 2 && CALL_CC_A16 &&  branch) /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ DBUS_BUSY;             x = in;                   set_addr(sp, 0); state_ = 3; }
    if (state == 3 && CALL_CC_A16 &&  branch) /**/ {                   spl = dec(spl, 1);        /**/                  sph = dec(sph, inc_c);       /**/ DBUS_BUSY;             out = pch;                set_addr(sp, 1); state_ = 4; }
    if (state == 4 && CALL_CC_A16 &&  branch) /**/ {                   spl = dec(spl, 1);        /**/                  sph = dec(sph, inc_c);       /**/ DBUS_BUSY;             out = pcl;                set_addr(sp, 1); state_ = 5; }
    ...
    if (state == 1 && CALL_A16)               /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ y = in;                                          set_addr(pc, 0); state_ = 2; }
    if (state == 2 && CALL_A16)               /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/ x = in;                                          set_addr(sp, 0); state_ = 3; }
    if (state == 3 && CALL_A16)               /**/ {                   spl = dec(spl, 1);        /**/                  sph = dec(sph, inc_c);       /**/ out = pch;                                       set_addr(sp, 1); state_ = 4; }
    if (state == 4 && CALL_A16)               /**/ {                   spl = dec(spl, 1);        /**/                  sph = dec(sph, inc_c);       /**/ out = pcl;                                       set_addr(sp, 1); state_ = 5; }
    ...
    if (state == 3 && RET_CC      &&  branch) /**/ {                   spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/ pcl = in;                                        set_addr(sp, 0); state_ = 4; }
    if (state == 4 && RET_CC      &&  branch) /**/ {                   spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/ pch = in;                                        set_addr(pc, 0); state_ = 0; }
                                                                                                                                                    
    if (state == 0 && RETI)                   /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(sp, 0); state_ = 1; }
    if (state == 1 && RETI)                   /**/ {                   spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/ DBUS_BUSY;             pcl = in;                 set_addr(sp, 0); state_ = 2; }
    if (state == 2 && RETI)                   /**/ {                   spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/ DBUS_BUSY;             pch = in;                 set_addr(sp, 0); state_ = 3; }
    if (state == 3 && RETI)                   /**/ {                                             /**/                                               /**/                                                  set_addr(pc, 0); state_ = 0; }
                                                                                                                                                    
    if (state == 0 && RET)                    /**/ {                   pcl = inc(pcl, 1);        /**/                  pch = inc(pch, inc_c);       /**/                                                  set_addr(sp, 0); state_ = 1; }
    if (state == 1 && RET)                    /**/ {                   spl = inc(spl, 1);        /**/                  sph = inc(sph, inc_c);       /**/ DBUS_BUSY;             pcl = in;                 set_addr(sp, 0); state_ = 2; }
    ...
    if (state == 1 && RST_NN)                 /**/ {                   spl = dec(spl, 1);        /**/                  sph = dec(sph, inc_c);       /**/ DBUS_BUSY;             out = pch;                set_addr(sp, 1); state_ = 2; }
    if (state == 2 && RST_NN)                 /**/ {                   spl = dec(spl, 1);        /**/                  sph = dec(sph, inc_c);       /**/ DBUS_BUSY;             out = pcl;                set_addr(sp, 1); state_ = 3; }
    if (state == 3 && RST_NN)                 /**/ { alu_x = op;                                 /**/ DBUS_BUSY;       pcl = alu(4, f);             /**/ DBUS_BUSY;             pch = 0x00;               set_addr(pc, 0); state_ = 0; }
  }
