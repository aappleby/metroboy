...
void Z80::tock(const int tcycle_, const uint8_t imask_, const uint8_t intf_) {
  ...
  do {
    ...
    if (                     state == INT4)   {                                                                                                                                         pc = temp;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (CALL_A16          && state == PUSH2)  {                                                                                                                                         pc = temp;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (JP_HL             && state == DECODE) {                                                                                                                                         pc = hl;             addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (ADD_SP_R8         && state == ALU1)   { ao = alu(1, p, bus_in, 0);                      lo = ao.x;                              set_flag(ao.f);                                 pc = addr;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ALU2;   break; }
    if (ADD_SP_R8         && state == ALU2)   { ao = alu(1, s, sxt(bus_in), f & F_CARRY);       hi = ao.x;                                                                              pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DELAY;  break; }
    if (ADD_SP_R8         && state == DELAY)  {                                                                                                                                         sp = temp;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (CALL_CC_A16 && nb && state == ARG2)   {                                                 hi = bus_in;                                                                            pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    if (CALL_CC_A16 && tb && state == ARG2)   {                                                 hi = bus_in;                                                                            pc = addr + 1;       addr = sp;               bus_out = 0;              write = 0; read = 1;     state_ = PUSH0;  break; }
    ...
    if (CALL_CC_A16       && state == PUSH2)  {                                                                                                                                         pc = temp;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
