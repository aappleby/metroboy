...
void Z80::tock(const int tcycle_, const uint8_t imask_, const uint8_t intf_) {
  ...
  do {
    ...
    if (RST_NN            && state == PUSH2)  {                                                                                                                                         pc = addr;           addr = op - 0xC7;        bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (JP_A16            && state == ARG2)   {                                                 hi = bus_in;                                                                            pc = addr + 1;       addr = temp;             bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    if (JP_A16            && state == PTR1)   {                                                                                                                                         pc = addr;           addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (JP_CC_A16         && state == ARG2)   {                                                 hi = bus_in;                                                                            pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = PTR1;   break; }
    if (JP_CC_A16         && state == PTR1)   {                                                                                                                                         pc = addr;           addr = temp;             bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }

    if (nb) {
    if (JP_CC_A16         && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG1;   break; }
    if (JP_CC_A16         && state == ARG1)   {                                                 lo = bus_in;                                                                            pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ARG2;   break; }
    if (JP_CC_A16         && state == ARG2)   {                                                 hi = bus_in;                                                                            pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }

    if (tb) {
    if (JR_CC_R8          && state == DECODE) {                                                                                                                                         pc = addr + 1;       addr = pc;               bus_out = 0;              write = 0; read = 1;     state_ = ALU1;   break; }
    if (JR_CC_R8          && state == ALU1)   { ao = alu(1, pcl, bus_in, 1);                    lo = ao.x;                                                                                                   addr = pc;               bus_out = 0;              write = 0; read = 0;     state_ = ALU2;   break; }
    if (JR_CC_R8          && state == ALU2)   { ao = alu(1, pch, sxt(bus_in), ao.f & F_CARRY);  hi = ao.x;                                                                              pc = addr;           addr = temp;             bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    }
    ...
    if (JR_R8             && state == ALU2)   { ao = alu(1, pch, sxt(bus_in), ao.f & F_CARRY);  hi = ao.x;                                                                              pc = addr;           addr = temp;             bus_out = 0;              write = 0; read = 1;     state_ = DECODE; break; }
    ...
    if (RET               && state == POP2)   {                                                 hi = bus_in;                                                                            sp = addr + 1;       addr = temp;             bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    ...
    if (RET_CC            && state == POP2)   {                                                 hi = bus_in;                                                                            sp = addr + 1;       addr = temp;             bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
    ...
    if (RETI              && state == POP2)   {                                                 hi = bus_in;                                                                            sp = addr + 1;       addr = temp;             bus_out = 0;              write = 0; read = 0;     state_ = PTR1;   break; }
