...
#define PASS(A)     { addr = A;           write = 0; read = 0; }
    ...
    // CB
      ...
      if (state == 0) {
        if (PREFIX_CB) { pc = addr + 1; READ(pc); state_ = 1; }
        break;
      }
      else if (state == 1) {
    ...
    // QUAD 1
      ...
      if (OP_COL == 6) {
        // ld r, (hl)
        if (state == 0) { pc = addr + 1;  READ(hl);       state_ = 1; break; }
        if (state == 1) { reg_put8(data); READ(pc);       state_ = 0; break; }
      }
      else if (OP_ROW == 6) {
        // st (hl),r
        if (state == 0) { pc = addr + 1;  WRITE(hl, r);   state_ = 1; break; }
        if (state == 1) {                 READ(pc);       state_ = 0; break; }
      }
      else {
        // mv r,r
        if (state == 0) { pc = addr + 1; reg_put8(r);    READ(pc);       state_ = 0; break; }
      }
    }
    ...
    // QUAD 2
      ...
      if (OP_COL == 6) {
        ...
        // alu a, r
        if (state == 0) { pc = addr + 1; ao = alu(OP_ROW, a, r); a = ao.x; update_flags(); READ(pc); state_ = 0; break; }
      }
    }
    ...
    // QUAD 3
      ...
      if (POP_RR) {
        if (state == 0) {
          if (POP_RR) { pc = addr + 1;           READ(sp);       state_ = 1; break; }
        }                                                        
        if (state == 1) {                                        
        ...
        if (state == 0) {
          if (PUSH_RR) { pc = addr + 1;          READ(sp);       state_ = 1; break; }
        }                                        
        if (state == 1) {                        
        ...
        // jp a16
        if (state == 0) { pc = addr + 1;           READ(pc); state_ = 1; break; }
        if (state == 1) { pc = addr + 1; y = data; READ(pc); state_ = 2; break; }
        if (state == 2) { pc = addr + 1; x = data; PASS(pc); state_ = 3; break; }
        if (state == 3) {                          READ(xy); state_ = 0; break; }
      }
      else if (OP_COL == 4) {
        // call cc a16
        if (state == 0)       { pc = addr + 1;           READ(pc);       state_ = 1; break; }
        if (state == 1)       { pc = addr + 1; y = data; READ(pc);       state_ = 2; break; }
        if (state == 2 && tb) { pc = addr + 1; x = data; PASS(sp);       state_ = 3; break; }
        if (state == 2 && nb) { pc = addr + 1;           READ(pc);       state_ = 0; break; }
        if (state == 3)       { sp = addr - 1;           WRITE(sp, pch); state_ = 4; break; }
        if (state == 4)       { sp = addr - 1;           WRITE(sp, pcl); state_ = 5; break; }
        if (state == 5)       { pc = addr + 1;           READ(xy);       state_ = 0; break; }
      }
      else if (OP_COL == 5) {
        // call a16
        if (state == 0) { pc = addr + 1;           READ(pc);       state_ = 1; break; }
        if (state == 1) { pc = addr + 1; y = data; READ(pc);       state_ = 2; break; }
        if (state == 2) { pc = addr + 1; x = data; PASS(sp);       state_ = 3; break; }
        if (state == 3) { sp = addr - 1;           WRITE(sp, pch); state_ = 4; break; }
        if (state == 4) { sp = addr - 1;           WRITE(sp, pcl); state_ = 5; break; }
        if (state == 5) {                          READ(xy);       state_ = 0; break; }
      }
      else if (OP_COL == 6) {
        // alu a, d8
        if (state == 0) { pc = addr + 1;                                                      READ(pc); state_ = 1; break; }
        ...
        // RST NNh
        if (state == 0) { pc = addr + 1;  PASS(sp);       state_ = 1; break; }
        if (state == 1) { sp = addr - 1;  WRITE(sp, pch); state_ = 2; break; }
        if (state == 2) { sp = addr - 1;  WRITE(sp, pcl); state_ = 3; break; }
    ...
    if (state == 0) {
      ...
      if (ADD_HL_BC        ) { ao = alu(0,      l,      c            );  l = ao.x;                  READ(pc);       state_ = 1; }
      if (ADD_HL_DE        ) { ao = alu(0,      l,      e            );  l = ao.x;                  READ(pc);       state_ = 1; }
      if (ADD_HL_HL        ) { ao = alu(0,      l,      l            );  l = ao.x;                  READ(pc);       state_ = 1; }
      ...
      if (INC_DE           ) {                                                                      PASS(de);       state_ = 1; }
      if (DEC_DE           ) {                                                                      PASS(de);       state_ = 1; }
      if (INC_HL           ) {                                                                      PASS(hl);       state_ = 1; }
      if (DEC_HL           ) {                                                                      PASS(hl);       state_ = 1; }
      if (INC_SP           ) {                                                                      PASS(sp);       state_ = 1; }
      if (DEC_SP           ) {                                                                      PASS(sp);       state_ = 1; }
      ...
      if (LDM_A_A8         ) {                                                                      READ(pc);       state_ = 1; }
      if (STM_A8_A         ) {                                                                      READ(pc);       state_ = 1; }

      // 16-bit constant
      if (JP_A16           ) {                                                                      READ(pc);       state_ = 1; }
      if (JP_CC_A16        ) {                                                                      READ(pc);       state_ = 1; }
      if (LD_BC_D16        ) {                                                                      READ(pc);       state_ = 1; }
      if (LD_DE_D16        ) {                                                                      READ(pc);       state_ = 1; }
      if (LD_HL_D16        ) {                                                                      READ(pc);       state_ = 1; }
      if (LD_SP_D16        ) {                                                                      READ(pc);       state_ = 1; }
      if (LDM_A_A16        ) {                                                                      READ(pc);       state_ = 1; }
      if (STM_A16_A        ) {                                                                      READ(pc);       state_ = 1; }
      if (STM_A16_SP       ) {                                                                      READ(pc);       state_ = 1; }

      // indirect read
      if (LDM_A_BC         ) {                                                                      READ(bc);       state_ = 1; }
      ...
      // fast reads
      if (LDM_A_C          ) {                                           y = c;                     READ(xy);       state_ = 1; }

      // fast writes
      if (STM_BC_A         ) {                                                                      WRITE(bc, a);   state_ = 1; }
      if (STM_DE_A         ) {                                                                      WRITE(de, a);   state_ = 1; }
      if (STM_HLM_A        ) {                                                                      WRITE(hl, a);   state_ = 1; }
      if (STM_HLP_A        ) {                                                                      WRITE(hl, a);   state_ = 1; }
      if (STM_C_A          ) {                                           y = c;                     WRITE(xy, a);   state_ = 1; }
      update_flags();
      break;
    ...
    if (state == 1) {
      ...
      if (ADD_HL_DE        ) {                  ao = alu(1,      h,      d            );  h = ao.x;                  update_flags();    READ(pc);       state_ = 0; break; }
      ...
      if (INC_DE           ) { de = addr + 1;                                                                                           READ(pc);       state_ = 0; break; }
      if (INC_HL           ) { hl = addr + 1;                                                                                           READ(pc);       state_ = 0; break; }
      if (INC_SP           ) { sp = addr + 1;                                                                                           READ(pc);       state_ = 0; break; }
      if (DEC_BC           ) { bc = addr - 1;                                                                                           READ(pc);       state_ = 0; break; }
