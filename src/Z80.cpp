...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 1 && JR_R8)                  /**/ { bus = data_in;                               /**/ alu_y = bus; alu_op = 0; alu(); bus = alu_o;                      addr = pc; write = 0; /**/ y = bus; bus = pch;              /**/ alu_x = bus; bus = sxt(data_in);       state_ = 2; }                                                         
    ...
    if (state == 1 && JR_CC_R8 && branch)     /**/ { bus = data_in;                               /**/ alu_y = bus; alu_op = 0; alu(); bus = alu_o;                      addr = pc; write = 0; /**/ y = bus; bus = pch;              /**/ alu_x = bus; bus = sxt(data_in);       state_ = 2; }
    if (state == 2 && JR_CC_R8 && branch)     /**/ { alu_y = bus; alu_op = 1; alu(); bus = alu_o; /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 0; }
    ...
    if (state == 1 && STM_A16_A)              /**/ { pc = addr + 1; bus = data_in;                /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && STM_A16_A)              /**/ { pc = addr + 1; bus = data_in;                /**/ x = bus;                                                          addr = xy; write = 1; /**/                                  /**/                                        state_ = 3; }
    if (state == 3 && STM_A16_A)              /**/ {                                              /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }
                                                                                                                                                                                                                                                          
    if (state == 0 && LDM_A_A16)              /**/ { pc = addr + 1;                               /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && LDM_A_A16)              /**/ { pc = addr + 1; bus = data_in;                /**/ y = bus;                                                          addr = pc; write = 0; /**/                                  /**/                                        state_ = 2; }
    if (state == 2 && LDM_A_A16)              /**/ { pc = addr + 1; bus = data_in;                /**/ x = bus;                                                          addr = xy; write = 0; /**/                                  /**/                                        state_ = 3; }
    ...
    if (state == 0 && ADD_SP_R8)              /**/ { pc = addr + 1; bus = p;                      /**/ alu_y = bus;                                                      addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ADD_SP_R8)              /**/ { pc = addr + 1; bus = data_in;                /**/ alu_x = bus; alu_f = f; alu_op = 0; alu(); bus = alu_o;           addr = pc; write = 0; /**/ p = bus; bus = sxt(data_in);     /**/                    update_flags(0xF0); state_ = 2; }
    if (state == 2 && ADD_SP_R8)              /**/ { alu_x = bus;   bus = s;                      /**/ alu_y = bus; alu_f = f; alu_op = 1; alu(); bus = alu_o;           addr = pc; write = 0; /**/ s = bus;                         /**/                                        state_ = 3; }
    if (state == 3 && ADD_SP_R8)              /**/ {                                              /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && LD_HL_SP_R8)            /**/ { pc = addr + 1; bus = p;                      /**/ alu_y = bus;                                                      addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && LD_HL_SP_R8)            /**/ { pc = addr + 1; bus = data_in;                /**/ alu_x = bus; alu_f = f; alu_op = 0; alu(); bus = alu_o;                      write = 0; /**/ l = bus; bus = s;                /**/ alu_y = bus;       update_flags(0xF0); state_ = 2; }
    if (state == 2 && LD_HL_SP_R8)            /**/ {                bus = sxt(data_in);           /**/ alu_x = bus; alu_f = f; alu_op = 1; alu(); bus = alu_o;                      write = 0; /**/ h = bus;                         /**/                                        state_ = 3; }
    if (state == 3 && LD_HL_SP_R8)            /**/ {                                              /**/                                                                   addr = pc; write = 0; /**/                                  /**/                                        state_ = 0; }

    if (state == 0 && ALU_A_D8)               /**/ { pc = addr + 1; bus = a;                      /**/ alu_x = bus;                                                      addr = pc; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ALU_A_D8)               /**/ { pc = addr + 1; bus = data_in;                /**/ alu_y = bus; alu_f = f; alu_op = OP_ROW; alu(); bus = alu_o;      addr = pc; write = 0; /**/ a = bus;                         /**/                    update_flags(0xF0); state_ = 0; }
                                                                                                  
    if (state == 0 && ALU_A_HL)               /**/ { pc = addr + 1; bus = a;                      /**/ alu_x = bus;                                                      addr = hl; write = 0; /**/                                  /**/                                        state_ = 1; }
    if (state == 1 && ALU_A_HL)               /**/ {                bus = data_in;                /**/ alu_y = bus; alu_f = f; alu_op = OP_ROW; alu(); bus = alu_o;      addr = pc; write = 0; /**/ a = bus;                         /**/                    update_flags(0xF0); state_ = 0; }
