...
void Z80::tock_t12(const uint8_t imask, const uint8_t intf) {
  ...
  if (INT) {
    ...
    if (state == 0 && DEC_BC)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/              pch = adh = inc(pch, inc_c); set_addr(bc, 0); /**/                                                  state_ = 1; }
    if (state == 0 && DEC_DE)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/              pch = adh = inc(pch, inc_c); set_addr(de, 0); /**/                                                  state_ = 1; }
    if (state == 0 && DEC_HL)                 /**/ {                   pcl = adl = inc(pcl, 1);  /**/              pch = adh = inc(pch, inc_c); set_addr(hl, 0); /**/                                                  state_ = 1; }
