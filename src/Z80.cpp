...
#define PASS(A)     { addr = A;           write = 0; read = 0; }
    ...
    if (LD_BC_D16         && state == 1) { pc = addr + 1;                                                          c = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_BC_D16         && state == 2) { pc = addr + 1;                                                          b = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_DE_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_DE_D16         && state == 1) { pc = addr + 1;                                                          e = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_DE_D16         && state == 2) { pc = addr + 1;                                                          d = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_HL_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_HL_D16         && state == 1) { pc = addr + 1;                                                          l = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_HL_D16         && state == 2) { pc = addr + 1;                                                          h = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_SP_D16         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_SP_D16         && state == 1) { pc = addr + 1;                                                          p = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (LD_SP_D16         && state == 2) { pc = addr + 1;                                                          s = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_BC          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(bc);       state_ = 1; break; }
    if (LDM_A_BC          && state == 1) { bc = addr;                                                              a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_DE          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(de);       state_ = 1; break; }
    if (LDM_A_DE          && state == 1) { de = addr;                                                              a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_HLP         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (LDM_A_HLP         && state == 1) { hl = addr + 1;                                                          a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_HLM         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (LDM_A_HLM         && state == 1) { hl = addr - 1;                                                          a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LD_R_D8           && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (LD_R_D8           && state == 1) { pc = addr + 1;                                                          reg_put8(data);                                                                           READ(pc);       state_ = 0; break; }
    ...
    if (LDM_A_A16         && state == 1) { pc = addr + 1;                                                          y = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (LDM_A_A16         && state == 2) { pc = addr + 1;                                                          x = data;                                                                                 READ(xy);       state_ = 3; break; }
    if (LDM_A_A16         && state == 3) {                                                                         a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_R_HL          && state == 0) { pc = addr + 1;                                                                                                                                                    READ(hl);       state_ = 1; break; }
    if (LDM_R_HL          && state == 1) {                                                                         reg_put8(data);                                                                           READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_BC            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_BC            && state == 1) { sp = addr + 1;                                                          c = data;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_BC            && state == 2) { sp = addr + 1;                                                          b = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_DE            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_DE            && state == 1) { sp = addr + 1;                                                          e = data;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_DE            && state == 2) { sp = addr + 1;                                                          d = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_HL            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_HL            && state == 1) { sp = addr + 1;                                                          l = data;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_HL            && state == 2) { sp = addr + 1;                                                          h = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (POP_AF            && state == 0) { pc = addr + 1;                                                                                                                                                    READ(sp);       state_ = 1; break; }
    if (POP_AF            && state == 1) { sp = addr + 1;                                                          f = data;                                                                                 READ(sp);       state_ = 2; break; }
    if (POP_AF            && state == 2) { sp = addr + 1;                                                          a = data;                                                                                 READ(pc);       state_ = 0; break; }
    ...
    if (RET               && state == 1) { sp = addr + 1;                                                          y = data;                                                                                 READ(sp);       state_ = 2; break; }
    if (RET               && state == 2) { sp = addr + 1;                                                          x = data;                                                                                 PASS(xy);       state_ = 3; break; }
    ...
    if (RET_CC && tb      && state == 2) { sp = addr + 1;                                                          y = data;                                                                                 READ(sp);       state_ = 3; break; }
    if (RET_CC && tb      && state == 3) { sp = addr + 1;                                                          x = data;                                                                                 PASS(xy);       state_ = 4; break; }
    ...
    if (RETI              && state == 1) { sp = addr + 1;                                                          y = data;                                                                                 READ(sp);       state_ = 2; break; }
    if (RETI              && state == 2) { sp = addr + 1;                                                          x = data;                                                                                 PASS(xy);       state_ = 3; break; }
    if (RETI              && state == 3) {                                                                                                                                                                   READ(xy);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_A16_A         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (STM_A16_A         && state == 1) { pc = addr + 1;                                                          y = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (STM_A16_A         && state == 2) { pc = addr + 1;                                                          x = data;                                                                                 WRITE(xy, a);   state_ = 3; break; }
    if (STM_A16_A         && state == 3) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_A8          && state == 0) { pc = addr + 1;                                                          x = 0xFF;                                                                                 READ(pc);       state_ = 1; break; }
    if (LDM_A_A8          && state == 1) { pc = addr + 1;                                                          y = data;                                                                                 READ(xy);       state_ = 2; break; }
    if (LDM_A_A8          && state == 2) {                                                                         a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_A8_A          && state == 0) { pc = addr + 1;                                                          x = 0xFF;                                                                                 READ(pc);       state_ = 1; break; }
    if (STM_A8_A          && state == 1) { pc = addr + 1;                                                          y = data;                                                                                 WRITE(xy, a);   state_ = 2; break; }
    if (STM_A8_A          && state == 2) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (LDM_A_C           && state == 0) { pc = addr + 1;                                                          y = c; x = 0xFF;                                                                          READ(xy);       state_ = 1; break; }
    if (LDM_A_C           && state == 1) {                                                                         a = data;                                                                                 READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_C_A           && state == 0) { pc = addr + 1;                                                          x = 0xFF; y = c;                                                                          WRITE(xy, a);   state_ = 1; break; }
    if (STM_C_A           && state == 1) {                                                                                                                                                                   READ(pc);       state_ = 0; break; }
                                                                                                                                                                                                                                  
    if (STM_HL_D8         && state == 0) { pc = addr + 1;                                                                                                                                                    READ(pc);       state_ = 1; break; }
    if (STM_HL_D8         && state == 1) { pc = addr + 1;                                                          y = data;                                                                                 WRITE(hl, y);   state_ = 2; break; }
    ...
    if (STM_A16_SP        && state == 1) { pc = addr + 1;                                                          y = data;                                                                                 READ(pc);       state_ = 2; break; }
    if (STM_A16_SP        && state == 2) { pc = addr + 1;                                                          x = data;                                                                                 WRITE(xy, p);   state_ = 3; break; }
