...
void Z80::tock(const int tcycle_, const uint8_t imask_, const uint8_t intf_) {
  ...
  if (                     state == INT3)   {                  bus_out = 0x00;                                                                                                           addr = pc;               write = false; state_ = INT4;   }
  ...
  if (ADD_HL_RR) {
    ...
    if (ADD_HL_BC       && state == ALU2)   {                  bus_out = 0x00;           ao = alu(1, h, b, f);                     h = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = DECODE; }
    if (ADD_HL_DE       && state == DECODE) { pc = addr + 1;   bus_out = 0x00;           ao = alu(1, l, e, 0);                     l = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = ALU2;   }
    if (ADD_HL_DE       && state == ALU2)   {                  bus_out = 0x00;           ao = alu(1, h, d, f);                     h = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = DECODE; }
    if (ADD_HL_HL       && state == DECODE) { pc = addr + 1;   bus_out = 0x00;           ao = alu(1, l, l, 0);                     l = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = ALU2;   }
    if (ADD_HL_HL       && state == ALU2)   {                  bus_out = 0x00;           ao = alu(1, h, h, f);                     h = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = DECODE; }
    if (ADD_HL_SP       && state == DECODE) { pc = addr + 1;   bus_out = 0x00;           ao = alu(1, l, p, 0);                     l = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = ALU2;   }
    if (ADD_HL_SP       && state == ALU2)   {                  bus_out = 0x00;           ao = alu(1, h, s, f);                     h = ao.x;                            set_flag(ao.f);  addr = pc;               write = false; state_ = DECODE; }
  }
  ...
  if (LDM_A_A8          && state == DECODE) { pc = addr + 1;   hi = 0xFF;                                                                                                                addr = pc;               write = false; state_ = ARG1;   }
  ...
  if (STM_A16_SP        && state == WRITE1) { temp = addr + 1; bus_out = 0x00;                                                     bus_out = s;                                          addr = temp;             write = true;  state_ = WRITE2; }
