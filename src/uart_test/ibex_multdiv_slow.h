...
struct ibex_multdiv_slow {
  ...
  void tock1(
    ...
    switch (operator_i) {
      ...
      case md_op_e::MD_OP_REM: {
        switch (md_state_q) {
          case md_fsm_e::MD_IDLE: {
            // 0 - B = 0 iff B == 0
            alu_operand_a_o = cat(b32(0), b1(1));
            alu_operand_b_o = cat(~op_b_i, b1(1));
            break;
          }
          case md_fsm_e::MD_ABS_A: {
            // ABS(A) = 0 - A
            alu_operand_a_o = cat(b32(0), b1(1));
            alu_operand_b_o = cat(~op_a_i, b1(1));
            break;
          }
          ...
          case md_fsm_e::MD_CHANGE_SIGN: {
            ...
            alu_operand_a_o = cat(b32(0), b1(1));
            alu_operand_b_o = cat(~b32(accum_window_q), b1(1));
            break;
          }
          default: {
            // Division
            alu_operand_a_o = cat(b32(accum_window_q), b1(1)); // it contains the remainder
            alu_operand_b_o = cat(~b32(op_b_shift_q), b1(1));     // -denominator two's compliment
            break;
         }
        }
        break;
      ...
      default: {
        alu_operand_a_o = accum_window_q;
        alu_operand_b_o = cat(~b32(op_b_shift_q), b1(1));
        break;
      }
    }
    ...
    op_a_bw_pp = cat(
      ~b1(b1(op_a_shift_q, 32) & b1(op_b_shift_q, 0)),
      b32(op_a_shift_q & b_0)
    );

    op_a_bw_last_pp = cat(b1(b1(op_a_shift_q, 32) & b1(op_b_shift_q, 0)), b32(~(b32(op_a_shift_q) & b_0)) );
    ...
    one_shift = b33( cat( b32(0), b1(1) ) << multdiv_count_q);
  ...
  void tock2(logic<1>  mult_en_i,
    ...
    if (mult_sel_i || div_sel_i) {
      switch (md_state_q) {
        case md_fsm_e::MD_IDLE : {
          switch (operator_i) {
            ...
            case md_op_e::MD_OP_MULH: {
              ...
              op_a_shift_d   = op_a_ext;
              accum_window_d = cat( b1(1), ~(op_a_ext[32] & op_b_i[0]), b31(op_a_ext, 1) & dup<31>(op_b_i[0])  );
              op_b_shift_d   = op_b_ext >> 1;
        ...
        case md_fsm_e::MD_ABS_B: {
          // remainder
          accum_window_d = cat(b32(0), b1(op_numerator_q, 31));
          // B abs value
        ...
        case md_fsm_e::MD_COMP: {
          multdiv_count_d = multdiv_count_q - b5(1);
          switch (operator_i) {
            case md_op_e::MD_OP_MULL: {
        ...
        case md_fsm_e::MD_LAST: {
          switch (operator_i) {
            ...
            case md_op_e::MD_OP_REM: {
              ...
              accum_window_d = cat(b1(0), b32(next_remainder));
              md_state_d     = md_fsm_e::MD_CHANGE_SIGN;
              break;
            }
            default: break;
  ...
  void tock3(bool rst_n, md_op_e operator_i, logic<1> div_en_i) {
    valid_o = b1(
      (md_state_q == md_fsm_e::MD_FINISH) ||
      (md_state_q == md_fsm_e::MD_LAST && (operator_i == md_op_e::MD_OP_MULL || operator_i == md_op_e::MD_OP_MULH))
    );
